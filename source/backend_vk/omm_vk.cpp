...
namespace vri::vk
{
    namespace
    {
        ...
        VriResult VRI_CALL CreateMicromap(VriDevice* device, const VriMicromapDesc* desc, VriMicromap** out)
        {
            ...
            if (!MakeBuffer(d, sizes.micromapSize,
                            VK_BUFFER_USAGE_MICROMAP_STORAGE_BIT_EXT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
                            m->buffer, m->bufferAlloc))
            { delete m; return VriResult_OutOfMemory; }
            ...
            { vmaDestroyBuffer(d->Allocator(), m->buffer, m->bufferAlloc); delete m; return VriResult_Failure; }

            if (!MakeBuffer(d, sizes.buildScratchSize + scratchAlign,
                            VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
                            m->scratch, m->scratchAlloc))
            {
        ...
        void VRI_CALL CmdBuildMicromap(VriCommandBuffer* cmd, const VriBuildMicromapDesc* desc)
        {
            ...
            mb.dstStageMask = VK_PIPELINE_STAGE_2_MICROMAP_BUILD_BIT_EXT | VK_PIPELINE_STAGE_2_ACCELERATION_STRUCTURE_BUILD_BIT_KHR;
