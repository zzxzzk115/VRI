...
namespace vri::vk
{
    namespace
    {
        ...
        AccelerationStructureVK*  AS(VriAccelerationStructure* h) { return reinterpret_cast<AccelerationStructureVK*>(h); }
        ...
        VkBuildAccelerationStructureFlagsKHR ToBuildFlags(VriAccelerationStructureBuildFlags f)
        {
            ...
            if (f & VriAccelerationStructureBuild_AllowUpdate)     r |= VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_UPDATE_BIT_KHR;
            if (f & VriAccelerationStructureBuild_AllowCompaction) r |= VK_BUILD_ACCELERATION_STRUCTURE_ALLOW_COMPACTION_BIT_KHR;
            if (f & VriAccelerationStructureBuild_PreferFastTrace) r |= VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_TRACE_BIT_KHR;
            if (f & VriAccelerationStructureBuild_PreferFastBuild) r |= VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_BUILD_BIT_KHR;
            if (f & VriAccelerationStructureBuild_LowMemory)       r |= VK_BUILD_ACCELERATION_STRUCTURE_LOW_MEMORY_BIT_KHR;
            return r;
        }
        ...
        void BuildGeoms(DeviceVK* d, const VriAccelerationStructureDesc* desc, bool withAddr, GeomBuild& gb)
        {
            for (uint32_t i = 0; i < desc->geometryCount; ++i)
            {
                ...
                if (g.type == VriAsGeometryType_Triangles)
                {
                    ...
                    vt.indexType = t.indexBuffer
                                       ? (t.indexType == VriIndexType_UInt16 ? VK_INDEX_TYPE_UINT16 : VK_INDEX_TYPE_UINT32)
                                       : VK_INDEX_TYPE_NONE_KHR;
                    if (withAddr)
                    {
                        vt.vertexData.deviceAddress = BufAddr(d, BUF(t.vertexBuffer)->buffer) + t.vertexOffset;
                        if (t.indexBuffer)     vt.indexData.deviceAddress = BufAddr(d, BUF(t.indexBuffer)->buffer) + t.indexOffset;
                ...
                else // instances (TLAS)
                {
                    ...
                    if (withAddr) vi.data.deviceAddress = BufAddr(d, BUF(in.instanceBuffer)->buffer) + in.offset;
        ...
        VriResult VRI_CALL CreateAccelerationStructure(VriDevice* device, const VriAccelerationStructureDesc* desc, VriAccelerationStructure** out)
        {
            ...
            d->Ext().GetAccelerationStructureBuildSizes(d->Device(), VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
                                                        &bi, gb.primCounts.data(), &sizes);
            ...
            a->device = d;
            a->type = bi.type;
            if (!MakeBuffer(d, sizes.accelerationStructureSize,
                            VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
                            a->buffer, a->bufferAlloc))
            { delete a; return VriResult_OutOfMemory; }
            ...
            { vmaDestroyBuffer(d->Allocator(), a->buffer, a->bufferAlloc); delete a; return VriResult_Failure; }

            if (!MakeBuffer(d, sizes.buildScratchSize + scratchAlign,
                            VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
                            a->scratch, a->scratchAlloc))
            {
        ...
        VriResult VRI_CALL CreateRayTracingPipeline(VriDevice* device, const VriRayTracingPipelineDesc* desc, VriPipeline** out)
        {
            ...
            std::vector<VkPipelineShaderStageCreateInfo> stages(desc->shaderNum, {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO});
            ...
            for (VkShaderModule m : modules) if (m == VK_NULL_HANDLE) moduleFailed = true;
            if (moduleFailed)
            {
            ...
            std::vector<VkRayTracingShaderGroupCreateInfoKHR> groups(desc->groupNum, {VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR});
            for (uint32_t i = 0; i < desc->groupNum; ++i)
            {
                ...
                vg.type = g.type == VriShaderGroupType_TrianglesHitGroup  ? VK_RAY_TRACING_SHADER_GROUP_TYPE_TRIANGLES_HIT_GROUP_KHR
                        : g.type == VriShaderGroupType_ProceduralHitGroup ? VK_RAY_TRACING_SHADER_GROUP_TYPE_PROCEDURAL_HIT_GROUP_KHR
                                                                          : VK_RAY_TRACING_SHADER_GROUP_TYPE_GENERAL_KHR;
                vg.generalShader      = g.generalShader == VRI_SHADER_UNUSED      ? VK_SHADER_UNUSED_KHR : g.generalShader;
                vg.closestHitShader   = g.closestHitShader == VRI_SHADER_UNUSED   ? VK_SHADER_UNUSED_KHR : g.closestHitShader;
                vg.anyHitShader       = g.anyHitShader == VRI_SHADER_UNUSED       ? VK_SHADER_UNUSED_KHR : g.anyHitShader;
                vg.intersectionShader = g.intersectionShader == VRI_SHADER_UNUSED ? VK_SHADER_UNUSED_KHR : g.intersectionShader;
            }
            ...
            for (VkShaderModule m : modules) vkDestroyShaderModule(dev, m, nullptr);
            if (vr != VK_SUCCESS)
                return VriResult_Failure;
        ...
        VriResult VRI_CALL GetShaderGroupHandles(VriPipeline* pipeline, uint32_t firstGroup, uint32_t groupNum, size_t dstSize, void* dst)
        {
            ...
            return p->device->Ext().GetRayTracingShaderGroupHandles(p->device->Device(), p->pipeline, firstGroup, groupNum, dstSize, dst) == VK_SUCCESS
        ...
        void VRI_CALL CmdTraceRays(VriCommandBuffer* cmd, const VriDispatchRaysDesc* desc)
        {
            ...
            auto region = [&](const VriStridedBufferRegion& r) {
                ...
                if (r.buffer) { v.deviceAddress = BufAddr(d, BUF(r.buffer)->buffer) + r.offset; v.stride = r.stride; v.size = r.size; }
                return v;
            };
