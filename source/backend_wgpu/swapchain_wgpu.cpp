...
namespace vri::wgpu
{
    namespace
    {
        ...
        WGPUSurface CreateSurface(DeviceWGPU* d, const VriWindowHandle& window)
        {
            ...
            src.hinstance =
                window.handle.win32.hinstance ? window.handle.win32.hinstance : (void*)GetModuleHandleW(nullptr);
        ...
        void VRI_CALL DestroySwapChain(VriSwapChain* swapChain)
        {
            ...
            if (sc->acquired)
                wgpuTextureRelease(sc->acquired);
            if (sc->surface)
            {
        ...
        VriResult VRI_CALL GetSwapChainTextures(VriSwapChain* swapChain, VriTexture** outTextures, uint32_t* ioCount)
        {
            ...
            if (*ioCount >= 1)
            {
                outTextures[0] = ToHandle(&sc->current);
                *ioCount       = 1;
            }
            return VriResult_Success;
        }
