...
namespace vri::d3d12
{
    namespace
    {
        ...
        AccelerationStructureD3D12* AS(VriAccelerationStructure* h) { return reinterpret_cast<AccelerationStructureD3D12*>(h); }

        std::wstring Widen(const char* s)
        {
            std::wstring w;
            if (s) while (*s) w.push_back(static_cast<wchar_t>(*s++));
            return w;
        }
        ...
        {
            D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS r = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_NONE;
            if (f & VriAccelerationStructureBuild_AllowUpdate)     r |= D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_ALLOW_UPDATE;
            if (f & VriAccelerationStructureBuild_AllowCompaction) r |= D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_ALLOW_COMPACTION;
            if (f & VriAccelerationStructureBuild_PreferFastTrace) r |= D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_TRACE;
            if (f & VriAccelerationStructureBuild_PreferFastBuild) r |= D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_BUILD;
            if (f & VriAccelerationStructureBuild_LowMemory)       r |= D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_MINIMIZE_MEMORY;
            return r;
        }
        ...
        void BuildGeoms(const VriAccelerationStructureDesc* desc, bool withAddr, std::vector<D3D12_RAYTRACING_GEOMETRY_DESC>& geoms)
        {
            for (uint32_t i = 0; i < desc->geometryCount; ++i)
            {
                ...
                vg.Flags = (g.flags & VriAsGeometry_Opaque) ? D3D12_RAYTRACING_GEOMETRY_FLAG_OPAQUE : D3D12_RAYTRACING_GEOMETRY_FLAG_NONE;
        ...
        {
            return t == VriAccelerationStructureType_TopLevel ? D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL
                        ...
                        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS& inputs)
        {
            ...
            if (desc->type == VriAccelerationStructureType_TopLevel)
            {
                ...
                if (withAddr) inputs.InstanceDescs = BUF(in.instanceBuffer)->resource->GetGPUVirtualAddress() + in.offset;
            }
            else
            {
        ...
        VriResult VRI_CALL CreateAccelerationStructure(VriDevice* device, const VriAccelerationStructureDesc* desc, VriAccelerationStructure** out)
        {
            ...
            a->device = d; a->type = inputs.Type;
            a->result = MakeUavBuffer(d, info.ResultDataMaxSizeInBytes, D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE);
            a->scratch = MakeUavBuffer(d, info.ScratchDataSizeInBytes, D3D12_RESOURCE_STATE_COMMON);
            if (!a->result || !a->scratch) { delete a; d->ReportError("AS buffer alloc failed"); return VriResult_OutOfMemory; }
            a->address = a->result->GetGPUVirtualAddress();
            *out = ToHandle(a);
            return VriResult_Success;
        }

        void VRI_CALL DestroyAccelerationStructure(VriAccelerationStructure* as) { if (as) delete AS(as); }
        ...
        VriResult VRI_CALL CreateRayTracingPipeline(VriDevice* device, const VriRayTracingPipelineDesc* desc, VriPipeline** out)
        {
            DeviceD3D12* d = Dev(device);
            ComPtr<ID3D12Device5> dev5;
            if (FAILED(d->Device()->QueryInterface(IID_PPV_ARGS(&dev5)))) { d->ReportError("ID3D12Device5 unavailable"); return VriResult_Unsupported; }
            ...
            for (uint32_t i = 0; i < desc->shaderNum; ++i)
            {
                ...
                D3D12_EXPORT_DESC ed = {}; ed.Name = exportNames[i]; ed.Flags = D3D12_EXPORT_FLAG_NONE;
                ...
                ld.NumExports = 1; ld.pExports = &exports.back();
                libs.push_back(ld);
                D3D12_STATE_SUBOBJECT so = {}; so.Type = D3D12_STATE_SUBOBJECT_TYPE_DXIL_LIBRARY; so.pDesc = &libs.back();
                subs.push_back(so);
            }
            ...
            for (uint32_t i = 0; i < desc->groupNum; ++i)
            {
                ...
                else // triangles/procedural hit group
                {
                    ...
                    hg.Type = g.type == VriShaderGroupType_ProceduralHitGroup ? D3D12_HIT_GROUP_TYPE_PROCEDURAL_PRIMITIVE : D3D12_HIT_GROUP_TYPE_TRIANGLES;
                    if (g.closestHitShader != VRI_SHADER_UNUSED)   { names.push_back(Widen(desc->shaders[g.closestHitShader].entryPointName));   hg.ClosestHitShaderImport = names.back().c_str(); }
                    if (g.anyHitShader != VRI_SHADER_UNUSED)       { names.push_back(Widen(desc->shaders[g.anyHitShader].entryPointName));       hg.AnyHitShaderImport = names.back().c_str(); }
                    if (g.intersectionShader != VRI_SHADER_UNUSED) { names.push_back(Widen(desc->shaders[g.intersectionShader].entryPointName)); hg.IntersectionShaderImport = names.back().c_str(); }
                    hitGroups.push_back(hg);
                    D3D12_STATE_SUBOBJECT so = {}; so.Type = D3D12_STATE_SUBOBJECT_TYPE_HIT_GROUP; so.pDesc = &hitGroups.back();
                    subs.push_back(so);
                }
            }
            ...
            p->device = d; p->rootSig = layout->rootSig.Get();
            p->isRt = true; p->isCompute = true; // descriptor sets bind on the compute root
            if (FAILED(dev5->CreateStateObject(&sod, IID_PPV_ARGS(&p->stateObject))))
            { delete p; d->ReportError("CreateStateObject (RT) failed"); return VriResult_Failure; }
            if (FAILED(p->stateObject.As(&p->stateProps)))
            { delete p; d->ReportError("ID3D12StateObjectProperties query failed"); return VriResult_Failure; }
        ...
        VriResult VRI_CALL GetShaderGroupHandles(VriPipeline* pipeline, uint32_t firstGroup, uint32_t groupNum, size_t dstSize, void* dst)
        {
            ...
            if (dstSize < static_cast<size_t>(groupNum) * idSize) return VriResult_InvalidArgument;
            uint8_t* out = static_cast<uint8_t*>(dst);
            for (uint32_t i = 0; i < groupNum; ++i)
            {
                const uint32_t gi = firstGroup + i;
                if (gi >= p->groupNames.size()) return VriResult_InvalidArgument;
                const void* id = p->stateProps->GetShaderIdentifier(p->groupNames[gi].c_str());
                if (!id) return VriResult_Failure;
        ...
        void VRI_CALL CmdTraceRays(VriCommandBuffer* cmd, const VriDispatchRaysDesc* desc)
        {
            ...
            dr.Width = desc->width; dr.Height = desc->height; dr.Depth = desc->depth ? desc->depth : 1;
