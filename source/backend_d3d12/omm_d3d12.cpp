...
namespace vri::d3d12
{
    namespace
    {
        ...
        VriResult VRI_CALL CreateMicromap(VriDevice* device, const VriMicromapDesc* desc, VriMicromap** out)
        {
            ...
            m->device = d; m->format = ToOmmFormat(desc->format);
            m->subdivisionLevel = desc->subdivisionLevel; m->triangleCount = desc->triangleCount;
            m->result = MakeUavBuffer(d, info.ResultDataMaxSizeInBytes, D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE);
            m->scratch = MakeUavBuffer(d, info.ScratchDataSizeInBytes, D3D12_RESOURCE_STATE_COMMON);
            if (!m->result || !m->scratch) { delete m; d->ReportError("OMM buffer alloc failed"); return VriResult_OutOfMemory; }
            m->address = m->result->GetGPUVirtualAddress();
            *out = ToHandle(m);
            return VriResult_Success;
        }

        void VRI_CALL DestroyMicromap(VriMicromap* micromap) { if (micromap) delete MM(micromap); }
        ...
        void VRI_CALL CmdBuildMicromap(VriCommandBuffer* cmd, const VriBuildMicromapDesc* desc)
        {
            ...
            ommDesc.PerOmmDescs.StartAddress = BUF(desc->triangleBuffer)->resource->GetGPUVirtualAddress() + desc->triangleOffset;
