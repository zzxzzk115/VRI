    ...
    namespace
    {
        ...
        bool WrapBackbuffers(SwapChainD3D12* s)
        {
            ...
            for (uint32_t i = 0; i < s->bufferCount; ++i)
            {
                ...
                if (FAILED(s->swapchain->GetBuffer(i, IID_PPV_ARGS(&res)))) return false;
                TextureD3D12* t = new TextureD3D12{};
                t->device = s->device; t->resource = res; t->format = fi.format; t->texelSize = fi.texelSize;
                t->width = s->width; t->height = s->height; t->depth = 1; t->mipNum = 1; t->layerNum = 1; t->sampleCount = 1;
                t->isRenderTarget = true; t->state = D3D12_RESOURCE_STATE_PRESENT;
                s->textures.push_back(t);
            }
            return true;
        }
        void ReleaseBackbuffers(SwapChainD3D12* s) { for (TextureD3D12* t : s->textures) delete t; s->textures.clear(); }
        ...
        VriResult VRI_CALL CreateSwapChain(VriDevice* device, const VriSwapChainDesc* desc, VriSwapChain** out)
        {
            ...
            s->device = d; s->width = desc->width ? desc->width : 1; s->height = desc->height ? desc->height : 1;
            s->format = desc->format; s->bufferCount = desc->textureNum >= 2 ? desc->textureNum : 2; s->vsync = desc->vsync != VRI_FALSE;

            DXGI_SWAP_CHAIN_DESC1 sd = {};
            sd.Width = s->width; sd.Height = s->height; sd.Format = ToDxgiFormat(desc->format).format;
            sd.SampleDesc.Count = 1; sd.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
            sd.BufferCount = s->bufferCount; sd.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD; sd.Scaling = DXGI_SCALING_STRETCH;
            sd.AlphaMode = DXGI_ALPHA_MODE_IGNORE;

            ComPtr<IDXGISwapChain1> sc1;
            if (FAILED(d->Factory()->CreateSwapChainForHwnd(queue, hwnd, &sd, nullptr, nullptr, &sc1)))
            { delete s; d->ReportError("CreateSwapChainForHwnd failed"); return VriResult_Failure; }
            if (FAILED(sc1.As(&s->swapchain)))
            { delete s; d->ReportError("IDXGISwapChain3 query failed"); return VriResult_Failure; }
            if (!WrapBackbuffers(s))
            { ReleaseBackbuffers(s); delete s; d->ReportError("swapchain GetBuffer failed"); return VriResult_Failure; }
            *out = ToHandle(s);
        ...
        VriResult VRI_CALL AcquireNextTexture(VriSwapChain* swapChain, VriFence* acquireFence, uint64_t signalValue, uint32_t* outIndex)
        {
            ...
            if (acquireFence) reinterpret_cast<FenceD3D12*>(acquireFence)->fence->Signal(signalValue); // CPU-side signal; image is ready
        ...
        VriResult VRI_CALL Resize(VriSwapChain* swapChain, uint32_t width, uint32_t height)
        {
            ...
            s->width = width ? width : 1; s->height = height ? height : 1;
            if (FAILED(s->swapchain->ResizeBuffers(s->bufferCount, s->width, s->height, ToDxgiFormat(s->format).format, 0)))
            { s->device->ReportError("ResizeBuffers failed"); return VriResult_Failure; }
