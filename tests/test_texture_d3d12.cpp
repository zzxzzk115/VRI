...
TEST_CASE("D3D12: sampled texture (SRV + sampler tables) -> blue (phase 3b)")
{
    ...
    VriDescriptor* viewB = nullptr; REQUIRE(c.CreateTextureView(dev, &vb, &viewB) == VriResult_Success);

    VriSamplerDesc smpDesc{}; smpDesc.magFilter = VriFilter_Linear; smpDesc.minFilter = VriFilter_Linear; smpDesc.addressModeU = VriAddressMode_ClampToEdge; smpDesc.addressModeV = VriAddressMode_ClampToEdge; smpDesc.addressModeW = VriAddressMode_ClampToEdge;
    ...
    VriShaderDesc sh[2]{};
    sh[0].stage = VriShaderStage_Vertex;   sh[0].bytecode = g_triangleTexDxbcVS; sh[0].bytecodeSize = sizeof(g_triangleTexDxbcVS); sh[0].entryPointName = "vertexMain";
    sh[1].stage = VriShaderStage_Fragment; sh[1].bytecode = g_triangleTexDxbcPS; sh[1].bytecodeSize = sizeof(g_triangleTexDxbcPS); sh[1].entryPointName = "fragmentMain";
    ...
    const VriDescriptor* texDescs[1] = {viewA}; upd[0].descriptors = texDescs; upd[0].descriptorNum = 1; upd[0].baseDescriptor = 0;
    const VriDescriptor* smpDescs[1] = {sampler}; upd[1].descriptors = smpDescs; upd[1].descriptorNum = 1; upd[1].baseDescriptor = 0;
    ...
    auto barrier = [&](VriTexture* t, VriLayout before, VriLayout after, VriAccessFlags afterAccess, VriPipelineStageFlags afterStage) {
        VriTextureBarrierDesc tb{}; tb.texture = t; tb.before.layout = before; tb.after.layout = after; tb.after.access = afterAccess; tb.after.stages = afterStage; tb.aspect = VriImageAspect_Color;
    ...
    barrier(texA, VriLayout_Undefined, VriLayout_ColorAttachment, VriAccess_ColorAttachmentWrite, VriPipelineStage_ColorAttachmentOutput);
    {
        VriAttachmentDesc rt{}; rt.view = viewA; rt.loadOp = VriAttachmentLoadOp_Clear; rt.storeOp = VriAttachmentStoreOp_Store;
        rt.clearValue.color.f32[2] = 1.0f; rt.clearValue.color.f32[3] = 1.0f; // blue
        VriAttachmentsDesc att{}; att.colors = &rt; att.colorNum = 1; att.renderArea.width = kW; att.renderArea.height = kH; att.layerNum = 1;
        c.CmdBeginRendering(cmd, &att); c.CmdEndRendering(cmd);
    }
    barrier(texA, VriLayout_ColorAttachment, VriLayout_ShaderResource, VriAccess_ShaderResourceRead, VriPipelineStage_FragmentShader);
    // pass 2: sample texA into texB
    barrier(texB, VriLayout_Undefined, VriLayout_ColorAttachment, VriAccess_ColorAttachmentWrite, VriPipelineStage_ColorAttachmentOutput);
    {
        VriAttachmentDesc rt{}; rt.view = viewB; rt.loadOp = VriAttachmentLoadOp_Clear; rt.storeOp = VriAttachmentStoreOp_Store; rt.clearValue.color.f32[3] = 1.0f;
        VriAttachmentsDesc att{}; att.colors = &rt; att.colorNum = 1; att.renderArea.width = kW; att.renderArea.height = kH; att.layerNum = 1;
    ...
    c.DestroyFence(fence); c.DestroyCommandAllocator(alloc); c.DestroyDescriptor(sampler); c.DestroyDescriptor(viewA); c.DestroyDescriptor(viewB);
    c.DestroyDescriptorPool(pool); c.DestroyPipeline(pipeline); c.DestroyPipelineLayout(layout);
    c.DestroyBuffer(readback); c.DestroyTexture(texA); c.DestroyTexture(texB);
