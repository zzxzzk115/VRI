...
namespace
{
    ...
    bool RunBlend(VriGraphicsAPI api, const void* shader, size_t shaderSize, bool& ran)
    {
        ...
        auto makeUbo = [&](float r, float g, float b, float a) {
            ...
            VriBuffer* u      = nullptr;
            REQUIRE(c.CreateBuffer(dev, &ud, &u) == VriResult_Success);
            VriBufferDesc sd {};
            sd.size           = 16;
            sd.usage          = VriBufferUsage_TransferSrc;
            sd.memoryLocation = VriMemoryLocation_HostUpload;
            VriBuffer* s      = nullptr;
            REQUIRE(c.CreateBuffer(dev, &sd, &s) == VriResult_Success);
            float* m = static_cast<float*>(c.MapBuffer(s, 0, 16));
            REQUIRE(m != nullptr);
            m[0] = r;
            m[1] = g;
            m[2] = b;
        ...
        REQUIRE(c.AllocateDescriptorSets(pool, layout, 0, &setBlue, 1) == VriResult_Success);
        VriDescriptorSet* setRed = nullptr;
        REQUIRE(c.AllocateDescriptorSets(pool, layout, 0, &setRed, 1) == VriResult_Success);
        auto bindUbo = [&](VriDescriptorSet* set, VriBuffer* ubo) {
            ...
            REQUIRE(c.CreateBufferView(dev, &bv, &d) == VriResult_Success);
        ...
        c.CmdCopyBuffer(cmd, uboRed, stRed, &cp);
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                bb[i].after.access  = VriAccess_ConstantBufferRead;
                bb[i].after.stages  = VriPipelineStage_FragmentShader;
            }
            bb[0].buffer = uboBlue;
            bb[1].buffer = uboRed;
        ...
        c.CmdDraw(cmd, &draw); // opaque blue
        c.CmdSetDescriptorSet(cmd, 0, setRed);
        c.CmdDraw(cmd, &draw); // red @ 0.5 over blue
        ...
        const bool purple =
            std::abs(int(px[o + 0]) - 128) <= 4 && px[o + 1] == 0 && std::abs(int(px[o + 2]) - 128) <= 4;
