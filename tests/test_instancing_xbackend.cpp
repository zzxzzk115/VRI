...
namespace
{
    ...
    {
        uint16_t idx[36];
        for (uint32_t f = 0; f < 6; ++f) { uint16_t b = uint16_t(f*4); uint16_t* o = idx+f*6; o[0]=b;o[1]=uint16_t(b+1);o[2]=uint16_t(b+2);o[3]=b;o[4]=uint16_t(b+2);o[5]=uint16_t(b+3); }
        // two instances: translated left / right (depth 0.5), scaled small. Stored column-major.
        Mat4 inst[2] = { Mul(Translate(-0.45f, 0.0f, 0.5f), Scale(0.3f)), Mul(Translate(0.45f, 0.0f, 0.5f), Scale(0.3f)) };
        Mat4 viewProj = Transpose(Mat4{}); // identity -> clip == object space (ortho-ish)

        VriDeviceCreationDesc dc{}; dc.graphicsAPI = api; dc.enableValidation = VRI_TRUE; dc.bestEffort = VRI_TRUE;
        ...
        td.clearValue.color.f32[2] = 1.0f; td.clearValue.color.f32[3] = 1.0f; // match the render-pass clear
        ...
        VriBufferDesc rb{}; rb.size = uint64_t(kW)*kH*4; rb.usage = VriBufferUsage_TransferDst; rb.memoryLocation = VriMemoryLocation_HostReadback;
        VriBuffer* readback = nullptr; REQUIRE(c.CreateBuffer(dev, &rb, &readback) == VriResult_Success);

        VriTextureDesc std_{}; std_.type = VriTextureType_2D; std_.format = VriFormat_RGBA8_UNORM; std_.width = kT; std_.height = kT; std_.depth = 1; std_.mipNum = 1; std_.layerNum = 1; std_.sampleNum = 1;
        std_.usage = VriTextureUsage_ShaderResource | VriTextureUsage_TransferDst; std_.memoryLocation = VriMemoryLocation_Device;
        VriTexture* sampled = nullptr; REQUIRE(c.CreateTexture(dev, &std_, &sampled) == VriResult_Success);
        ...
        VriDescriptor* texView = nullptr; REQUIRE(c.CreateTextureView(dev, &svd, &texView) == VriResult_Success);
        const uint64_t texBytes = uint64_t(kT)*kT*4;
        VriBufferDesc tsb{}; tsb.size = texBytes; tsb.usage = VriBufferUsage_TransferSrc; tsb.memoryLocation = VriMemoryLocation_HostUpload;
        VriBuffer* tstg = nullptr; REQUIRE(c.CreateBuffer(dev, &tsb, &tstg) == VriResult_Success);
        { uint8_t* m = static_cast<uint8_t*>(c.MapBuffer(tstg, 0, texBytes)); for (uint64_t i=0;i<texBytes;i+=4){m[i]=0;m[i+1]=255;m[i+2]=0;m[i+3]=255;} c.UnmapBuffer(tstg); }
        VriSamplerDesc smp{}; smp.magFilter = VriFilter_Nearest; smp.minFilter = VriFilter_Nearest; smp.maxLod = 1.0f;
        smp.addressModeU = VriAddressMode_Repeat; smp.addressModeV = VriAddressMode_Repeat; smp.addressModeW = VriAddressMode_Repeat;
        VriDescriptor* sampler = nullptr; REQUIRE(c.CreateSampler(dev, &smp, &sampler) == VriResult_Success);
        ...
        auto devBuf = [&](uint64_t size, VriBufferUsageFlags usage, const void* src, VriBuffer** outStg) {
            ...
            VriBuffer* s = nullptr; c.CreateBuffer(dev, &sd, &s); std::memcpy(c.MapBuffer(s,0,size), src, size); c.UnmapBuffer(s); *outStg = s; return b; };
        ...
        VriDescriptor* uboView = nullptr; REQUIRE(c.CreateBufferView(dev, &ubv, &uboView) == VriResult_Success);
        ...
        VriDescriptorRangeUpdateDesc upd[3]{}; upd[0].descriptors=d0; upd[0].descriptorNum=1; upd[1].descriptors=d1; upd[1].descriptorNum=1; upd[2].descriptors=d2; upd[2].descriptorNum=1;
        ...
        VriBufferCopyDesc cp{}; cp.size=sizeof(kCube); c.CmdCopyBuffer(cmd, vbuf, vstg, &cp); cp.size=sizeof(idx); c.CmdCopyBuffer(cmd, ibuf, istg, &cp);
        cp.size=sizeof(inst); c.CmdCopyBuffer(cmd, instBuf, nstg, &cp); cp.size=sizeof(Mat4); c.CmdCopyBuffer(cmd, ubo, ustg, &cp);
        { VriBufferBarrierDesc bb[4]{};
          bb[0].buffer=vbuf; bb[0].after.access=VriAccess_VertexBufferRead; bb[1].buffer=ibuf; bb[1].after.access=VriAccess_IndexBufferRead;
          bb[2].buffer=instBuf; bb[2].after.access=VriAccess_VertexBufferRead; bb[3].buffer=ubo; bb[3].after.access=VriAccess_ConstantBufferRead;
          for (auto& b : bb){ b.before.access=VriAccess_CopyDestinationWrite; b.before.stages=VriPipelineStage_Transfer; b.after.stages=VriPipelineStage_VertexInput|VriPipelineStage_VertexShader; }
          VriBarrierGroupDesc g{}; g.buffers=bb; g.bufferNum=4; c.CmdBarrier(cmd,&g); }
        { VriTextureBarrierDesc tb{}; tb.texture=sampled; tb.before.layout=VriLayout_Undefined; tb.before.stages=VriPipelineStage_None; tb.after.access=VriAccess_CopyDestinationWrite; tb.after.layout=VriLayout_CopyDestination; tb.after.stages=VriPipelineStage_Transfer; tb.aspect=VriImageAspect_Color;
          VriBarrierGroupDesc g{}; g.textures=&tb; g.textureNum=1; c.CmdBarrier(cmd,&g); }
        VriBufferTextureCopyDesc tup{}; tup.texture.aspect=VriImageAspect_Color; tup.texture.layerNum=1; tup.texture.width=kT; tup.texture.height=kT; c.CmdUploadBufferToTexture(cmd, sampled, tstg, &tup);
        { VriTextureBarrierDesc tb{}; tb.texture=sampled; tb.before.access=VriAccess_CopyDestinationWrite; tb.before.layout=VriLayout_CopyDestination; tb.before.stages=VriPipelineStage_Transfer; tb.after.access=VriAccess_ShaderResourceRead; tb.after.layout=VriLayout_ShaderResource; tb.after.stages=VriPipelineStage_FragmentShader; tb.aspect=VriImageAspect_Color;
          VriBarrierGroupDesc g{}; g.textures=&tb; g.textureNum=1; c.CmdBarrier(cmd,&g); }
        { VriTextureBarrierDesc tb{}; tb.texture=color; tb.before.layout=VriLayout_Undefined; tb.before.stages=VriPipelineStage_None; tb.after.access=VriAccess_ColorAttachmentWrite; tb.after.layout=VriLayout_ColorAttachment; tb.after.stages=VriPipelineStage_ColorAttachmentOutput; tb.aspect=VriImageAspect_Color;
          VriBarrierGroupDesc g{}; g.textures=&tb; g.textureNum=1; c.CmdBarrier(cmd,&g); }
        VriAttachmentDesc rt{}; rt.view=colorView; rt.loadOp=VriAttachmentLoadOp_Clear; rt.storeOp=VriAttachmentStoreOp_Store; rt.clearValue.color.f32[2]=1.0f; rt.clearValue.color.f32[3]=1.0f; // blue clear
        ...
        VriViewport vp{0,0,float(kW),float(kH),0,1}; c.CmdSetViewports(cmd,&vp,1); VriRect sc{0,0,kW,kH}; c.CmdSetScissors(cmd,&sc,1);
        c.CmdSetPipeline(cmd, pipeline); c.CmdSetPipelineLayout(cmd, layout); c.CmdSetDescriptorSet(cmd, 0, set);
        VriVertexBufferBinding vbs[2]{}; vbs[0].buffer=vbuf; vbs[1].buffer=instBuf; c.CmdSetVertexBuffers(cmd,0,vbs,2);
        c.CmdSetIndexBuffer(cmd, ibuf, 0, VriIndexType_UInt16);
        VriDrawIndexedDesc di{}; di.indexNum=36; di.instanceNum=2; c.CmdDrawIndexed(cmd,&di);
        c.CmdEndRendering(cmd);
        { VriTextureBarrierDesc tb{}; tb.texture=color; tb.before.access=VriAccess_ColorAttachmentWrite; tb.before.layout=VriLayout_ColorAttachment; tb.before.stages=VriPipelineStage_ColorAttachmentOutput; tb.after.access=VriAccess_CopySourceRead; tb.after.layout=VriLayout_CopySource; tb.after.stages=VriPipelineStage_Transfer; tb.aspect=VriImageAspect_Color;
          VriBarrierGroupDesc g{}; g.textures=&tb; g.textureNum=1; c.CmdBarrier(cmd,&g); }
        VriBufferTextureCopyDesc rc{}; rc.texture.aspect=VriImageAspect_Color; rc.texture.layerNum=1; c.CmdReadbackTextureToBuffer(cmd, readback, color, &rc);
        ...
        c.QueueSubmit(queue, &submit); c.Wait(fence, 1);

        const uint8_t* px = static_cast<const uint8_t*>(c.MapBuffer(readback, 0, rb.size));
        auto green = [&](uint32_t x, uint32_t y){ const uint8_t* p = px + (uint64_t(y)*kW + x)*4; return p[0] < 64 && p[1] > 192 && p[2] < 64; };
        auto blue  = [&](uint32_t x, uint32_t y){ const uint8_t* p = px + (uint64_t(y)*kW + x)*4; return p[2] > 192 && p[1] < 64; };
        ...
        c.DestroyPipeline(pipeline); c.DestroyPipelineLayout(layout); c.DestroyDescriptor(sampler);
        c.DestroyDescriptor(uboView); c.DestroyBuffer(ustg); c.DestroyBuffer(ubo); c.DestroyBuffer(nstg); c.DestroyBuffer(instBuf);
        c.DestroyBuffer(istg); c.DestroyBuffer(ibuf); c.DestroyBuffer(vstg); c.DestroyBuffer(vbuf);
        c.DestroyBuffer(tstg); c.DestroyDescriptor(texView); c.DestroyTexture(sampled);
