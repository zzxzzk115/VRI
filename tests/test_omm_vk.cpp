...
{
    Vk vk;
    if (!InitVk(vk)) { MESSAGE("Vulkan unavailable - skipping OMM test"); return; }
...
TEST_CASE("Vulkan OMM: a transparent micromap makes the triangle vanish")
{
    ...
    if (dd->hasOpacityMicromap == VRI_FALSE) { MESSAGE("adapter lacks opacity micromaps - skipping"); return; }
    ...
    auto makeHostBuffer = [&](uint64_t size, VriBufferUsageFlags usage, const void* data) {
        VriBufferDesc bd{};
        bd.size = size; bd.usage = usage; bd.memoryLocation = VriMemoryLocation_HostUpload;
        VriBuffer* b = nullptr;
        REQUIRE(c.CreateBuffer(dev, &bd, &b) == VriResult_Success);
        if (data) { std::memcpy(Map(c, b, size), data, size); c.UnmapBuffer(b); }
        return b;
    };
    ...
    MicromapTriangle ommTri{}; ommTri.dataOffset = 0; ommTri.subdivisionLevel = 0; ommTri.format = 1; // 2-state
    ...
    tlasGeom.instances.instanceBuffer = ibuf; tlasGeom.instances.instanceCount = 1;
    ...
    rbDesc.size = static_cast<uint64_t>(kWidth) * kHeight * 4; rbDesc.usage = VriBufferUsage_TransferDst; rbDesc.memoryLocation = VriMemoryLocation_HostReadback;
    ...
    ranges[1].baseRegister = 1; ranges[1].descriptorNum = 1; ranges[1].descriptorType = VriDescriptorType_StorageTexture;        ranges[1].shaderStages = VriShaderStage_RayGen;
    ...
    VriShaderDesc sh[3]{};
    sh[0].stage = VriShaderStage_RayGen;     sh[0].bytecode = g_rtTriangleSpv; sh[0].bytecodeSize = sizeof(g_rtTriangleSpv); sh[0].entryPointName = "rayGenMain";
    sh[1].stage = VriShaderStage_Miss;       sh[1].bytecode = g_rtTriangleSpv; sh[1].bytecodeSize = sizeof(g_rtTriangleSpv); sh[1].entryPointName = "missMain";
    sh[2].stage = VriShaderStage_ClosestHit; sh[2].bytecode = g_rtTriangleSpv; sh[2].bytecodeSize = sizeof(g_rtTriangleSpv); sh[2].entryPointName = "closestHitMain";
    ...
    mmBuild.dst = micromap; mmBuild.dataBuffer = ommDataBuf; mmBuild.dataOffset = 0; mmBuild.triangleBuffer = ommTriBuf; mmBuild.triangleOffset = 0;
...

    {
        ...
        b.texture = outTex;
        b.before.layout = VriLayout_Undefined; b.before.stages = VriPipelineStage_None;
        ...
        VriBarrierGroupDesc g{}; g.textures = &b; g.textureNum = 1;
        c.CmdBarrier(cmd, &g);
    }
...

    {
        VriTextureBarrierDesc b{};
        b.texture = outTex;
        b.before.access = VriAccess_ShaderResourceStorageWrite; b.before.layout = VriLayout_ShaderResourceStorage; b.before.stages = VriPipelineStage_RayTracingShader;
