...
namespace
{
    ...
    {
        sh[0].stage = VriShaderStage_Vertex;      sh[0].bytecode = g_triangleTessSpv; sh[0].bytecodeSize = sizeof(g_triangleTessSpv); sh[0].entryPointName = "vertexMain";
        sh[1].stage = VriShaderStage_TessControl; sh[1].bytecode = g_triangleTessSpv; sh[1].bytecodeSize = sizeof(g_triangleTessSpv); sh[1].entryPointName = "hullMain";
        sh[2].stage = VriShaderStage_TessEval;    sh[2].bytecode = g_triangleTessSpv; sh[2].bytecodeSize = sizeof(g_triangleTessSpv); sh[2].entryPointName = "domainMain";
        sh[3].stage = VriShaderStage_Fragment;    sh[3].bytecode = g_triangleTessSpv; sh[3].bytecodeSize = sizeof(g_triangleTessSpv); sh[3].entryPointName = "fragmentMain";
    }

    bool RunTess(VriGraphicsAPI api, bool& ran, bool& hasTess)
    {
        ran = false; hasTess = false;
        ...
        VriShaderDesc sh[4]{}; MakeTessShaders(sh);
        ...
        pd.pipelineLayout = layout; pd.shaders = sh; pd.shaderNum = 4;
        ...
        VriDrawDesc draw{}; draw.vertexNum = 3; draw.instanceNum = 1; c.CmdDraw(cmd, &draw); // 3 control points = one patch
...
TEST_CASE("tessellation parity: hull/domain tessellate a triangle green (or Unsupported where absent)")
{
    ...
    if (ran) { CHECK(vk); MESSAGE("Vulkan tessellation=", hasTess); } else { MESSAGE("Vulkan unavailable - skipped"); }

    const bool wgpu = RunTess(VriGraphicsAPI_WebGPU, ran, hasTess);
    if (ran) { CHECK(wgpu); MESSAGE("WebGPU tessellation=", hasTess); } else { MESSAGE("WebGPU unavailable - skipped"); }

    const bool gl = RunTess(VriGraphicsAPI_OpenGL, ran, hasTess);
    if (ran) { CHECK(gl); MESSAGE("OpenGL tessellation=", hasTess); } else { MESSAGE("OpenGL unavailable - skipped"); }
}
