...
namespace
{
    ...
    void RunCube(VriGraphicsAPI api)
    {
        ...
        VriBuffer* tstg = nullptr; REQUIRE(c.CreateBuffer(dev, &sb, &tstg) == VriResult_Success);
        ...
        cp.size = sizeof(idx); c.CmdCopyBuffer(cmd, ibuf, istg, &cp);
        cp.size = sizeof(Mat4); c.CmdCopyBuffer(cmd, ubo, ustg, &cp);
        { VriBufferBarrierDesc bb[3]{};
          bb[0].buffer=vbuf; bb[0].after.access=VriAccess_VertexBufferRead; bb[1].buffer=ibuf; bb[1].after.access=VriAccess_IndexBufferRead; bb[2].buffer=ubo; bb[2].after.access=VriAccess_ConstantBufferRead;
          ...
          VriBarrierGroupDesc g{}; g.buffers=bb; g.bufferNum=3; c.CmdBarrier(cmd,&g); }
        ...
        rt.clearValue.color.f32[0]=0.0f; rt.clearValue.color.f32[1]=0.0f; rt.clearValue.color.f32[2]=1.0f; rt.clearValue.color.f32[3]=1.0f; // blue clear
        ...
        CHECK(px[co+0] < 64);  CHECK(px[co+1] > 192); CHECK(px[co+2] < 64);   // green
