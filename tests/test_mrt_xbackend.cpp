    ...
    bool RunMrt(VriGraphicsAPI api, const void* shader, size_t shaderSize, bool& ran)
    {
        ...
        REQUIRE(c.BeginCommandBuffer(cmd) == VriResult_Success);
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                tb[i].before.layout = VriLayout_Undefined;
                tb[i].before.stages = VriPipelineStage_None;
                tb[i].after.access  = VriAccess_ColorAttachmentWrite;
                tb[i].after.layout  = VriLayout_ColorAttachment;
                tb[i].after.stages  = VriPipelineStage_ColorAttachmentOutput;
...

        {
            ...
            {
                tb[i].texture       = i == 0 ? tex0 : tex1;
                tb[i].before.access = VriAccess_ColorAttachmentWrite;
                tb[i].before.layout = VriLayout_ColorAttachment;
                tb[i].before.stages = VriPipelineStage_ColorAttachmentOutput;
                tb[i].after.access  = VriAccess_CopySourceRead;
                tb[i].after.layout  = VriLayout_CopySource;
                tb[i].after.stages  = VriPipelineStage_Transfer;
                tb[i].aspect        = VriImageAspect_Color;
            }
            VriBarrierGroupDesc g {};
            g.textures   = tb;
