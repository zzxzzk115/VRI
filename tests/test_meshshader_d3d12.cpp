// Mesh shaders (D3D12, DispatchMesh): request the feature, confirm the interface is
// queryable iff the capability is reported, build a mesh+pixel pipeline (pipeline-state
// stream, no vertex input) from DXIL, dispatch one mesh group that emits a triangle,
// and read back the center pixel = red. Skips gracefully when the adapter lacks mesh
// shaders. Mirrors test_meshshader_vk.cpp on D3D12.
...
namespace
{
    ...
    bool Init(Dx& dx)
    {
        ...
        dc.enabledFeatures = VriFeature_MeshShader;
...
TEST_CASE("D3D12 mesh shader: interface availability tracks the reported capability")
{
    Dx dx;
    if (!Init(dx)) { MESSAGE("D3D12 unavailable - skipping mesh shader test"); return; }
    const VriDeviceDesc* d = dx.core.GetDeviceDesc(dx.device);
    VriMeshShaderInterface mesh{};
    const bool queryable = vriGetInterface(dx.device, VRI_INTERFACE_MESHSHADER, sizeof(mesh), &mesh) == VriResult_Success;
    CHECK(queryable == (d->hasMeshShader != VRI_FALSE));
    if (queryable)
        CHECK(mesh.CmdDrawMeshTasks != nullptr);
}

TEST_CASE("D3D12 mesh shader: dispatch one group that emits a triangle")
{
    Dx dx;
    if (!Init(dx)) { MESSAGE("D3D12 unavailable - skipping mesh shader test"); return; }
    const VriCoreInterface& c = dx.core;
    VriDevice* dev = dx.device;
    if (c.GetDeviceDesc(dev)->hasMeshShader == VRI_FALSE) { MESSAGE("adapter lacks mesh shaders - skipping"); return; }
    ...
    VriShaderDesc sh[2]{};
    sh[0].stage = VriShaderStage_Mesh;     sh[0].bytecode = g_triangleMeshDxilMS; sh[0].bytecodeSize = sizeof(g_triangleMeshDxilMS); sh[0].entryPointName = "meshMain";
    sh[1].stage = VriShaderStage_Fragment; sh[1].bytecode = g_meshFragDxilPS;     sh[1].bytecodeSize = sizeof(g_meshFragDxilPS);     sh[1].entryPointName = "fragmentMain";
