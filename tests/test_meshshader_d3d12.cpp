...
TEST_CASE("D3D12 mesh shader: interface availability tracks the reported capability")
{
    ...
    if (!Init(dx))
    {
        MESSAGE("D3D12 unavailable - skipping mesh shader test");
        return;
    }
    const VriDeviceDesc*   d = dx.core.GetDeviceDesc(dx.device);
    VriMeshShaderInterface mesh {};
    const bool             queryable =
        vriGetInterface(dx.device, VRI_INTERFACE_MESHSHADER, sizeof(mesh), &mesh) == VriResult_Success;
...
TEST_CASE("D3D12 mesh shader: dispatch one group that emits a triangle")
{
    ...
    VriShaderDesc sh[2] {};
    sh[0].stage          = VriShaderStage_Mesh;
    sh[0].bytecode       = g_triangleMeshDxilMS;
    sh[0].bytecodeSize   = sizeof(g_triangleMeshDxilMS);
    sh[0].entryPointName = "meshMain";
    sh[1].stage          = VriShaderStage_Fragment;
    sh[1].bytecode       = g_meshFragDxilPS;
    sh[1].bytecodeSize   = sizeof(g_meshFragDxilPS);
