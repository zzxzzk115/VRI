...
namespace
{
    ...
    bool RunUboTint(VriGraphicsAPI api, const void* shader, size_t shaderSize, bool& ran)
    {
        ...
        vd.texture = color; vd.viewType = VriTextureViewType_2D; vd.format = VriFormat_Unknown; vd.aspect = VriImageAspect_Color;
        ...
        range.baseRegister = 0; range.descriptorNum = 1; range.descriptorType = VriDescriptorType_ConstantBuffer;
        range.shaderStages = VriShaderStage_Fragment;
        VriDescriptorSetDesc setDesc{};
        setDesc.registerSpace = 0; setDesc.ranges = &range; setDesc.rangeNum = 1;
        ...
        shaders[0].stage = VriShaderStage_Vertex;   shaders[0].bytecode = shader; shaders[0].bytecodeSize = shaderSize; shaders[0].entryPointName = "vertexMain";
        shaders[1].stage = VriShaderStage_Fragment; shaders[1].bytecode = shader; shaders[1].bytecodeSize = shaderSize; shaders[1].entryPointName = "fragmentMain";

        VriColorAttachmentDesc ca{};
        ca.format = VriFormat_RGBA8_UNORM; ca.colorWriteMask = VriColorWrite_RGBA;
        VriGraphicsPipelineDesc pd{};
        pd.pipelineLayout = layout; pd.shaders = shaders; pd.shaderNum = 2;
        ...
        pdsc.descriptorSetMaxNum = 1; pdsc.constantBufferMaxNum = 1;
        ...
        uboView.buffer = ubo; uboView.viewType = VriDescriptorType_ConstantBuffer; uboView.offset = 0; uboView.size = 16;
        ...
        update.descriptors = descs; update.descriptorNum = 1; update.baseDescriptor = 0;
        ...
        VriBufferCopyDesc copy{}; copy.size = 16;
        c.CmdCopyBuffer(cmd, ubo, staging, &copy);
        {
            VriBufferBarrierDesc bb{};
            bb.buffer = ubo;
            bb.before.access = VriAccess_CopyDestinationWrite; bb.before.stages = VriPipelineStage_Transfer;
            bb.after.access = VriAccess_ConstantBufferRead;    bb.after.stages = VriPipelineStage_FragmentShader;
            VriBarrierGroupDesc g{}; g.buffers = &bb; g.bufferNum = 1;
            c.CmdBarrier(cmd, &g);
        }
        ...
        // color target: Undefined -> ColorAttachment (Vulkan; WebGPU ignores)
        {
            ...
            VriBarrierGroupDesc g{}; g.textures = &tb; g.textureNum = 1;
            c.CmdBarrier(cmd, &g);
        }

        VriAttachmentDesc rt{};
        rt.view = colorView; rt.loadOp = VriAttachmentLoadOp_Clear; rt.storeOp = VriAttachmentStoreOp_Store; rt.clearValue.color.f32[3] = 1.0f;
        VriAttachmentsDesc att{};
        att.colors = &rt; att.colorNum = 1; att.renderArea.width = kW; att.renderArea.height = kH; att.layerNum = 1;
