...
#include "shaders/triangle_spv.h"  // g_triangleSpv  (solid red triangle, SV_VertexID)
...
namespace
{
    ...
    bool RunMsaa(VriGraphicsAPI api, const void* shader, size_t shaderSize, bool& ran)
    {
        ...
        VriTextureDesc mt{};
        mt.type = VriTextureType_2D; mt.format = VriFormat_RGBA8_UNORM;
        ...
        VriTextureDesc rt{};
        rt.type = VriTextureType_2D; rt.format = VriFormat_RGBA8_UNORM;
        ...
        REQUIRE(c.BeginCommandBuffer(cmd) == VriResult_Success);
        {
            ...
            tb[0].texture = msaa; tb[0].before.layout = VriLayout_Undefined; tb[0].before.stages = VriPipelineStage_None;
            ...
            tb[1].texture = resolve; tb[1].before.layout = VriLayout_Undefined; tb[1].before.stages = VriPipelineStage_None;
            tb[1].after.access = VriAccess_ColorAttachmentWrite; tb[1].after.layout = VriLayout_ColorAttachment; tb[1].after.stages = VriPipelineStage_ColorAttachmentOutput;
        ...
        c.CmdEndRendering(cmd);
        {
            ...
            tb.texture = resolve; tb.before.access = VriAccess_ColorAttachmentWrite; tb.before.layout = VriLayout_ColorAttachment; tb.before.stages = VriPipelineStage_ColorAttachmentOutput;
        ...
        for (uint32_t p = 0; p < kW * kH; ++p)
        {
            ...
            if (r == 255 && g == 0 && b == 0) anyRed = true;            // fully-covered interior
