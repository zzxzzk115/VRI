...
{
    Dx dx;
    if (!Init(dx)) { MESSAGE("D3D12 unavailable - skipping VRS test"); return; }
...
TEST_CASE("D3D12 VRS: render a triangle with a 2x2 coarse shading rate")
{
    ...
    if (c.GetDeviceDesc(dev)->hasVariableShadingRate == VRI_FALSE) { MESSAGE("adapter lacks VRS - skipping"); return; }
