// Conservative rasterization (Vulkan VK_EXT_conservative_rasterization + D3D12
// ConservativeRasterizationTier) via the same VRI rasterization-state flag. A
// sub-pixel-thin triangle is drawn twice - once normal, once conservative - and the
// conservative draw must light up STRICTLY MORE pixels (it covers every pixel the
// primitive touches, not just those whose center is inside). Self-skips where
// conservative raster is unavailable.
...
namespace
{
    // 64-wide so the readback row pitch (64*4=256) satisfies D3D12's 256-byte alignment
    // (and stays tightly packed for the pixel count). The triangle is sub-pixel-thin.
    constexpr uint32_t kW = 64, kH = 64;

    void RunConsRast(VriGraphicsAPI api)
    {
        ...
        if (c.GetDeviceDesc(dev)->hasConservativeRaster == VRI_FALSE) { MESSAGE("no conservative raster - skipping"); return; }
        ...
        {
            sh[0].bytecode = g_consrastDxbcVS; sh[0].bytecodeSize = sizeof(g_consrastDxbcVS);
            sh[1].bytecode = g_consrastDxbcPS; sh[1].bytecodeSize = sizeof(g_consrastDxbcPS);
        }
        else
        {
            sh[0].bytecode = g_consrastSpv; sh[0].bytecodeSize = sizeof(g_consrastSpv);
            sh[1].bytecode = g_consrastSpv; sh[1].bytecodeSize = sizeof(g_consrastSpv);
        }

        // Render the thin triangle with conservativeRaster on/off; return # of red pixels.
        auto render = [&](bool conservative) -> uint32_t {
            VriTextureDesc td{};
            ...
            uint32_t count = 0;
            for (uint32_t i = 0; i < kW * kH; ++i) if (px[i * 4 + 0] == 255 && px[i * 4 + 1] == 0 && px[i * 4 + 2] == 0) ++count;
        ...
        const uint32_t normal = render(false);
        const uint32_t conservative = render(true);
        CHECK(conservative > normal);  // conservative covers every touched pixel
        CHECK(conservative > 0);
    }
} // namespace

TEST_CASE("Vulkan: conservative rasterization covers more pixels") { RunConsRast(VriGraphicsAPI_Vulkan); }
TEST_CASE("D3D12: conservative rasterization covers more pixels")  { RunConsRast(VriGraphicsAPI_D3D12); }
