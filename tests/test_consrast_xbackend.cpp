...
namespace
{
    ...
    void RunConsRast(VriGraphicsAPI api)
    {
        ...
        if (c.GetDeviceDesc(dev)->hasConservativeRaster == VRI_FALSE)
        {
            MESSAGE("no conservative raster - skipping");
            return;
        }
        ...
        {
            sh[0].bytecode     = g_consrastDxbcVS;
            sh[0].bytecodeSize = sizeof(g_consrastDxbcVS);
            sh[1].bytecode     = g_consrastDxbcPS;
            sh[1].bytecodeSize = sizeof(g_consrastDxbcPS);
        }
        else
        {
            sh[0].bytecode     = g_consrastSpv;
            sh[0].bytecodeSize = sizeof(g_consrastSpv);
            sh[1].bytecode     = g_consrastSpv;
            sh[1].bytecodeSize = sizeof(g_consrastSpv);
        }
        ...
        auto render = [&](bool conservative) -> uint32_t {
            ...
            for (uint32_t i = 0; i < kW * kH; ++i)
                if (px[i * 4 + 0] == 255 && px[i * 4 + 1] == 0 && px[i * 4 + 2] == 0)
                    ++count;
