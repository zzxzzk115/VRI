...
target("vri-tests")
    ...
    add_files("test_async_compute_xbackend.cpp") -- compute queue + cross-queue timeline-fence sync
    ...
    add_files("test_query_xbackend.cpp")      -- GPU timestamp query pools (VK + D3D12 + Metal + WebGPU + GL)
    add_files("test_occlusion_xbackend.cpp")  -- occlusion queries (VK + D3D12 + Metal + GL)
    ...
    if has_config("vri_backend_vulkan") then
        ...
        add_files("test_external_vk.cpp")     -- external memory/semaphore export (CUDA interop seam)
    end
    ...
    if has_config("vri_backend_d3d12") then
        ...
        add_files("test_bc_upload_d3d12.cpp")
