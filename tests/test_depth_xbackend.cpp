...
namespace
{
    ...
    void FillTri(float* v, float z, float r, float g, float b)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            v[i * 6 + 0] = xy[i][0]; v[i * 6 + 1] = xy[i][1]; v[i * 6 + 2] = z;
            v[i * 6 + 3] = r; v[i * 6 + 4] = g; v[i * 6 + 5] = b;
        }
    }
    ...
    bool RunDepth(VriGraphicsAPI api, const void* shader, size_t shaderSize, bool& ran)
    {
        ...
        // depth target + view
        VriTextureDesc dtd{};
        dtd.type = VriTextureType_2D; dtd.format = VriFormat_D32_SFLOAT;
        ...
        dvd.texture = depth; dvd.viewType = VriTextureViewType_2D; dvd.format = VriFormat_D32_SFLOAT; dvd.aspect = VriImageAspect_Depth;
        ...
        pd.outputMerger.colors = &ca; pd.outputMerger.colorNum = 1; pd.outputMerger.depthStencilFormat = VriFormat_D32_SFLOAT;
        ...
        for (int i = 0; i < 3; ++i)
        {
            VriBufferCopyDesc cp{}; cp.size = 18 * sizeof(float); c.CmdCopyBuffer(cmd, vbs[i], sts[i], &cp);
        }
        {
            ...
            tb[1].texture = depth; tb[1].before.layout = VriLayout_Undefined; tb[1].before.stages = VriPipelineStage_None;
        ...
        for (int i = 0; i < 3; ++i)
        {
            VriVertexBufferBinding vbb{}; vbb.buffer = vbs[i]; vbb.offset = 0;
        ...
        c.DestroyBuffer(vbRed); c.DestroyBuffer(vbGreen); c.DestroyBuffer(vbBlue);
        c.DestroyBuffer(stRed); c.DestroyBuffer(stGreen); c.DestroyBuffer(stBlue);
