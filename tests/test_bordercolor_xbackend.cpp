...
namespace
{
    ...
    void RunBorder(VriGraphicsAPI api)
    {
        ...
        if (c.GetDeviceDesc(dev)->hasCustomBorderColor == VRI_FALSE) { MESSAGE("no custom border color - skipping"); return; }
        ...
        {
            sh[0].bytecode = g_bordercolorDxbcVS; sh[0].bytecodeSize = sizeof(g_bordercolorDxbcVS);
            sh[1].bytecode = g_bordercolorDxbcPS; sh[1].bytecodeSize = sizeof(g_bordercolorDxbcPS);
        }
        else
        {
            sh[0].bytecode = g_bordercolorSpv; sh[0].bytecodeSize = sizeof(g_bordercolorSpv);
            sh[1].bytecode = g_bordercolorSpv; sh[1].bytecodeSize = sizeof(g_bordercolorSpv);
        }
        ...
        VriBuffer* staging = nullptr; REQUIRE(c.CreateBuffer(dev, &sb, &staging) == VriResult_Success);
        { uint8_t* m = static_cast<uint8_t*>(c.MapBuffer(staging, 0, texBytes)); for (uint64_t i = 0; i < texBytes; i += 4) { m[i]=0; m[i+1]=0; m[i+2]=255; m[i+3]=255; } c.UnmapBuffer(staging); }
        ...
        smpDesc.addressModeU = VriAddressMode_ClampToBorder; smpDesc.addressModeV = VriAddressMode_ClampToBorder; smpDesc.addressModeW = VriAddressMode_ClampToBorder;
        smpDesc.maxLod = 1.0f;
        smpDesc.useCustomBorderColor = VRI_TRUE;
        smpDesc.customBorderColor[0] = 0.0f; smpDesc.customBorderColor[1] = 1.0f; smpDesc.customBorderColor[2] = 0.0f; smpDesc.customBorderColor[3] = 1.0f;
        ...
        const VriDescriptor* texDescs[1] = {texView}; const VriDescriptor* smpDescs[1] = {sampler};
        ...
        { VriTextureBarrierDesc tb{}; tb.texture = sampled; tb.before.layout = VriLayout_Undefined; tb.before.stages = VriPipelineStage_None;
        ...
        { VriTextureBarrierDesc tb{}; tb.texture = sampled; tb.before.access = VriAccess_CopyDestinationWrite; tb.before.layout = VriLayout_CopyDestination; tb.before.stages = VriPipelineStage_Transfer;
        ...
        VriRect scs{0, 0, kW, kH}; c.CmdSetScissors(cmd, &scs, 1);
        ...
        c.DestroyBuffer(staging); c.DestroyDescriptor(texView); c.DestroyTexture(sampled);
