...
{
    Vk vk;
    if (!InitVk(vk)) { MESSAGE("Vulkan unavailable - skipping RT test"); return; }
...
TEST_CASE("Vulkan RT: trace a triangle into a storage image")
{
    ...
    rtpd.shaders = sh; rtpd.shaderNum = 3;
    rtpd.groups = groups; rtpd.groupNum = 3;
    ...
    pdsc.descriptorSetMaxNum = 1; pdsc.accelerationStructureMaxNum = 1; pdsc.storageTextureMaxNum = 1;
    ...
    submit.commandBuffers = &cmd; submit.commandBufferNum = 1;
    submit.signalFences = &signal; submit.signalFenceNum = 1;
