...
namespace
{
    ...
    struct Vk
    {
        ...
        ~Vk() { if (device) vriDestroyDevice(device); }
    };
...
{
    Vk vk;
    if (!InitVk(vk)) { MESSAGE("Vulkan unavailable - skipping bindless test"); return; }
...
TEST_CASE("Vulkan bindless: sample a dynamic index into a runtime texture array")
{
    ...
    if (c.GetDeviceDesc(dev)->hasBindless == VRI_FALSE) { MESSAGE("adapter lacks bindless - skipping"); return; }
    ...
    VriTexture* tex[3] = {}; VriDescriptor* texView[3] = {}; VriBuffer* staging[3] = {};
    const uint64_t texBytes = static_cast<uint64_t>(kT) * kT * 4;
    for (int i = 0; i < 3; ++i)
    {
        VriTextureDesc std_{};
        std_.type = VriTextureType_2D; std_.format = VriFormat_RGBA8_UNORM;
        std_.width = kT; std_.height = kT; std_.depth = 1; std_.mipNum = 1; std_.layerNum = 1; std_.sampleNum = 1;
        ...
        VriTextureViewDesc svd{}; svd.texture = tex[i]; svd.viewType = VriTextureViewType_2D; svd.format = VriFormat_Unknown; svd.aspect = VriImageAspect_Color;
        ...
        for (uint64_t b = 0; b < texBytes; b += 4) { m[b]=colors[i][0]; m[b+1]=colors[i][1]; m[b+2]=colors[i][2]; m[b+3]=colors[i][3]; }
    ...
    smpDesc.maxLod = 1.0f;
    VriDescriptor* sampler = nullptr; REQUIRE(c.CreateSampler(dev, &smpDesc, &sampler) == VriResult_Success);

    // UBO holding the index to sample (1 -> green)
    VriBufferDesc ub{}; ub.size = 16; ub.usage = VriBufferUsage_ConstantBuffer; ub.memoryLocation = VriMemoryLocation_HostUpload;
    VriBuffer* ubo = nullptr; REQUIRE(c.CreateBuffer(dev, &ub, &ubo) == VriResult_Success);
    { uint32_t* m = static_cast<uint32_t*>(c.MapBuffer(ubo, 0, 16)); m[0] = 1; c.UnmapBuffer(ubo); }
    ...
    VriDescriptor* uboDescriptor = nullptr; REQUIRE(c.CreateBufferView(dev, &uboView, &uboDescriptor) == VriResult_Success);

    // pipeline layout: set 0 = { UBO@0, Sampler@1, Texture[kArraySize]@2 (bindless, last) }
    VriDescriptorRangeDesc ranges[3]{};
    ranges[0].baseRegister = 0; ranges[0].descriptorNum = 1;          ranges[0].descriptorType = VriDescriptorType_ConstantBuffer; ranges[0].shaderStages = VriShaderStage_Fragment;
    ...
    ranges[2].baseRegister = 2; ranges[2].descriptorNum = kArraySize; ranges[2].descriptorType = VriDescriptorType_Texture;        ranges[2].shaderStages = VriShaderStage_Fragment;
    ...
    VriShaderDesc sh[2]{};
    sh[0].stage = VriShaderStage_Vertex;   sh[0].bytecode = g_bindlessTexSpv; sh[0].bytecodeSize = sizeof(g_bindlessTexSpv); sh[0].entryPointName = "vertexMain";
    sh[1].stage = VriShaderStage_Fragment; sh[1].bytecode = g_bindlessTexSpv; sh[1].bytecodeSize = sizeof(g_bindlessTexSpv); sh[1].entryPointName = "fragmentMain";
    ...
    VriDescriptorPoolDesc pdsc{}; pdsc.descriptorSetMaxNum = 1; pdsc.constantBufferMaxNum = 1; pdsc.samplerMaxNum = 1; pdsc.textureMaxNum = kArraySize;
    ...
    updates[0].descriptors = uboArr; updates[0].descriptorNum = 1; updates[0].baseDescriptor = 0;
    updates[1].descriptors = smpArr; updates[1].descriptorNum = 1; updates[1].baseDescriptor = 0;
    updates[2].descriptors = texArr; updates[2].descriptorNum = 3; updates[2].baseDescriptor = 0;
    ...
    for (int i = 0; i < 3; ++i)
    {
        { VriTextureBarrierDesc tb{}; tb.texture = tex[i]; tb.before.layout = VriLayout_Undefined; tb.before.stages = VriPipelineStage_None;
        ...
        VriBufferTextureCopyDesc up{}; up.texture.aspect = VriImageAspect_Color; up.texture.layerNum = 1; up.texture.width = kT; up.texture.height = kT;
        c.CmdUploadBufferToTexture(cmd, tex[i], staging[i], &up);
        { VriTextureBarrierDesc tb{}; tb.texture = tex[i]; tb.before.access = VriAccess_CopyDestinationWrite; tb.before.layout = VriLayout_CopyDestination; tb.before.stages = VriPipelineStage_Transfer;
          tb.after.access = VriAccess_ShaderResourceRead; tb.after.layout = VriLayout_ShaderResource; tb.after.stages = VriPipelineStage_FragmentShader; tb.aspect = VriImageAspect_Color;
          VriBarrierGroupDesc g{}; g.textures = &tb; g.textureNum = 1; c.CmdBarrier(cmd, &g); }
    }
    { VriTextureBarrierDesc tb{}; tb.texture = color; tb.before.layout = VriLayout_Undefined; tb.before.stages = VriPipelineStage_None;
      tb.after.access = VriAccess_ColorAttachmentWrite; tb.after.layout = VriLayout_ColorAttachment; tb.after.stages = VriPipelineStage_ColorAttachmentOutput; tb.aspect = VriImageAspect_Color;
      VriBarrierGroupDesc g{}; g.textures = &tb; g.textureNum = 1; c.CmdBarrier(cmd, &g); }
    ...
    { VriTextureBarrierDesc tb{}; tb.texture = color; tb.before.access = VriAccess_ColorAttachmentWrite; tb.before.layout = VriLayout_ColorAttachment; tb.before.stages = VriPipelineStage_ColorAttachmentOutput;
    ...
    c.DestroyDescriptor(uboDescriptor); c.DestroyBuffer(ubo); c.DestroyDescriptor(sampler);
    for (int i = 0; i < 3; ++i) { c.DestroyBuffer(staging[i]); c.DestroyDescriptor(texView[i]); c.DestroyTexture(tex[i]); }
