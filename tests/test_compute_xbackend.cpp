...
namespace
{
    ...
    bool RunCompute(VriGraphicsAPI api, const void* shader, size_t shaderSize, bool& ran, bool& hasCompute)
    {
        ...
        if (!hasCompute)
        {
            ...
            VriPipelineLayoutDesc ld0 {};
            VriPipelineLayout*    l0 = nullptr;
            c.CreatePipelineLayout(dev, &ld0, &l0);
            VriComputePipelineDesc cpd {};
            cpd.pipelineLayout        = l0;
...
TEST_CASE("compute parity: a compute shader fills a storage buffer (out[i] = i + 100)")
{
    ...
    {
        CHECK(vk);
        MESSAGE("Vulkan compute=", hasCompute);
    }
    else
    {
    ...
    {
        CHECK(wgpu);
        MESSAGE("WebGPU compute=", hasCompute);
    }
    else
    {
    ...
    {
        CHECK(gl);
        MESSAGE("OpenGL compute=", hasCompute);
    }
    else
    {
    ...
    const bool d3d12 =
        RunCompute(VriGraphicsAPI_D3D12, g_computeFillDxbcCS, sizeof(g_computeFillDxbcCS), ran, hasCompute);
    if (ran)
    {
        CHECK(d3d12);
        MESSAGE("D3D12 compute=", hasCompute);
    }
    else
    {
