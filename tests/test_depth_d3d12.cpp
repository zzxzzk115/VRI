...
namespace
{
    ...
    const float kVertices[] = {
        ...
        0.0f, -0.5f, 0.3f, 0.0f, 1.0f, 0.0f,
        0.5f,  0.5f, 0.3f, 0.0f, 1.0f, 0.0f,
       -0.5f,  0.5f, 0.3f, 0.0f, 1.0f, 0.0f,
        // red triangle, z = 0.7 (far)
        0.0f, -0.5f, 0.7f, 1.0f, 0.0f, 0.0f,
...
TEST_CASE("D3D12: depth test keeps the nearer triangle (green over red)")
{
    ...
    VriTexture* depth = nullptr; REQUIRE(c.CreateTexture(dev, &dtd, &depth) == VriResult_Success);
    ...
    VriDescriptor* depthView = nullptr; REQUIRE(c.CreateTextureView(dev, &dvd, &depthView) == VriResult_Success);
    ...
    VriBufferDesc vbd{}; vbd.size = sizeof(kVertices); vbd.usage = VriBufferUsage_VertexBuffer; vbd.memoryLocation = VriMemoryLocation_HostUpload;
    VriBuffer* vbuf = nullptr; REQUIRE(c.CreateBuffer(dev, &vbd, &vbuf) == VriResult_Success);
    { void* m = c.MapBuffer(vbuf, 0, sizeof(kVertices)); REQUIRE(m != nullptr); std::memcpy(m, kVertices, sizeof(kVertices)); c.UnmapBuffer(vbuf); }
    ...
    REQUIRE(c.BeginCommandBuffer(cmd) == VriResult_Success);
    {
        ...
        tb[0].texture = color; tb[0].before.layout = VriLayout_Undefined; tb[0].after.access = VriAccess_ColorAttachmentWrite; tb[0].after.layout = VriLayout_ColorAttachment; tb[0].after.stages = VriPipelineStage_ColorAttachmentOutput; tb[0].aspect = VriImageAspect_Color;
        tb[1].texture = depth; tb[1].before.layout = VriLayout_Undefined; tb[1].after.access = VriAccess_DepthStencilAttachmentWrite; tb[1].after.layout = VriLayout_DepthStencilAttachment; tb[1].after.stages = VriPipelineStage_EarlyFragmentTests; tb[1].aspect = VriImageAspect_Depth;
    ...
    VriDrawDesc green{}; green.vertexNum = 3; green.instanceNum = 1; green.baseVertex = 0; c.CmdDraw(cmd, &green); // near
    VriDrawDesc red{}; red.vertexNum = 3; red.instanceNum = 1; red.baseVertex = 3; c.CmdDraw(cmd, &red);          // far -> depth-fails
    ...
    c.DestroyBuffer(vbuf); c.DestroyBuffer(readback); c.DestroyDescriptor(colorView); c.DestroyDescriptor(depthView);
    c.DestroyTexture(color); c.DestroyTexture(depth);
