...
namespace
{
    ...
    bool RunMipmap(VriGraphicsAPI api, const void* shader, size_t shaderSize, bool& ran)
    {
        ...
        std_.width = kMip0; std_.height = kMip0; std_.depth = 1; std_.mipNum = 2; std_.layerNum = 1; std_.sampleNum = 1;
        ...
        VriTextureViewDesc svd{}; svd.texture = sampled; svd.viewType = VriTextureViewType_2D; svd.format = VriFormat_Unknown; svd.aspect = VriImageAspect_Color; svd.baseMip = 1; svd.mipNum = 1;
        ...
        auto makeStaging = [&](uint32_t dim, uint8_t r, uint8_t g, uint8_t b) {
            ...
            VriBufferDesc sb{}; sb.size = bytes; sb.usage = VriBufferUsage_TransferSrc; sb.memoryLocation = VriMemoryLocation_HostUpload;
            VriBuffer* s = nullptr; REQUIRE(c.CreateBuffer(dev, &sb, &s) == VriResult_Success);
            uint8_t* m = static_cast<uint8_t*>(c.MapBuffer(s, 0, bytes)); REQUIRE(m != nullptr);
            for (uint64_t i = 0; i < bytes; i += 4) { m[i] = r; m[i + 1] = g; m[i + 2] = b; m[i + 3] = 255; }
            c.UnmapBuffer(s);
            return s;
        };
        ...
        VriBufferTextureCopyDesc up0{}; up0.texture.aspect = VriImageAspect_Color; up0.texture.layerNum = 1; up0.texture.mip = 0; up0.texture.width = kMip0; up0.texture.height = kMip0;
        c.CmdUploadBufferToTexture(cmd, sampled, stage0, &up0);
        VriBufferTextureCopyDesc up1{}; up1.texture.aspect = VriImageAspect_Color; up1.texture.layerNum = 1; up1.texture.mip = 1; up1.texture.width = kMip1; up1.texture.height = kMip1;
        ...
        c.DestroyBuffer(stage0); c.DestroyBuffer(stage1); c.DestroyBuffer(readback); c.DestroyDescriptor(colorView); c.DestroyTexture(color);
