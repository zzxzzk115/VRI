...
namespace
{
    ...
    void RunRayQuery(VriGraphicsAPI api, const void* cs, size_t csSize)
    {
        ...
        if (!hasRq)
        {
            MESSAGE("ray query unavailable on this adapter - skipping");
            return;
        }
        ...
        cpd.shader.bytecode       = cs;
        cpd.shader.bytecodeSize   = csSize;
        ...
        rt.CmdBuildAccelerationStructure(cmd, &tlasBuild);
        {
            ...
            b.after.stages  = VriPipelineStage_ComputeShader;
        ...
        c.CmdDispatch(cmd, &disp);
        {
            ...
            b.before.stages = VriPipelineStage_ComputeShader;
...
TEST_CASE("Vulkan ray query: inline trace in a compute shader")
{
    RunRayQuery(VriGraphicsAPI_Vulkan, g_rayquerySpv, sizeof(g_rayquerySpv));
}
TEST_CASE("D3D12 ray query: inline trace in a compute shader")
{
    RunRayQuery(VriGraphicsAPI_D3D12, g_rayqueryDxilCS, sizeof(g_rayqueryDxilCS));
}
// Metal transpiles the SPIR-V ray query (OpRayQuery*) to metal::raytracing via SPIRV-Cross.
TEST_CASE("Metal ray query: inline trace in a compute shader")
{
    RunRayQuery(VriGraphicsAPI_Metal, g_rayquerySpv, sizeof(g_rayquerySpv));
}
