...
TEST_CASE("D3D12 OMM: interface availability tracks the reported capability")
{
    ...
    if (!Init(dx))
    {
        MESSAGE("D3D12 unavailable - skipping OMM test");
        return;
    }
...
TEST_CASE("D3D12 OMM: a transparent micromap makes the triangle vanish")
{
    ...
    {
        MESSAGE("adapter/runtime lacks DXR 1.2 opacity micromaps - skipping");
        return;
    }
    ...
    blasGeom.flags                    = VriAsGeometry_None; // not opaque -> OMM consulted
    ...
    sh[0].bytecode       = g_rtTriangleDxilRGEN;
    sh[0].bytecodeSize   = sizeof(g_rtTriangleDxilRGEN);
    sh[0].entryPointName = "rayGenMain";
    sh[1].stage          = VriShaderStage_Miss;
    sh[1].bytecode       = g_rtTriangleDxilMISS;
    sh[1].bytecodeSize   = sizeof(g_rtTriangleDxilMISS);
    sh[1].entryPointName = "missMain";
    sh[2].stage          = VriShaderStage_ClosestHit;
    sh[2].bytecode       = g_rtTriangleDxilCHIT;
    sh[2].bytecodeSize   = sizeof(g_rtTriangleDxilCHIT);
