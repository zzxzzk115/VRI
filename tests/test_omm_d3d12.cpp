// Opacity micromaps (D3D12, DXR 1.2): build a 2-state OMM that marks the triangle
// fully TRANSPARENT, attach it to the BLAS (geometry NOT opaque, no any-hit), and
// trace. A transparent microtriangle discards the hit, so the ray passes through ->
// the center pixel reads back BLACK (where the plain D3D12 RT test gets red). That
// contrast proves the OMM is honored. Self-skips when the runtime/adapter doesn't
// report Raytracing Tier 1.2 (e.g. WARP, or no Agility DXR-1.2 runtime). Mirrors
// test_omm_vk.cpp on D3D12.
...
namespace
{
    ...
    bool Init(Dx& dx)
    {
        ...
        dc.enabledFeatures = VriFeature_RayTracing | VriFeature_OpacityMicromap;
    ...
    struct AsInstance // D3D12_RAYTRACING_INSTANCE_DESC, 64 bytes
    {
        float    transform[12];
    ...
    struct MicromapTriangle // D3D12_RAYTRACING_OPACITY_MICROMAP_DESC, 8 bytes
    {
        uint32_t byteOffset;
...
TEST_CASE("D3D12 OMM: interface availability tracks the reported capability")
{
    Dx dx;
    if (!Init(dx)) { MESSAGE("D3D12 unavailable - skipping OMM test"); return; }
    const VriDeviceDesc* d = dx.core.GetDeviceDesc(dx.device);
    VriOpacityMicromapInterface omm{};
    const bool queryable = vriGetInterface(dx.device, VRI_INTERFACE_OMM, sizeof(omm), &omm) == VriResult_Success;
    CHECK(queryable == (d->hasOpacityMicromap != VRI_FALSE));
    if (queryable)
        CHECK(omm.CmdBuildMicromap != nullptr);
}

TEST_CASE("D3D12 OMM: a transparent micromap makes the triangle vanish")
{
    Dx dx;
    if (!Init(dx)) { MESSAGE("D3D12 unavailable - skipping OMM test"); return; }
    ...
    if (dd->hasOpacityMicromap == VRI_FALSE) { MESSAGE("adapter/runtime lacks DXR 1.2 opacity micromaps - skipping"); return; }
    ...
    const uint8_t ommData[1] = { 0x00 }; // 2-state, 1 microtriangle = transparent
    VriBuffer* ommDataBuf = hostBuf(sizeof(ommData), VriBufferUsage_MicromapBuildInput, ommData);
    MicromapTriangle ommTri{}; ommTri.byteOffset = 0; ommTri.subdivisionLevel = 0; ommTri.format = 1; // OC1_2_STATE
    VriBuffer* ommTriBuf = hostBuf(sizeof(ommTri), VriBufferUsage_MicromapBuildInput, &ommTri);
    const uint32_t ommIndex[1] = { 0 };
    VriBuffer* ommIndexBuf = hostBuf(sizeof(ommIndex), VriBufferUsage_AccelerationBuildInput, ommIndex);

    VriMicromapDesc mmDesc{}; mmDesc.format = VriOpacityMicromapFormat_2State; mmDesc.subdivisionLevel = 0; mmDesc.triangleCount = 1;
    VriMicromap* micromap = nullptr;
    REQUIRE(omm.CreateMicromap(dev, &mmDesc, &micromap) == VriResult_Success);

    VriAsGeometryDesc blasGeom{};
    blasGeom.type = VriAsGeometryType_Triangles; blasGeom.flags = VriAsGeometry_None; // not opaque -> OMM consulted
    ...
    blasGeom.triangles.micromap = micromap; blasGeom.triangles.ommIndexBuffer = ommIndexBuf; blasGeom.triangles.ommIndexType = VriIndexType_UInt32;
    ...
    inst.sbtOffsetAndFlags = 0x1u << 24; // TRIANGLE_CULL_DISABLE (no FORCE_OPAQUE, so OMM is honored)
    inst.blasReference = rt.GetAccelerationStructureDeviceAddress(blas);
    VriBuffer* ibuf = hostBuf(sizeof(inst), VriBufferUsage_AccelerationBuildInput, &inst);
    ...
    VriRayTracingPipelineDesc rtpd{}; rtpd.pipelineLayout = layout; rtpd.shaders = sh; rtpd.shaderNum = 3; rtpd.groups = groups; rtpd.groupNum = 3; rtpd.maxRecursionDepth = 1;
    VriPipeline* pipeline = nullptr; REQUIRE(rt.CreateRayTracingPipeline(dev, &rtpd, &pipeline) == VriResult_Success);

    const uint32_t handleSize = dd->rtShaderGroupHandleSize;
    const uint32_t baseAlign = dd->rtShaderGroupBaseAlignment;
    uint8_t handles[3 * 64] = {}; REQUIRE(handleSize <= 64);
    ...
    VriBuildMicromapDesc mmBuild{}; mmBuild.dst = micromap; mmBuild.dataBuffer = ommDataBuf; mmBuild.dataOffset = 0; mmBuild.triangleBuffer = ommTriBuf; mmBuild.triangleOffset = 0;
    ...
    CHECK(px[center + 0] == 0); // transparent OMM -> ray passes through -> black (not red)
    CHECK(px[center + 1] == 0);
    CHECK(px[center + 2] == 0);
    ...
    rt.DestroyAccelerationStructure(tlas); rt.DestroyAccelerationStructure(blas); omm.DestroyMicromap(micromap);
    c.DestroyBuffer(ibuf); c.DestroyBuffer(ommIndexBuf); c.DestroyBuffer(ommTriBuf); c.DestroyBuffer(ommDataBuf); c.DestroyBuffer(vbuf);
}
