...
namespace
{
    ...
    bool RunAsyncCompute(VriGraphicsAPI api,
                         const void*    shader,
                         size_t         shaderSize,
                         VriQueueType   copyQueueType,
                         bool&          ran,
                         bool&          hasCompute)
    {
        ran        = false;
        ...
        VriQueue* copyQueue = nullptr;
        REQUIRE(c.GetQueue(dev, copyQueueType, 0, &copyQueue) == VriResult_Success);
        VriQueue* compQueue = nullptr;
        REQUIRE(c.GetQueue(dev, VriQueueType_Compute, 0, &compQueue) == VriResult_Success);

        // storage buffer (compute writes) + host readback
        VriBufferDesc sbd {};
        sbd.size           = kCount * sizeof(uint32_t);
        ...
        VriBufferDesc rbd {};
        rbd.size            = kCount * sizeof(uint32_t);
        ...
        VriCommandAllocator* compAlloc = nullptr;
        REQUIRE(c.CreateCommandAllocator(dev, VriQueueType_Compute, &compAlloc) == VriResult_Success);
        VriCommandBuffer* compCmd = nullptr;
        REQUIRE(c.CreateCommandBuffer(compAlloc, &compCmd) == VriResult_Success);
        ...
        c.CmdDispatch(compCmd, &disp);
        {
            ...
            g.buffers   = &bb;
            g.bufferNum = 1;
            c.CmdBarrier(compCmd, &g);
        }
        REQUIRE(c.EndCommandBuffer(compCmd) == VriResult_Success);
        VriFenceSubmitDesc sig1 {};
        sig1.fence  = fence;
        sig1.value  = 1;
        sig1.stages = VriPipelineStage_AllCommands;
        VriQueueSubmitDesc sub1 {};
        sub1.commandBuffers   = &compCmd;
        sub1.commandBufferNum = 1;
        sub1.signalFences     = &sig1;
        sub1.signalFenceNum   = 1;
        c.QueueSubmit(compQueue, &sub1);

        // ---- pass 2: copy queue (Graphics or Transfer) waits for fence value 1, copies, signals value 2 ----
        VriCommandAllocator* copyAlloc = nullptr;
        REQUIRE(c.CreateCommandAllocator(dev, copyQueueType, &copyAlloc) == VriResult_Success);
        VriCommandBuffer* copyCmd = nullptr;
        REQUIRE(c.CreateCommandBuffer(copyAlloc, &copyCmd) == VriResult_Success);
        ...
        VriFenceSubmitDesc wait1 {};
        wait1.fence  = fence;
        wait1.value  = 1;
        wait1.stages = VriPipelineStage_Transfer;
        VriFenceSubmitDesc sig2 {};
        sig2.fence  = fence;
        sig2.value  = 2;
        sig2.stages = VriPipelineStage_AllCommands;
        VriQueueSubmitDesc sub2 {};
        sub2.waitFences       = &wait1;
        sub2.waitFenceNum     = 1;
        sub2.commandBuffers   = &copyCmd;
        sub2.commandBufferNum = 1;
        sub2.signalFences     = &sig2;
        sub2.signalFenceNum   = 1;
        ...
        c.DestroyCommandAllocator(copyAlloc);
        c.DestroyCommandAllocator(compAlloc);
...
TEST_CASE("async compute: compute queue fills a buffer; a graphics- or transfer-queue copy waits on a timeline fence")
{
    ...
    for (const Backend& b : backends)
        for (const CopyQueue& cq : copyQueues)
        {
            ...
            const std::string msg = ran ? std::string(b.name) + " async-compute via " + cq.name +
                                              " copy, compute=" + (hasCompute ? "yes" : "no") :
