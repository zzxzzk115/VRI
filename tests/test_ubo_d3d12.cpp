...
TEST_CASE("D3D12: constant-buffer descriptor -> green triangle (phase 3a)")
{
    ...
    { void* m = c.MapBuffer(staging, 0, 16); REQUIRE(m != nullptr); std::memcpy(m, green, 16); c.UnmapBuffer(staging); }
    ...
    VriShaderDesc sh[2]{};
    sh[0].stage = VriShaderStage_Vertex;   sh[0].bytecode = g_triangleUboDxbcVS; sh[0].bytecodeSize = sizeof(g_triangleUboDxbcVS); sh[0].entryPointName = "vertexMain";
    sh[1].stage = VriShaderStage_Fragment; sh[1].bytecode = g_triangleUboDxbcPS; sh[1].bytecodeSize = sizeof(g_triangleUboDxbcPS); sh[1].entryPointName = "fragmentMain";
    ...
    VriBufferViewDesc uboView{}; uboView.buffer = ubo; uboView.offset = 0; uboView.size = 16;
    ...
    c.DestroyBuffer(ubo); c.DestroyBuffer(staging); c.DestroyBuffer(readback); c.DestroyDescriptor(colorView); c.DestroyTexture(color);
