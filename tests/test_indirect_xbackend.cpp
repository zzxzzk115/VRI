...
namespace
{
    ...
    bool RunIndirect(VriGraphicsAPI api, const void* shader, size_t shaderSize, bool& ran)
    {
        ...
        VriBufferDesc ibd{}; ibd.size = sizeof(kIndirect); ibd.usage = VriBufferUsage_IndirectBuffer | VriBufferUsage_TransferDst; ibd.memoryLocation = VriMemoryLocation_Device;
        VriBuffer* indirect = nullptr; REQUIRE(c.CreateBuffer(dev, &ibd, &indirect) == VriResult_Success);
        VriBufferDesc sbd{}; sbd.size = sizeof(kIndirect); sbd.usage = VriBufferUsage_TransferSrc; sbd.memoryLocation = VriMemoryLocation_HostUpload;
        VriBuffer* staging = nullptr; REQUIRE(c.CreateBuffer(dev, &sbd, &staging) == VriResult_Success);
        {
            void* m = c.MapBuffer(staging, 0, sizeof(kIndirect)); REQUIRE(m != nullptr);
            for (uint64_t i = 0; i < sizeof(kIndirect); ++i) static_cast<uint8_t*>(m)[i] = reinterpret_cast<const uint8_t*>(kIndirect)[i];
        ...
        VriBufferCopyDesc cp{}; cp.size = sizeof(kIndirect); c.CmdCopyBuffer(cmd, indirect, staging, &cp);
        {
            VriBufferBarrierDesc bb{};
            bb.buffer = indirect; bb.before.access = VriAccess_CopyDestinationWrite; bb.before.stages = VriPipelineStage_Transfer;
            bb.after.access = VriAccess_IndirectBufferRead; bb.after.stages = VriPipelineStage_DrawIndirect;
        ...
        c.DestroyBuffer(indirect); c.DestroyBuffer(staging); c.DestroyBuffer(readback);
