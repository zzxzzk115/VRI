// Cross-backend vertex-buffer + indexed-draw parity: the same VRI calls draw a
// triangle from a vertex buffer (position + color attributes) via CmdDrawIndexed
// on each backend, asserting the center pixel is the vertex color (yellow).
...
namespace
{
    ...
    // Interleaved [pos.xyz, color.rgb] per vertex; apex-down triangle covering the
    // center; all vertices yellow so the center pixel is exactly (255,255,0).
    ...
    const uint32_t kIndices[] = {0, 1, 2}; // uint32 so the buffer size is a multiple of 4 (WebGPU map requirement)

    bool RunVertexBuffer(VriGraphicsAPI api, const void* shader, size_t shaderSize, bool& ran)
    {
        ran = false;
        ...
        // device-local vertex/index buffers, filled from host staging (portable upload)
        ...
        // pipeline layout (empty) + vertex input: one stream, two RGB32F attributes
        ...
        attrs[0].offset      = 0;
        attrs[0].streamIndex = 0; // POSITION -> location 0
        attrs[1].format      = VriFormat_RGB32_SFLOAT;
        attrs[1].offset      = 3 * sizeof(float);
        attrs[1].streamIndex = 0; // COLOR0  -> location 1
        ...
        VriBufferCopyDesc vcopy {};
        vcopy.size = sizeof(kVertices);
        c.CmdCopyBuffer(cmd, vbuf, vStaging, &vcopy);
        VriBufferCopyDesc icopy {};
        icopy.size = sizeof(kIndices);
        c.CmdCopyBuffer(cmd, ibuf, iStaging, &icopy);
        {
            VriBufferBarrierDesc bb[2] {};
            bb[0].buffer        = vbuf;
            bb[0].before.access = VriAccess_CopyDestinationWrite;
            bb[0].before.stages = VriPipelineStage_Transfer;
            bb[0].after.access  = VriAccess_VertexBufferRead;
            bb[0].after.stages  = VriPipelineStage_VertexInput;
            bb[1].buffer        = ibuf;
            bb[1].before.access = VriAccess_CopyDestinationWrite;
            bb[1].before.stages = VriPipelineStage_Transfer;
        ...
        c.DestroyBuffer(vStaging);
        c.DestroyBuffer(iStaging);
...
TEST_CASE("vertex-buffer + indexed-draw parity: a vertex-colored triangle on each backend")
{
    bool       ran = false;
    const bool vk  = RunVertexBuffer(VriGraphicsAPI_Vulkan, g_triangleVbufSpv, sizeof(g_triangleVbufSpv), ran);
    if (ran)
    {
        CHECK(vk);
    }
    ...
    const bool wgpu = RunVertexBuffer(VriGraphicsAPI_WebGPU, g_triangleVbufWgsl, sizeof(g_triangleVbufWgsl), ran);
    if (ran)
    {
        CHECK(wgpu);
    }
    ...
    const bool gl = RunVertexBuffer(VriGraphicsAPI_OpenGL, g_triangleVbufSpv, sizeof(g_triangleVbufSpv), ran);
    if (ran)
    {
        CHECK(gl);
    }
