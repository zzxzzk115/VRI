...
namespace
{
    ...
    bool RunCull(VriGraphicsAPI api, const void* shader, size_t shaderSize, bool& ran)
    {
        ...
        { VriVertexBufferBinding vbb{}; vbb.buffer = vbFront; vbb.offset = 0; c.CmdSetVertexBuffers(cmd, 0, &vbb, 1); c.CmdDraw(cmd, &draw); } // front: drawn
        { VriVertexBufferBinding vbb{}; vbb.buffer = vbBack;  vbb.offset = 0; c.CmdSetVertexBuffers(cmd, 0, &vbb, 1); c.CmdDraw(cmd, &draw); } // back: culled
        ...
        c.DestroyBuffer(vbFront); c.DestroyBuffer(vbBack); c.DestroyBuffer(stFront); c.DestroyBuffer(stBack);
