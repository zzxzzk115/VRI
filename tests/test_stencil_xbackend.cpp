...
#include "shaders/tests/triangle_vbuf_dxbc.h" // g_triangleVbufDxbcVS / PS  (D3D12)
...
namespace
{
    ...
    VriPipeline* MakePipeline(const VriCoreInterface& c,
                              VriDevice*              dev,
                              ...
                              bool                    write)
    {
        ...
        if (write)
        {
            face.compareOp = VriCompareOp_Always;
            face.passOp    = VriStencilOp_Replace;
            face.writeMask = 0xFFu;
        }
        else
        {
            face.compareOp = VriCompareOp_Equal;
            face.passOp    = VriStencilOp_Keep;
            face.writeMask = 0x00u;
        }
        face.failOp      = VriStencilOp_Keep;
        face.depthFailOp = VriStencilOp_Keep;
    ...
    bool RunStencil(VriGraphicsAPI api, const void* vs, size_t vsSize, const void* ps, size_t psSize, bool& ran)
    {
        ...
        VriTextureDesc dsd {};
        dsd.type                          = VriTextureType_2D;
        dsd.format                        = VriFormat_D24_UNORM_S8_UINT;
        dsd.width                         = kW;
        dsd.height                        = kH;
        dsd.depth                         = 1;
        dsd.mipNum                        = 1;
        dsd.layerNum                      = 1;
        dsd.sampleNum                     = 1;
        dsd.usage                         = VriTextureUsage_DepthStencilAttachment;
        dsd.memoryLocation                = VriMemoryLocation_Device;
        dsd.clearValue.depthStencil.depth = 1.0f; // match the render-pass clear
        VriTexture* depthStencil          = nullptr;
        REQUIRE(c.CreateTexture(dev, &dsd, &depthStencil) == VriResult_Success);
        VriTextureViewDesc dvd {};
        dvd.texture           = depthStencil;
        dvd.viewType          = VriTextureViewType_2D;
        dvd.format            = VriFormat_Unknown;
        dvd.aspect            = VriImageAspect_Depth | VriImageAspect_Stencil;
        VriDescriptor* dsView = nullptr;
        REQUIRE(c.CreateTextureView(dev, &dvd, &dsView) == VriResult_Success);
        ...
        c.CmdCopyBuffer(cmd, vbLarge, stLarge, &cp);
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                bb[i].before.access = VriAccess_CopyDestinationWrite;
                bb[i].before.stages = VriPipelineStage_Transfer;
                bb[i].after.access  = VriAccess_VertexBufferRead;
                bb[i].after.stages  = VriPipelineStage_VertexInput;
            }
            VriBarrierGroupDesc gb {};
            gb.buffers   = bb;
            gb.bufferNum = 2;
            c.CmdBarrier(cmd, &gb);
        }
        ...
        VriAttachmentDesc dsAtt {};
        dsAtt.view                            = dsView;
        dsAtt.loadOp                          = VriAttachmentLoadOp_Clear;
        dsAtt.storeOp                         = VriAttachmentStoreOp_Store;
        dsAtt.clearValue.depthStencil.depth   = 1.0f;
        dsAtt.clearValue.depthStencil.stencil = 0u;
        ...
        draw.instanceNum = 1;
        {
            ...
            vbb.buffer = vbSmall;
            vbb.offset = 0;
            c.CmdSetPipeline(cmd, writePipe);
        ...
        c.DestroyPipeline(writePipe);
        c.DestroyPipeline(testPipe);
        c.DestroyPipelineLayout(layout);
        c.DestroyBuffer(vbSmall);
        c.DestroyBuffer(vbLarge);
        c.DestroyBuffer(stSmall);
        c.DestroyBuffer(stLarge);
        c.DestroyBuffer(readback);
        c.DestroyDescriptor(colorView);
        c.DestroyDescriptor(dsView);
        c.DestroyTexture(color);
        c.DestroyTexture(depthStencil);
...
{
    bool       ran = false;
    const bool vk  = RunStencil(VriGraphicsAPI_Vulkan,
                               g_triangleVbufSpv,
                               sizeof(g_triangleVbufSpv),
                               g_triangleVbufSpv,
                               sizeof(g_triangleVbufSpv),
                               ran);
    if (ran)
    ...
    const bool d3d12 = RunStencil(VriGraphicsAPI_D3D12,
                                  g_triangleVbufDxbcVS,
                                  sizeof(g_triangleVbufDxbcVS),
                                  g_triangleVbufDxbcPS,
                                  sizeof(g_triangleVbufDxbcPS),
                                  ran);
    if (ran)
    {
        CHECK(d3d12);
    ...
    const bool wgpu = RunStencil(VriGraphicsAPI_WebGPU,
                                 g_triangleVbufWgsl,
                                 sizeof(g_triangleVbufWgsl),
                                 g_triangleVbufWgsl,
                                 sizeof(g_triangleVbufWgsl),
                                 ran);
    if (ran)
    ...
    const bool gl = RunStencil(VriGraphicsAPI_OpenGL,
                               g_triangleVbufSpv,
                               sizeof(g_triangleVbufSpv),
                               g_triangleVbufSpv,
                               sizeof(g_triangleVbufSpv),
