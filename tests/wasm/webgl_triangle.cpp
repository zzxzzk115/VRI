...
#include "shaders/common/triangle_spv.h"     // g_triangleSpv     (solid red, vertex-id only)
#include "shaders/tests/compute_fill_spv.h"  // g_computeFillSpv  (used only for the negative compute check)
#include "shaders/tests/triangle_inst_spv.h" // g_triangleInstSpv (instanced draw)
#include "shaders/tests/triangle_mrt_spv.h"  // g_triangleMrtSpv  (two render targets)
#include "shaders/tests/triangle_tex_spv.h"  // g_triangleTexSpv  (color sampled from a texture)
#include "shaders/tests/triangle_ubo_spv.h"  // g_triangleUboSpv  (color from a UBO descriptor)
#include "shaders/tests/triangle_vbuf_spv.h" // g_triangleVbufSpv (vertex buffer + indexed draw)
...
namespace
{
    ...
    bool RenderAndProbe(bool& topRed, bool& botRed)
    {
        ...
        VriBufferDesc bd {};
        bd.size             = static_cast<uint64_t>(kW) * kH * 4;
    ...
    bool RenderUboAndProbe(bool& centerGreen)
    {
        ...
        c.CreateTextureView(dev, &vd, &colorView);
        ...
        VriBuffer* readback = nullptr;
        c.CreateBuffer(dev, &rb, &readback);
        ...
        VriShaderDesc sh[2] {};
        sh[0].stage          = VriShaderStage_Vertex;
        sh[0].bytecode       = g_triangleUboSpv;
        sh[0].bytecodeSize   = sizeof(g_triangleUboSpv);
        sh[0].entryPointName = "vertexMain";
        sh[1].stage          = VriShaderStage_Fragment;
        sh[1].bytecode       = g_triangleUboSpv;
        sh[1].bytecodeSize   = sizeof(g_triangleUboSpv);
        sh[1].entryPointName = "fragmentMain";
        VriColorAttachmentDesc ca {};
        ca.format         = VriFormat_RGBA8_UNORM;
        ca.colorWriteMask = VriColorWrite_RGBA;
        ...
        c.CreateBufferView(dev, &uboView, &uboDescriptor);
    ...
    bool RenderTexAndProbe(bool& centerBlue)
    {
        ...
        VriDescriptor* colorView = nullptr;
        c.CreateTextureView(dev, &cvd, &colorView);
        ...
        c.CreateTexture(dev, &std_, &sampled);
        ...
        c.CreateTextureView(dev, &svd, &texView);
        ...
        c.CreateSampler(dev, &smpDesc, &sampler);
        ...
        VriShaderDesc sh[2] {};
        sh[0].stage          = VriShaderStage_Vertex;
        sh[0].bytecode       = g_triangleTexSpv;
        sh[0].bytecodeSize   = sizeof(g_triangleTexSpv);
        sh[0].entryPointName = "vertexMain";
        sh[1].stage          = VriShaderStage_Fragment;
        sh[1].bytecode       = g_triangleTexSpv;
        sh[1].bytecodeSize   = sizeof(g_triangleTexSpv);
        sh[1].entryPointName = "fragmentMain";
        VriColorAttachmentDesc ca {};
        ca.format         = VriFormat_RGBA8_UNORM;
        ca.colorWriteMask = VriColorWrite_RGBA;
        ...
        up.texture.width    = kTexW;
        up.texture.height   = kTexH;
    ...
    bool RenderVbufAndProbe(bool& centerYellow)
    {
        ...
        VriBuffer* readback = nullptr;
        c.CreateBuffer(dev, &rbd, &readback);
        ...
        auto hostBuf = [&](const void* data, uint64_t size, VriBufferUsageFlags usage) {
            ...
            for (uint64_t i = 0; i < size; ++i)
                static_cast<uint8_t*>(m)[i] = static_cast<const uint8_t*>(data)[i];
            c.UnmapBuffer(b);
            return b;
        };
        ...
        VriShaderDesc sh[2] {};
        sh[0].stage          = VriShaderStage_Vertex;
        sh[0].bytecode       = g_triangleVbufSpv;
        sh[0].bytecodeSize   = sizeof(g_triangleVbufSpv);
        sh[0].entryPointName = "vertexMain";
        sh[1].stage          = VriShaderStage_Fragment;
        sh[1].bytecode       = g_triangleVbufSpv;
        sh[1].bytecodeSize   = sizeof(g_triangleVbufSpv);
        sh[1].entryPointName = "fragmentMain";
    ...
    bool RenderDepthAndProbe(bool& centerGreen)
    {
        ...
        float tri[3][18];
        fillTri(tri[0], 0.7f, 1, 0, 0);
        fillTri(tri[1], 0.3f, 0, 1, 0);
        fillTri(tri[2], 0.9f, 0, 0, 1);
        ...
        c.CreateTextureView(dev, &dvd, &depthView);
        ...
        auto hostVbuf = [&](const float* data) {
            ...
            VriBufferDesc  bd {};
            bd.size           = sz;
            bd.usage          = VriBufferUsage_VertexBuffer;
            bd.memoryLocation = VriMemoryLocation_HostUpload;
            VriBuffer* b      = nullptr;
            c.CreateBuffer(dev, &bd, &b);
        ...
        if (c.CreateGraphicsPipeline(dev, &pd, &pipeline) != VriResult_Success)
        {
            std::printf("VRI_WEBGL: depth pipeline FAILED\n");
            return false;
        }
        ...
        c.DestroyPipeline(pipeline);
        c.DestroyPipelineLayout(layout);
        c.DestroyBuffer(vb[0]);
        c.DestroyBuffer(vb[1]);
        c.DestroyBuffer(vb[2]);
        c.DestroyBuffer(readback);
    ...
    bool RenderBlendAndProbe(bool& centerPurple)
    {
        ...
        auto hostUbo = [&](float r, float g, float b, float a) {
            VriBufferDesc ud {};
            ud.size           = 16;
            ud.usage          = VriBufferUsage_ConstantBuffer;
            ud.memoryLocation = VriMemoryLocation_HostUpload;
            VriBuffer* u      = nullptr;
            c.CreateBuffer(dev, &ud, &u);
            float* m = static_cast<float*>(c.MapBuffer(u, 0, 16));
            m[0]     = r;
            m[1]     = g;
            m[2]     = b;
            m[3]     = a;
        ...
        if (c.CreateGraphicsPipeline(dev, &pd, &pipeline) != VriResult_Success)
        {
            std::printf("VRI_WEBGL: blend pipeline FAILED\n");
            return false;
        }
        ...
        VriDescriptorPool* pool   = nullptr;
        c.CreateDescriptorPool(dev, &pdsc, &pool);
        VriDescriptorSet* setBlue = nullptr;
        c.AllocateDescriptorSets(pool, layout, 0, &setBlue, 1);
        VriDescriptorSet* setRed = nullptr;
        c.AllocateDescriptorSets(pool, layout, 0, &setRed, 1);
        auto bind = [&](VriDescriptorSet* set, VriBuffer* ubo) {
            ...
            c.CreateBufferView(dev, &bv, &d);
            const VriDescriptor*         arr[1] = {d};
            VriDescriptorRangeUpdateDesc u {};
            u.descriptors    = arr;
            u.descriptorNum  = 1;
            u.baseDescriptor = 0;
        ...
        draw.vertexNum   = 3;
        draw.instanceNum = 1;
        c.CmdSetDescriptorSet(cmd, 0, setBlue);
        c.CmdDraw(cmd, &draw);
        c.CmdSetDescriptorSet(cmd, 0, setRed);
        c.CmdDraw(cmd, &draw);
        c.CmdEndRendering(cmd);
        VriBufferTextureCopyDesc tc {};
        tc.texture.aspect   = VriImageAspect_Color;
        ...
        c.DestroyPipeline(pipeline);
        c.DestroyPipelineLayout(layout);
        c.DestroyDescriptor(dBlue);
        c.DestroyDescriptor(dRed);
        c.DestroyDescriptorPool(pool);
        c.DestroyBuffer(uboBlue);
        c.DestroyBuffer(uboRed);
        c.DestroyBuffer(readback);
        c.DestroyDescriptor(colorView);
        c.DestroyTexture(color);
        vriDestroyDevice(dev);
    ...
    bool RenderCullAndProbe(bool& centerGreen)
    {
        ...
        if (c.CreateGraphicsPipeline(dev, &pd, &pipeline) != VriResult_Success)
        {
            std::printf("VRI_WEBGL: cull pipeline FAILED\n");
            return false;
        }
    ...
    bool RenderMrtAndProbe(bool& ok2)
    {
        ...
        VriTexture*    tex0  = nullptr;
        VriDescriptor* view0 = nullptr;
        makeTarget(tex0, view0);
        VriTexture*    tex1  = nullptr;
        VriDescriptor* view1 = nullptr;
        makeTarget(tex1, view1);
        VriBufferDesc rbd {};
        rbd.size           = static_cast<uint64_t>(kW) * kH * 4;
        rbd.usage          = VriBufferUsage_TransferDst;
        rbd.memoryLocation = VriMemoryLocation_HostReadback;
        VriBuffer* rb0     = nullptr;
        c.CreateBuffer(dev, &rbd, &rb0);
        VriBuffer* rb1 = nullptr;
        c.CreateBuffer(dev, &rbd, &rb1);
        ...
        VriShaderDesc sh[2] {};
        sh[0].stage          = VriShaderStage_Vertex;
        sh[0].bytecode       = g_triangleMrtSpv;
        sh[0].bytecodeSize   = sizeof(g_triangleMrtSpv);
        sh[0].entryPointName = "vertexMain";
        sh[1].stage          = VriShaderStage_Fragment;
        sh[1].bytecode       = g_triangleMrtSpv;
        sh[1].bytecodeSize   = sizeof(g_triangleMrtSpv);
        sh[1].entryPointName = "fragmentMain";
        VriColorAttachmentDesc ca[2] {};
        ca[0].format         = VriFormat_RGBA8_UNORM;
        ca[0].colorWriteMask = VriColorWrite_RGBA;
        ca[1].format         = VriFormat_RGBA8_UNORM;
        ca[1].colorWriteMask = VriColorWrite_RGBA;
        VriGraphicsPipelineDesc pd {};
        pd.pipelineLayout          = layout;
        pd.shaders                 = sh;
        pd.shaderNum               = 2;
        ...
        pd.multisample.sampleNum   = 1;
        pd.outputMerger.colors     = ca;
        pd.outputMerger.colorNum   = 2;
        VriPipeline* pipeline      = nullptr;
        if (c.CreateGraphicsPipeline(dev, &pd, &pipeline) != VriResult_Success)
        {
            std::printf("VRI_WEBGL: mrt pipeline FAILED\n");
            return false;
        }
        ...
        rt[0].view                    = view0;
        rt[0].loadOp                  = VriAttachmentLoadOp_Clear;
        rt[0].storeOp                 = VriAttachmentStoreOp_Store;
        rt[0].clearValue.color.f32[3] = 1.0f;
        rt[1].view                    = view1;
        rt[1].loadOp                  = VriAttachmentLoadOp_Clear;
        rt[1].storeOp                 = VriAttachmentStoreOp_Store;
        rt[1].clearValue.color.f32[3] = 1.0f;
        VriAttachmentsDesc att {};
        att.colors            = rt;
        att.colorNum          = 2;
        att.renderArea.width  = kW;
        ...
        c.DestroyBuffer(rb0);
        c.DestroyBuffer(rb1);
        c.DestroyDescriptor(view0);
        c.DestroyTexture(tex0);
        c.DestroyDescriptor(view1);
        c.DestroyTexture(tex1);
        vriDestroyDevice(dev);
        return true;
    }
    ...
    bool RenderInstanceAndProbe(bool& ok2)
    {
        ...
        VriShaderDesc sh[2] {};
        sh[0].stage          = VriShaderStage_Vertex;
        sh[0].bytecode       = g_triangleInstSpv;
        sh[0].bytecodeSize   = sizeof(g_triangleInstSpv);
        sh[0].entryPointName = "vertexMain";
        sh[1].stage          = VriShaderStage_Fragment;
        sh[1].bytecode       = g_triangleInstSpv;
        sh[1].bytecodeSize   = sizeof(g_triangleInstSpv);
        sh[1].entryPointName = "fragmentMain";
        VriColorAttachmentDesc ca {};
        ca.format         = VriFormat_RGBA8_UNORM;
        ca.colorWriteMask = VriColorWrite_RGBA;
        ...
        if (c.CreateGraphicsPipeline(dev, &pd, &pipeline) != VriResult_Success)
        {
            std::printf("VRI_WEBGL: inst pipeline FAILED\n");
            return false;
        }
    ...
    // Scissor to the right half: a left-of-scissor pixel inside the triangle stays
    // black while a right-of-scissor pixel is red.
    bool RenderScissorAndProbe(bool& ok2)
    {
        ok2 = false;
        VriDeviceCreationDesc dc {};
        dc.graphicsAPI      = VriGraphicsAPI_OpenGLES;
        ...
        if (c.CreateGraphicsPipeline(dev, &pd, &pipeline) != VriResult_Success)
        {
            std::printf("VRI_WEBGL: scissor pipeline FAILED\n");
            return false;
        }
        ...
        if (ok)
        {
            const uint32_t oL        = (24 * kW + 24) * 4;
            const uint32_t oR        = (24 * kW + 40) * 4;
            const bool     leftBlack = px[oL + 0] == 0 && px[oL + 1] == 0 && px[oL + 2] == 0;
            const bool     rightRed  = px[oR + 0] == 255 && px[oR + 1] == 0 && px[oR + 2] == 0;
            ok2                      = leftBlack && rightRed;
            c.UnmapBuffer(readback);
        }
        c.DeviceWaitIdle(dev);
        c.DestroyFence(fence);
    ...
    bool RenderComputeUnsupportedProbe(bool& correct)
    {
        ...
        cpd.shader.bytecode       = g_computeFillSpv;
        cpd.shader.bytecodeSize   = sizeof(g_computeFillSpv);
    ...
    bool RenderMsaaAndProbe(bool& edgeBlend)
    {
        ...
        if (c.CreateTexture(dev, &mt, &msaa) != VriResult_Success)
        {
            std::printf("VRI_WEBGL: msaa texture FAILED\n");
            vriDestroyDevice(dev);
            return false;
        }
        ...
        c.CreateTextureView(dev, &mvd, &msaaView);

        VriTextureDesc rtd {};
        rtd.type            = VriTextureType_2D;
        rtd.format          = VriFormat_RGBA8_UNORM;
        rtd.width           = kW;
        rtd.height          = kH;
        rtd.depth           = 1;
        rtd.mipNum          = 1;
        rtd.layerNum        = 1;
        rtd.sampleNum       = 1;
        rtd.usage           = VriTextureUsage_ColorAttachment | VriTextureUsage_TransferSrc;
        rtd.memoryLocation  = VriMemoryLocation_Device;
        VriTexture* resolve = nullptr;
        c.CreateTexture(dev, &rtd, &resolve);
        ...
        c.CreateTextureView(dev, &rvd, &resolveView);
        ...
        pd.rasterization.cullMode  = VriCullMode_None;
        pd.rasterization.lineWidth = 1.0f;
        pd.multisample.sampleNum   = 4;
        pd.outputMerger.colors     = &ca;
        pd.outputMerger.colorNum   = 1;
        VriPipeline* pipeline      = nullptr;
        if (c.CreateGraphicsPipeline(dev, &pd, &pipeline) != VriResult_Success)
        {
            std::printf("VRI_WEBGL: msaa pipeline FAILED\n");
            vriDestroyDevice(dev);
            return false;
        }
    ...
    bool RenderSwapchainAndProbe(bool& centerRed)
    {
        ...
        scd.window.type                = VriWindowSystem_Web;
        scd.window.handle.web.selector = "#canvas";
        scd.queue                      = queue;
        scd.format                     = VriFormat_RGBA8_UNORM;
        scd.width                      = kW;
        scd.height                     = kH;
        scd.textureNum                 = 2;
        ...
        uint32_t index = 0;
        sw.AcquireNextTexture(swc, nullptr, 0, &index);
        VriTexture* texs[8] = {};
        uint32_t    cnt     = 8;
        sw.GetSwapChainTextures(swc, texs, &cnt);
        VriTexture*        backbuffer = texs[index];
        VriTextureViewDesc vdsc {};
        vdsc.texture        = backbuffer;
        vdsc.viewType       = VriTextureViewType_2D;
        vdsc.format         = VriFormat_Unknown;
        vdsc.aspect         = VriImageAspect_Color;
        VriDescriptor* view = nullptr;
...
int main()
{
    ...
    const bool pass = triPass && uboPass && texPass && vbufPass && depthPass && blendPass && cullPass && mrtPass &&
                      instPass && scissorPass && computeNoPass && msaaPass && swapPass;
    std::printf("VRI_WEBGL_RESULT: %s (triangle: ran=%d topRed=%d botRed=%d | ubo: ran=%d green=%d | tex: ran=%d "
                "blue=%d | vbuf: ran=%d yellow=%d | depth: ran=%d green=%d | blend: ran=%d purple=%d | cull: ran=%d "
                "green=%d | mrt: ran=%d ok=%d | inst: ran=%d ok=%d | scissor: ran=%d ok=%d | compute-unsupported: "
                "ran=%d ok=%d | msaa: ran=%d edge=%d | swapchain: ran=%d red=%d)\n",
                pass ? "PASS" : "FAIL",
                ranTri,
                topRed,
                botRed,
