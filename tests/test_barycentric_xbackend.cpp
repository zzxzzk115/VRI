...
namespace
{
    ...
    void RunBary(VriGraphicsAPI api)
    {
        ...
        if (c.GetDeviceDesc(dev)->hasFragmentShaderBarycentric == VRI_FALSE) { MESSAGE("no barycentrics - skipping"); return; }
        ...
        {
            sh[0].bytecode = g_barycentricDxilVS; sh[0].bytecodeSize = sizeof(g_barycentricDxilVS);
            sh[1].bytecode = g_barycentricDxilPS; sh[1].bytecodeSize = sizeof(g_barycentricDxilPS);
        }
        else
        {
            sh[0].bytecode = g_barycentricSpv; sh[0].bytecodeSize = sizeof(g_barycentricSpv);
            sh[1].bytecode = g_barycentricSpv; sh[1].bytecodeSize = sizeof(g_barycentricSpv);
        }
        ...
        CHECK(sum >= 250); CHECK(sum <= 256);  // barycentrics sum to 1 -> delivered
        CHECK(mx >= 118); CHECK(mx <= 137);    // dominant (apex) weight ~= 0.5 -> ~128
        CHECK(mn >= 54);  CHECK(mn <= 74);     // the other two ~= 0.25 -> ~64
