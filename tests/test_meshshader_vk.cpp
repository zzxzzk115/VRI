// Mesh shaders (Vulkan, VK_EXT_mesh_shader): request the feature, confirm the
...
// mesh pipeline + CmdDrawMeshTasks records and executes cleanly under validation.
// Skips gracefully when the adapter lacks mesh shaders.
...
TEST_CASE("Vulkan mesh shader: interface availability tracks the reported capability")
{
    Vk vk;
    if (!InitVk(vk)) { MESSAGE("Vulkan unavailable - skipping mesh shader test"); return; }

    const VriDeviceDesc* d = vk.core.GetDeviceDesc(vk.device);
    VriMeshShaderInterface mesh{};
    const bool queryable = vriGetInterface(vk.device, VRI_INTERFACE_MESHSHADER, sizeof(mesh), &mesh) == VriResult_Success;
    CHECK(queryable == (d->hasMeshShader != VRI_FALSE));
    if (queryable)
        CHECK(mesh.CmdDrawMeshTasks != nullptr);
}

TEST_CASE("Vulkan mesh shader: dispatch one task that emits a triangle")
{
    Vk vk;
    if (!InitVk(vk)) { MESSAGE("Vulkan unavailable - skipping mesh shader test"); return; }
