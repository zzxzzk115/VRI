...
int main(int, char**)
{
    ...
    for (int z = 0; z < kGrid; ++z)
        for (int y = 0; y < kGrid; ++y)
            for (int x = 0; x < kGrid; ++x)
            {
                ...
                instances.push_back(Mul(Translate(x * kSpacing - half, y * kSpacing - half, z * kSpacing - half),
                                        Mul(RotateY(phase), Scale(0.7f))));
            }
    ...
    else
    {
        sh[0].bytecode = sh[1].bytecode =
            app.useWgsl ? static_cast<const void*>(g_cubeInstWgsl) : static_cast<const void*>(g_cubeInstSpv);
    ...
    app.onUpdate = [ustg](uint64_t) {
        ...
        std::memcpy(app.c.MapBuffer(ustg, 0, sizeof(Mat4)), &viewProj, sizeof(Mat4));
