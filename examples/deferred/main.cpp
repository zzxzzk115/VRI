...
#include "shaders/examples/deferred_light_dxbc.h" // g_deferredLightDxbcVS / PS
#include "shaders/examples/deferred_light_spv.h"  // g_deferredLightSpv
#include "shaders/examples/deferred_light_wgsl.h" // g_deferredLightWgsl
#include "shaders/examples/deferred_mrt_dxbc.h"   // g_deferredMrtDxbcVS / PS (D3D12)
#include "shaders/examples/deferred_mrt_spv.h"    // g_deferredMrtSpv    (Vulkan + OpenGL)
#include "shaders/examples/deferred_mrt_wgsl.h"   // g_deferredMrtWgsl   (WebGPU)
...
namespace
{
    ...
    struct Instance
    {
        float a[4];
        float b[4];
    }; // a: offset.xyz + uvScale, b: tint.rgb + specular
    ...
    void BuildCube(std::vector<Vertex>& v, std::vector<uint16_t>& idx)
    {
        ...
        struct Face
        {
            ...
            float t[3];
            float b[3];
        }; // normal + the face's two in-plane axes
        const Face faces[6] = {
            {{0, 0, 1}, {1, 0, 0}, {0, 1, 0}},   // +Z
            {{0, 0, -1}, {-1, 0, 0}, {0, 1, 0}}, // -Z
            {{1, 0, 0}, {0, 0, -1}, {0, 1, 0}},  // +X
    ...
    struct LightUbo
    {
        float   lightPosition[kLightCount][4];
        float   lightColor[kLightCount][4];
        float   viewPos[4];
        int32_t params[4];
    };
                               ...
                               VriClearValue        clear = {})
    {
        ...
        if (app.c.CreateTexture(app.dev, &td, &t) != VriResult_Success)
            app.Fail("G-buffer CreateTexture failed");
...
int main(int, char**)
{
    ...
    const float           palette[6][3] = {{0.90f, 0.30f, 0.25f},
                                           {0.30f, 0.75f, 0.40f},
                                           {0.25f, 0.45f, 0.95f},
                                           {0.95f, 0.80f, 0.25f},
                                           {0.70f, 0.35f, 0.85f},
    ...
    for (int z = 0; z < kGrid; ++z)
        for (int x = 0; x < kGrid; ++x)
        {
            ...
            in.a[1]          = 0.5f;
            in.a[2]          = z * kSpacing - half;
            in.a[3]          = 1.0f; // uvScale 1
    ...
    cbv.offset             = 0;
    cbv.size               = sizeof(Mat4);
    VriDescriptor* camView = nullptr;
    c.CreateBufferView(app.dev, &cbv, &camView);

    VriBufferDesc lbd {};
    lbd.size            = sizeof(LightUbo);
    lbd.usage           = VriBufferUsage_ConstantBuffer | VriBufferUsage_TransferDst;
    lbd.memoryLocation  = VriMemoryLocation_Device;
    VriBuffer* lightUbo = nullptr;
    c.CreateBuffer(app.dev, &lbd, &lightUbo);
    VriBufferDesc lsd {};
    lsd.size            = sizeof(LightUbo);
    lsd.usage           = VriBufferUsage_TransferSrc;
    lsd.memoryLocation  = VriMemoryLocation_HostUpload;
    VriBuffer* lightStg = nullptr;
    c.CreateBuffer(app.dev, &lsd, &lightStg);
    VriBufferViewDesc lbv {};
    lbv.buffer               = lightUbo;
    lbv.viewType             = VriDescriptorType_ConstantBuffer;
    lbv.offset               = 0;
    lbv.size                 = sizeof(LightUbo);
    VriDescriptor* lightView = nullptr;
    c.CreateBufferView(app.dev, &lbv, &lightView);
    ...
    rsmp.mipmapMode              = VriMipmapMode_Nearest;
    ...
    VriSamplerDesc gsmp {};
    gsmp.magFilter                = VriFilter_Nearest;
    gsmp.minFilter                = VriFilter_Nearest;
    gsmp.mipmapMode               = VriMipmapMode_Nearest;
    gsmp.addressModeU             = VriAddressMode_ClampToEdge;
    gsmp.addressModeV             = VriAddressMode_ClampToEdge;
    gsmp.addressModeW             = VriAddressMode_ClampToEdge;
    gsmp.maxLod                   = 1.0f;
    VriDescriptor* gbufferSampler = nullptr;
    c.CreateSampler(app.dev, &gsmp, &gbufferSampler);

    // ---- the G-buffer: 3 color targets (sampled later) + a depth target ----
    VriDescriptor* gPosView    = nullptr;
    VriDescriptor* gNormalView = nullptr;
    VriDescriptor* gAlbedoView = nullptr;
    VriDescriptor* gDepthView  = nullptr;
    VriTexture*    gPos        = MakeAttachment(app,
                                      kPositionFormat,
                                      VriTextureUsage_ColorAttachment | VriTextureUsage_ShaderResource,
                                      VriImageAspect_Color,
                                      &gPosView);
    VriTexture*    gNormal     = MakeAttachment(app,
                                         kNormalFormat,
                                         VriTextureUsage_ColorAttachment | VriTextureUsage_ShaderResource,
                                         VriImageAspect_Color,
                                         &gNormalView);
    VriTexture*    gAlbedo     = MakeAttachment(app,
                                         kAlbedoFormat,
                                         VriTextureUsage_ColorAttachment | VriTextureUsage_ShaderResource,
                                         VriImageAspect_Color,
                                         &gAlbedoView);
    VriClearValue  gDepthClear {};
    gDepthClear.depthStencil.depth = 1.0f;
    VriTexture* gDepth             = MakeAttachment(
        app, app.depthFormat, VriTextureUsage_DepthStencilAttachment, VriImageAspect_Depth, &gDepthView, gDepthClear);
    ...
    mr[0].shaderStages   = VriShaderStage_Vertex;
    mr[1].baseRegister   = 1;
    ...
    VriPipelineLayout* mrtLayout = nullptr;
    c.CreatePipelineLayout(app.dev, &mld, &mrtLayout);
    ...
    if (useDxbc)
    {
        msh[0].bytecode     = g_deferredMrtDxbcVS;
        msh[0].bytecodeSize = sizeof(g_deferredMrtDxbcVS);
        msh[1].bytecode     = g_deferredMrtDxbcPS;
        msh[1].bytecodeSize = sizeof(g_deferredMrtDxbcPS);
    }
    else
    {
        msh[0].bytecode = msh[1].bytecode =
            useWgsl ? static_cast<const void*>(g_deferredMrtWgsl) : static_cast<const void*>(g_deferredMrtSpv);
    ...
    mattrs[0].format      = VriFormat_RGB32_SFLOAT;
    mattrs[0].offset      = 0;
    mattrs[0].streamIndex = 0; // position
    mattrs[1].format      = VriFormat_RGB32_SFLOAT;
    mattrs[1].offset      = 12;
    mattrs[1].streamIndex = 0; // normal
    mattrs[2].format      = VriFormat_RG32_SFLOAT;
    mattrs[2].offset      = 24;
    mattrs[2].streamIndex = 0; // uv
    mattrs[3].format      = VriFormat_RGBA32_SFLOAT;
    mattrs[3].offset      = 0;
    mattrs[3].streamIndex = 1; // instA
    mattrs[4].format      = VriFormat_RGBA32_SFLOAT;
    mattrs[4].offset      = 16;
    mattrs[4].streamIndex = 1; // instB
    VriVertexStreamDesc mstreams[2] {};
    mstreams[0].stride      = sizeof(Vertex);
    mstreams[0].bindingSlot = 0;
    mstreams[0].stepRate    = VriVertexStepRate_PerVertex;
    mstreams[1].stride      = sizeof(Instance);
    mstreams[1].bindingSlot = 1;
    mstreams[1].stepRate    = VriVertexStepRate_PerInstance;

    VriColorAttachmentDesc gca[3] {};
    gca[0].format         = kPositionFormat;
    gca[0].colorWriteMask = VriColorWrite_RGBA;
    gca[1].format         = kNormalFormat;
    gca[1].colorWriteMask = VriColorWrite_RGBA;
    gca[2].format         = kAlbedoFormat;
    gca[2].colorWriteMask = VriColorWrite_RGBA;
    VriGraphicsPipelineDesc mpd {};
    mpd.pipelineLayout                  = mrtLayout;
    mpd.shaders                         = msh;
    mpd.shaderNum                       = 2;
    mpd.vertexInput.attributes          = mattrs;
    mpd.vertexInput.attributeNum        = 5;
    mpd.vertexInput.streams             = mstreams;
    mpd.vertexInput.streamNum           = 2;
    ...
    mpd.outputMerger.colors             = gca;
    mpd.outputMerger.colorNum           = 3;
    mpd.outputMerger.depthStencilFormat = app.depthFormat;
    VriPipeline* mrtPipeline            = nullptr;
    if (c.CreateGraphicsPipeline(app.dev, &mpd, &mrtPipeline) != VriResult_Success)
        app.Fail("MRT CreateGraphicsPipeline failed");
    ...
    lr[0].baseRegister   = 0;
    lr[0].descriptorNum  = 1;
    lr[0].descriptorType = VriDescriptorType_ConstantBuffer;
    lr[0].shaderStages   = VriShaderStage_Fragment;
    lr[1].baseRegister   = 1;
    lr[1].descriptorNum  = 1;
    lr[1].descriptorType = VriDescriptorType_Texture;
    lr[1].shaderStages   = VriShaderStage_Fragment;
    lr[2].baseRegister   = 2;
    lr[2].descriptorNum  = 1;
    lr[2].descriptorType = VriDescriptorType_Texture;
    lr[2].shaderStages   = VriShaderStage_Fragment;
    lr[3].baseRegister   = 3;
    lr[3].descriptorNum  = 1;
    lr[3].descriptorType = VriDescriptorType_Texture;
    lr[3].shaderStages   = VriShaderStage_Fragment;
    lr[4].baseRegister   = 4;
    lr[4].descriptorNum  = 1;
    lr[4].descriptorType = VriDescriptorType_Sampler;
    lr[4].shaderStages   = VriShaderStage_Fragment;
    VriDescriptorSetDesc lsetd {};
    lsetd.registerSpace = 0;
    lsetd.ranges        = lr;
    lsetd.rangeNum      = 5;
    VriPipelineLayoutDesc lld {};
    lld.descriptorSets             = &lsetd;
    lld.descriptorSetNum           = 1;
    VriPipelineLayout* lightLayout = nullptr;
    c.CreatePipelineLayout(app.dev, &lld, &lightLayout);

    VriShaderDesc lsh[2] {};
    lsh[0].stage          = VriShaderStage_Vertex;
    lsh[0].entryPointName = "vertexMain";
    lsh[1].stage          = VriShaderStage_Fragment;
    lsh[1].entryPointName = "fragmentMain";
    if (useDxbc)
    {
        lsh[0].bytecode     = g_deferredLightDxbcVS;
        lsh[0].bytecodeSize = sizeof(g_deferredLightDxbcVS);
        lsh[1].bytecode     = g_deferredLightDxbcPS;
        lsh[1].bytecodeSize = sizeof(g_deferredLightDxbcPS);
    }
    else
    {
        lsh[0].bytecode = lsh[1].bytecode =
            useWgsl ? static_cast<const void*>(g_deferredLightWgsl) : static_cast<const void*>(g_deferredLightSpv);
        lsh[0].bytecodeSize = lsh[1].bytecodeSize = useWgsl ? sizeof(g_deferredLightWgsl) : sizeof(g_deferredLightSpv);
    }

    VriColorAttachmentDesc lca {};
    lca.format         = app.swapFormat;
    lca.colorWriteMask = VriColorWrite_RGBA;
    VriGraphicsPipelineDesc lpd {};
    lpd.pipelineLayout          = lightLayout;
    lpd.shaders                 = lsh;
    lpd.shaderNum               = 2;
    lpd.inputAssembly.topology  = VriPrimitiveTopology_TriangleList;
    lpd.rasterization.cullMode  = VriCullMode_None;
    lpd.rasterization.lineWidth = 1.0f;
    lpd.multisample.sampleNum   = 1;
    lpd.outputMerger.colors     = &lca;
    lpd.outputMerger.colorNum   = 1;
    VriPipeline* lightPipeline  = nullptr;
    if (c.CreateGraphicsPipeline(app.dev, &lpd, &lightPipeline) != VriResult_Success)
        app.Fail("lighting CreateGraphicsPipeline failed");

    // ---- descriptor sets ----
    VriDescriptorPoolDesc pdsc {};
    pdsc.descriptorSetMaxNum  = 2;
    pdsc.constantBufferMaxNum = 2;
    pdsc.textureMaxNum        = 4;
    pdsc.samplerMaxNum        = 2;
    VriDescriptorPool* pool   = nullptr;
    c.CreateDescriptorPool(app.dev, &pdsc, &pool);
    VriDescriptorSet* mrtSet = nullptr;
    c.AllocateDescriptorSets(pool, mrtLayout, 0, &mrtSet, 1);
    VriDescriptorSet* lightSet = nullptr;
    c.AllocateDescriptorSets(pool, lightLayout, 0, &lightSet, 1);
    {
        const VriDescriptor*         a[1] = {camView};
        ...
        u[2].descriptors   = s;
        u[2].descriptorNum = 1;
        c.UpdateDescriptorRanges(mrtSet, 0, 3, u);
    }
    {
        const VriDescriptor*         a[1] = {lightView};
        const VriDescriptor*         b[1] = {gPosView};
        const VriDescriptor*         d[1] = {gNormalView};
        const VriDescriptor*         e[1] = {gAlbedoView};
        const VriDescriptor*         s[1] = {gbufferSampler};
        VriDescriptorRangeUpdateDesc u[5] {};
        u[0].descriptors   = a;
        u[0].descriptorNum = 1;
        u[1].descriptors   = b;
        u[1].descriptorNum = 1;
        ...
        u[4].descriptors   = s;
        u[4].descriptorNum = 1;
        c.UpdateDescriptorRanges(lightSet, 0, 5, u);
    }
    ...
    // ---- one-time upload: all geometry buffers + the floor texture (fence value 1) ----
    {
        ...
        for (int i = 0; i < 6; ++i)
        {
            ...
            cp.size = ups[i].size;
            c.CmdCopyBuffer(cmd, ups[i].dst, ups[i].src, &cp);
            gb[i].buffer        = ups[i].dst;
            gb[i].before.access = VriAccess_CopyDestinationWrite;
            gb[i].before.stages = VriPipelineStage_Transfer;
            gb[i].after.access  = ups[i].access;
            gb[i].after.stages  = VriPipelineStage_VertexInput;
        }
    ...
    static float orbit = 1.0f, t = 0.0f, lightT = 0.0f;
    ...
    app.onUpdate             = [camStg, lightStg](uint64_t) {
        ...
        std::memcpy(app.c.MapBuffer(camStg, 0, sizeof(Mat4)), &vp, sizeof(Mat4));
        app.c.UnmapBuffer(camStg);

        LightUbo    lu {};
        const float lightColors[kLightCount][3] = {{1.0f, 1.0f, 1.0f},
                                                   {1.0f, 0.25f, 0.15f},
                                                   {0.2f, 1.0f, 0.3f},
                                                   {0.25f, 0.4f, 1.0f},
                                                   {1.0f, 0.85f, 0.2f},
        ...
        for (int i = 0; i < kLightCount; ++i)
        {
            ...
            lu.lightColor[i][0]    = lightColors[i][0];
            lu.lightColor[i][1]    = lightColors[i][1];
            lu.lightColor[i][2]    = lightColors[i][2];
        ...
        std::memcpy(app.c.MapBuffer(lightStg, 0, sizeof(LightUbo)), &lu, sizeof(LightUbo));
        app.c.UnmapBuffer(lightStg);
    };
    app.onGui = [] {
    ...
    app.onPreRender = [=](VriCommandBuffer* cmd) {
        ...
        VriBufferCopyDesc ccp {};
        ccp.size = sizeof(Mat4);
        c.CmdCopyBuffer(cmd, camUbo, camStg, &ccp);
        VriBufferCopyDesc lcp {};
        lcp.size = sizeof(LightUbo);
        c.CmdCopyBuffer(cmd, lightUbo, lightStg, &lcp);
        ...
        ub[1].after.stages  = VriPipelineStage_FragmentShader;
        ...
        tb[0].texture       = gPos;
        tb[0].before.layout = g_gInit ? VriLayout_ShaderResource : VriLayout_Undefined;
        tb[0].before.stages = g_gInit ? VriPipelineStage_FragmentShader : VriPipelineStage_None;
        ...
        tb[1]               = tb[0];
        tb[1].texture       = gNormal;
        tb[2]               = tb[0];
        tb[2].texture       = gAlbedo;
        tb[3].texture       = gDepth;
        tb[3].before.layout = g_gDepthInit ? VriLayout_DepthStencilAttachment : VriLayout_Undefined;
        tb[3].before.stages = VriPipelineStage_None;
        tb[3].after.access  = VriAccess_DepthStencilAttachmentWrite;
        tb[3].after.layout  = VriLayout_DepthStencilAttachment;
        tb[3].after.stages  = VriPipelineStage_EarlyFragmentTests;
        tb[3].aspect        = VriImageAspect_Depth;
        ...
        colors[0].view    = gPosView;
        colors[0].loadOp  = VriAttachmentLoadOp_Clear;
        colors[0].storeOp = VriAttachmentStoreOp_Store; // position clears to 0
        colors[1].view    = gNormalView;
        colors[1].loadOp  = VriAttachmentLoadOp_Clear;
        colors[1].storeOp = VriAttachmentStoreOp_Store;
        colors[2].view    = gAlbedoView;
        colors[2].loadOp  = VriAttachmentLoadOp_Clear;
        colors[2].storeOp = VriAttachmentStoreOp_Store; // albedo+spec clears to 0
        VriAttachmentDesc depthRT {};
        depthRT.view                          = gDepthView;
        ...
        VriVertexBufferBinding fvb[2] {};
        fvb[0].buffer = floorVB;
        fvb[1].buffer = floorInstB;
        c.CmdSetVertexBuffers(cmd, 0, fvb, 2);
        c.CmdSetIndexBuffer(cmd, floorIB, 0, VriIndexType_UInt16);
        VriDrawIndexedDesc fdi {};
        fdi.indexNum    = floorIndexNum;
        fdi.instanceNum = 1;
        c.CmdDrawIndexed(cmd, &fdi);
        // draw the cube grid (N instances)
        VriVertexBufferBinding cvb[2] {};
        cvb[0].buffer = cubeVB;
        cvb[1].buffer = cubeInstB;
        c.CmdSetVertexBuffers(cmd, 0, cvb, 2);
        c.CmdSetIndexBuffer(cmd, cubeIB, 0, VriIndexType_UInt16);
        VriDrawIndexedDesc cdi {};
        cdi.indexNum    = cubeIndexNum;
        cdi.instanceNum = cubeInstanceNum;
        c.CmdDrawIndexed(cmd, &cdi);
        ...
        ts[1]               = ts[0];
        ts[1].texture       = gNormal;
        ts[2]               = ts[0];
        ts[2].texture       = gAlbedo;
    ...
    c.DestroyPipeline(lightPipeline);
    c.DestroyPipelineLayout(lightLayout);
    c.DestroyPipeline(mrtPipeline);
    c.DestroyPipelineLayout(mrtLayout);
    c.DestroyDescriptor(gPosView);
    c.DestroyDescriptor(gNormalView);
    c.DestroyDescriptor(gAlbedoView);
    c.DestroyDescriptor(gDepthView);
    c.DestroyTexture(gPos);
    c.DestroyTexture(gNormal);
    c.DestroyTexture(gAlbedo);
    c.DestroyTexture(gDepth);
    c.DestroyDescriptor(gbufferSampler);
    c.DestroyDescriptor(repeatSampler);
    c.DestroyDescriptor(texView);
    c.DestroyTexture(texture);
    c.DestroyBuffer(staging);
    c.DestroyDescriptor(lightView);
    c.DestroyBuffer(lightStg);
    c.DestroyBuffer(lightUbo);
    c.DestroyDescriptor(camView);
    c.DestroyBuffer(camStg);
    c.DestroyBuffer(camUbo);
    c.DestroyBuffer(floorInstStg);
    c.DestroyBuffer(floorInstB);
    c.DestroyBuffer(cubeInstStg);
    c.DestroyBuffer(cubeInstB);
    c.DestroyBuffer(floorIStg);
    c.DestroyBuffer(floorIB);
    c.DestroyBuffer(floorVStg);
    c.DestroyBuffer(floorVB);
    c.DestroyBuffer(cubeIStg);
    c.DestroyBuffer(cubeIB);
    c.DestroyBuffer(cubeVStg);
    c.DestroyBuffer(cubeVB);
    app.Shutdown();
#endif
    return 0;
}
