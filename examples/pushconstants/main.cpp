...
int main(int, char**)
{
    ...
    VriPushConstantDesc pcDesc{}; pcDesc.baseRegister = 0; pcDesc.size = sizeof(g_color); pcDesc.shaderStages = VriShaderStage_Vertex | VriShaderStage_Fragment;
    VriPipelineLayoutDesc ld{}; ld.pushConstants = &pcDesc; ld.pushConstantNum = 1;
    ...
    if (app.useDxbc) { sh[0].bytecode = g_pushconstDxbcVS; sh[0].bytecodeSize = sizeof(g_pushconstDxbcVS); sh[1].bytecode = g_pushconstDxbcPS; sh[1].bytecodeSize = sizeof(g_pushconstDxbcPS); }
    else { sh[0].bytecode = sh[1].bytecode = app.useWgsl ? static_cast<const void*>(g_pushconstWgsl) : static_cast<const void*>(g_pushconstSpv);
