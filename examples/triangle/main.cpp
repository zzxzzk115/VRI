...
int main(int, char**)
{
    ...
    VriPipelineLayout* layout = nullptr; c.CreatePipelineLayout(app.dev, &layoutDesc, &layout);

    VriShaderDesc shaders[2]{};
    shaders[0].stage = VriShaderStage_Vertex; shaders[1].stage = VriShaderStage_Fragment;
    shaders[0].entryPointName = "vertexMain"; shaders[1].entryPointName = "fragmentMain";
    if (app.useDxbc)
    {
    ...
    {
        shaders[0].bytecode = app.useWgsl ? static_cast<const void*>(g_triangleWgsl) : static_cast<const void*>(g_triangleSpv);
    ...
    VriColorAttachmentDesc colorAttach{}; colorAttach.format = app.swapFormat; colorAttach.colorWriteMask = VriColorWrite_RGBA;
    ...
    pd.multisample.sampleNum = 1; pd.outputMerger.colors = &colorAttach; pd.outputMerger.colorNum = 1;
