...
#include "tests/shaders/pbr_spv.h"  // g_pbrSpv  (Vulkan + OpenGL)
#include "tests/shaders/pbr_wgsl.h" // g_pbrWgsl (WebGPU)
#include "tests/shaders/pbr_dxbc.h" // g_pbrDxbcVS / PS (D3D12)
...
namespace
{
    ...
    struct Instance { float a[4]; float b[4]; }; // a: pos.xyz + roughness, b: color.rgb + metallic
    ...
    void BuildSphere(std::vector<Vertex>& verts, std::vector<uint16_t>& indices, int rings = 32, int sectors = 32)
    {
        ...
        for (int r = 0; r < rings; ++r)
            for (int s = 0; s < sectors; ++s)
            {
                ...
                indices.push_back(a); indices.push_back(b); indices.push_back(uint16_t(a + 1));
                indices.push_back(uint16_t(a + 1)); indices.push_back(b); indices.push_back(uint16_t(b + 1));
            }
    }

    // Matches the shader Ubo: transposed view-projection + camera + 4 light positions.
    struct Ubo { Mat4 viewProj; float camPos[4]; float lights[4][4]; };
} // namespace
...
int main(int, char**)
{
    ...
    for (int y = 0; y < kGrid; ++y)
        ...
        {
            Instance in{};
            in.a[0] = x * kSpacing - half; in.a[1] = y * kSpacing - half; in.a[2] = 0.0f;
            in.a[3] = std::fmax(0.05f, float(y) / float(kGrid - 1));       // roughness
            in.b[0] = 1.0f; in.b[1] = 0.86f; in.b[2] = 0.57f;             // base color (gold)
    ...
    VriDescriptorRangeDesc range{}; range.baseRegister = 0; range.descriptorNum = 1; range.descriptorType = VriDescriptorType_ConstantBuffer; range.shaderStages = VriShaderStage_Vertex | VriShaderStage_Fragment;
    ...
    if (app.useDxbc) { sh[0].bytecode = g_pbrDxbcVS; sh[0].bytecodeSize = sizeof(g_pbrDxbcVS); sh[1].bytecode = g_pbrDxbcPS; sh[1].bytecodeSize = sizeof(g_pbrDxbcPS); }
    else { sh[0].bytecode = sh[1].bytecode = app.useWgsl ? static_cast<const void*>(g_pbrWgsl) : static_cast<const void*>(g_pbrSpv);
    ...
    attrs[2].format = VriFormat_RGBA32_SFLOAT; attrs[2].offset = 0;  attrs[2].streamIndex = 1; // instA
    attrs[3].format = VriFormat_RGBA32_SFLOAT; attrs[3].offset = 16; attrs[3].streamIndex = 1; // instB
    ...
    streams[1].stride = sizeof(Instance); streams[1].bindingSlot = 1; streams[1].stepRate = VriVertexStepRate_PerInstance;
    ...
    pd.vertexInput.attributes = attrs; pd.vertexInput.attributeNum = 4; pd.vertexInput.streams = streams; pd.vertexInput.streamNum = 2;
    ...
    VriDescriptorRangeUpdateDesc upd{}; upd.descriptors = d0; upd.descriptorNum = 1;
    ...
    // one-time upload: sphere vertices / indices / instances -> read state (fence value 1)
    {
        ...
        VriBufferCopyDesc cp{}; cp.size = vbBytes; c.CmdCopyBuffer(cmd, vbuf, vstg, &cp);
        cp.size = ibBytes; c.CmdCopyBuffer(cmd, ibuf, istg, &cp);
    ...
    app.onUpdate = [ustg](uint64_t frame) {
        ...
        Ubo u{};
        u.viewProj = Transpose(Mul(Perspective(0.9f, float(kWidth) / float(kHeight), 0.1f, 200.0f), LookAt(eye, ctr, up)));
    ...
    app.onRecord = [pipeline, layout, set, vbuf, inst, ibuf, indexNum, instanceNum](VriCommandBuffer* cmd) {
        ...
        VriDrawIndexedDesc di{}; di.indexNum = indexNum; di.instanceNum = instanceNum; app.c.CmdDrawIndexed(cmd, &di);
    };
    ...
    c.DestroyBuffer(inststg); c.DestroyBuffer(inst); c.DestroyBuffer(istg); c.DestroyBuffer(ibuf); c.DestroyBuffer(vstg); c.DestroyBuffer(vbuf);
    app.Shutdown();
#endif
    return 0;
}
