...
namespace
{
    ...
    void BuildGeometry()
    {
        ...
        for (int i = 0; i < kStacks; ++i)
            for (int j = 0; j < kSlices; ++j)
            {
                ...
                g_indices.push_back(a); g_indices.push_back(b); g_indices.push_back(uint16_t(a + 1));
                g_indices.push_back(uint16_t(a + 1)); g_indices.push_back(b); g_indices.push_back(uint16_t(b + 1));
            }
    ...
    struct SkyUbo { Mat4 viewProj; };
    struct ReflectUbo { Mat4 viewProj; Mat4 model; float cameraPos[4]; };
} // namespace

int main(int, char**)
{
    BuildGeometry();
    std::vector<uint8_t> faces; BuildCubemap(faces);
    ...
    VriBufferDesc vbd{}; vbd.size = g_verts.size() * sizeof(Vertex); vbd.usage = VriBufferUsage_VertexBuffer | VriBufferUsage_TransferDst; vbd.memoryLocation = VriMemoryLocation_Device;
    VriBuffer* vbuf = nullptr; c.CreateBuffer(app.dev, &vbd, &vbuf);
    VriBufferDesc ibd{}; ibd.size = g_indices.size() * sizeof(uint16_t); ibd.usage = VriBufferUsage_IndexBuffer | VriBufferUsage_TransferDst; ibd.memoryLocation = VriMemoryLocation_Device;
    VriBuffer* ibuf = nullptr; c.CreateBuffer(app.dev, &ibd, &ibuf);

    // ---- the cubemap: 6 square faces, sampled through a Cube view ----
    VriTextureDesc td{}; td.type = VriTextureType_Cube; td.format = VriFormat_RGBA8_UNORM; td.width = kFace; td.height = kFace; td.depth = 1;
    td.mipNum = 1; td.layerNum = 6; td.sampleNum = 1; td.usage = VriTextureUsage_ShaderResource | VriTextureUsage_TransferDst; td.memoryLocation = VriMemoryLocation_Device;
    VriTexture* cubeTex = nullptr; if (c.CreateTexture(app.dev, &td, &cubeTex) != VriResult_Success) app.Fail("CreateTexture (cubemap) failed");
    VriTextureViewDesc cvd{}; cvd.texture = cubeTex; cvd.viewType = VriTextureViewType_Cube; cvd.format = VriFormat_Unknown; cvd.aspect = VriImageAspect_Color; cvd.layerNum = 6;
    VriDescriptor* cubeView = nullptr; if (c.CreateTextureView(app.dev, &cvd, &cubeView) != VriResult_Success) app.Fail("CreateTextureView (cube) failed");
    ...
    auto makeUbo = [&](uint64_t size, VriBuffer*& ubo, VriBuffer*& stg, VriDescriptor*& view) {
        VriBufferDesc ud{}; ud.size = size; ud.usage = VriBufferUsage_ConstantBuffer | VriBufferUsage_TransferDst; ud.memoryLocation = VriMemoryLocation_Device;
        ...
        VriBufferViewDesc bv{}; bv.buffer = ubo; bv.viewType = VriDescriptorType_ConstantBuffer; bv.offset = 0; bv.size = size;
    ...
    r[0].baseRegister = 0; r[0].descriptorNum = 1; r[0].descriptorType = VriDescriptorType_ConstantBuffer; r[0].shaderStages = VriShaderStage_Vertex | VriShaderStage_Fragment;
    r[1].baseRegister = 1; r[1].descriptorNum = 1; r[1].descriptorType = VriDescriptorType_Texture; r[1].shaderStages = VriShaderStage_Fragment; r[1].viewType = VriTextureViewType_Cube;
    r[2].baseRegister = 2; r[2].descriptorNum = 1; r[2].descriptorType = VriDescriptorType_Sampler; r[2].shaderStages = VriShaderStage_Fragment;
    VriDescriptorSetDesc setDesc{}; setDesc.registerSpace = 0; setDesc.ranges = r; setDesc.rangeNum = 3;
    VriPipelineLayoutDesc pld{}; pld.descriptorSets = &setDesc; pld.descriptorSetNum = 1;
    ...
    auto makePipeline = [&](const void* spv, size_t spvLen, const void* wgsl, size_t wgslLen,
        ...
        if (useDxbc) { sh[0].bytecode = vs; sh[0].bytecodeSize = vsLen; sh[1].bytecode = ps; sh[1].bytecodeSize = psLen; }
        else { sh[0].bytecode = sh[1].bytecode = useWgsl ? wgsl : spv; sh[0].bytecodeSize = sh[1].bytecodeSize = useWgsl ? wgslLen : spvLen; }
        ...
        pd.depthStencil.depthTest = depthOn ? VRI_TRUE : VRI_FALSE; pd.depthStencil.depthWrite = depthOn ? VRI_TRUE : VRI_FALSE; pd.depthStencil.depthCompareOp = VriCompareOp_Less;
        ca.format = app.swapFormat;
        pd.outputMerger.colors = &ca; pd.outputMerger.colorNum = 1; pd.outputMerger.depthStencilFormat = app.depthFormat; // pass has a depth attachment
        VriPipeline* p = nullptr;
        if (c.CreateGraphicsPipeline(app.dev, &pd, &p) != VriResult_Success) app.Fail("CreateGraphicsPipeline failed");
        return p;
    };
    ...
    VriDescriptorSet* skySet = nullptr; c.AllocateDescriptorSets(pool, layout, 0, &skySet, 1);
    VriDescriptorSet* refSet = nullptr; c.AllocateDescriptorSets(pool, layout, 0, &refSet, 1);
    auto writeSet = [&](VriDescriptorSet* set, VriDescriptor* uboView) {
        const VriDescriptor* a[1] = {uboView}; const VriDescriptor* b[1] = {cubeView}; const VriDescriptor* s[1] = {sampler};
        VriDescriptorRangeUpdateDesc u[3]{}; u[0].descriptors = a; u[0].descriptorNum = 1; u[1].descriptors = b; u[1].descriptorNum = 1; u[2].descriptors = s; u[2].descriptorNum = 1;
    ...
    app.BeginUpload();
    app.UploadBuffer(vbuf, g_verts.data(), g_verts.size() * sizeof(Vertex), VriAccess_VertexBufferRead, VriPipelineStage_VertexInput);
    app.UploadBuffer(ibuf, g_indices.data(), g_indices.size() * sizeof(uint16_t), VriAccess_IndexBufferRead, VriPipelineStage_VertexInput);
    ...
    app.onUpdate = [=](uint64_t) {
        ...
        Mat4 viewNoT = view; viewNoT.m[12] = viewNoT.m[13] = viewNoT.m[14] = 0.0f;
        SkyUbo sky{}; sky.viewProj = Transpose(Mul(proj, viewNoT));
        std::memcpy(c.MapBuffer(skyStg, 0, sizeof(SkyUbo)), &sky, sizeof(SkyUbo)); c.UnmapBuffer(skyStg);

        ReflectUbo ref{};
        ref.viewProj = Transpose(Mul(proj, view));
        ref.model = Transpose(Mul(RotateY(t * spinSpeed), Scale(sphereRadius)));
        ref.cameraPos[0] = eye[0]; ref.cameraPos[1] = eye[1]; ref.cameraPos[2] = eye[2]; ref.cameraPos[3] = 1.0f;
        std::memcpy(c.MapBuffer(refStg, 0, sizeof(ReflectUbo)), &ref, sizeof(ReflectUbo)); c.UnmapBuffer(refStg);
    };
    ...
    app.onPreRender = [=](VriCommandBuffer* cmd) {
        VriBuffer* ubos[2] = {skyUbo, refUbo}; VriBuffer* stgs[2] = {skyStg, refStg}; uint64_t sizes[2] = {sizeof(SkyUbo), sizeof(ReflectUbo)};
        for (int i = 0; i < 2; ++i)
        {
            VriBufferCopyDesc cp{}; cp.size = sizes[i]; c.CmdCopyBuffer(cmd, ubos[i], stgs[i], &cp);
            VriBufferBarrierDesc bb{}; bb.buffer = ubos[i]; bb.before.access = VriAccess_CopyDestinationWrite; bb.before.stages = VriPipelineStage_Transfer;
            bb.after.access = VriAccess_ConstantBufferRead; bb.after.stages = VriPipelineStage_VertexShader | VriPipelineStage_FragmentShader;
            VriBarrierGroupDesc g{}; g.buffers = &bb; g.bufferNum = 1; c.CmdBarrier(cmd, &g);
        }
    };

    app.onRecord = [=](VriCommandBuffer* cmd) {
        VriVertexBufferBinding vb{}; vb.buffer = vbuf; vb.offset = 0;
        ...
        VriDrawIndexedDesc sky{}; sky.indexNum = 36; sky.instanceNum = 1; sky.baseIndex = 0; sky.vertexOffset = 0;
        ...
        if (showSphere)
        {
            ...
            VriDrawIndexedDesc s{}; s.indexNum = g_sphereIndexNum; s.instanceNum = 1; s.baseIndex = g_sphereBaseIndex; s.vertexOffset = 0;
    ...
    c.DestroyPipeline(skyPipe); c.DestroyPipeline(refPipe); c.DestroyPipelineLayout(layout);
    c.DestroyDescriptor(sampler); c.DestroyDescriptor(cubeView); c.DestroyTexture(cubeTex);
    c.DestroyDescriptor(skyUboView); c.DestroyDescriptor(refUboView);
    c.DestroyBuffer(skyStg); c.DestroyBuffer(skyUbo); c.DestroyBuffer(refStg); c.DestroyBuffer(refUbo);
