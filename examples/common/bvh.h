...
namespace vriex
{
    ...
    namespace detail
    {
        ...
        {
            for (int k = 0; k < 3; ++k) { bmin[k] = FLT_MAX; bmax[k] = -FLT_MAX; }
        }

        inline void GrowBounds(float* bmin, float* bmax, const float* p)
        {
            for (int k = 0; k < 3; ++k) { bmin[k] = std::min(bmin[k], p[k]); bmax[k] = std::max(bmax[k], p[k]); }
        }
        ...
        inline uint32_t BuildNode(std::vector<BuildTri>& tris, uint32_t first, uint32_t count, Bvh& out)
        {
            ...
            ResetBounds(bmin, bmax); ResetBounds(cmin, cmax);
            ...
            for (int k = 0; k < 3; ++k) { node.aabbMin[k] = bmin[k]; node.aabbMax[k] = bmax[k]; }
    ...
    inline Bvh BuildBvh(const GltfModel& model)
    {
        ...
        for (uint32_t geom = 0; geom < static_cast<uint32_t>(model.primitives.size()); ++geom)
        {
            ...
            for (uint32_t i = 0; i + 2 < prim.indexCount; i += 3)
            {
                ...
                for (int k = 0; k < 3; ++k) tri.centroid[k] = (tri.bmin[k] + tri.bmax[k]) * 0.5f;
                tri.indexBase = base;
