    ...
    struct GltfModel
    {
        ...
        static void XformPoint(const Mat4& a, const float* p, float* out)
        {
            for (int row = 0; row < 3; ++row)
                out[row] =
                    a.m[0 * 4 + row] * p[0] + a.m[1 * 4 + row] * p[1] + a.m[2 * 4 + row] * p[2] + a.m[3 * 4 + row];
        }
        ...
        static Mat4 NodeLocal(const tinygltf::Node& n)
        {
            if (n.matrix.size() == 16)
            {
                Mat4 r {};
                for (int i = 0; i < 16; ++i)
                    r.m[i] = static_cast<float>(n.matrix[i]);
                return r;
            }
            Mat4 t = Identity(), r = Identity(), s = Identity();
            if (n.translation.size() == 3)
            {
                t.m[12] = (float)n.translation[0];
                t.m[13] = (float)n.translation[1];
                t.m[14] = (float)n.translation[2];
            }
            if (n.rotation.size() == 4)
            {
                const float x = (float)n.rotation[0], y = (float)n.rotation[1], z = (float)n.rotation[2],
                            w = (float)n.rotation[3];
                r.m[0]        = 1 - 2 * (y * y + z * z);
                r.m[1]        = 2 * (x * y + z * w);
                r.m[2]        = 2 * (x * z - y * w);
                r.m[4]        = 2 * (x * y - z * w);
            ...
            if (n.scale.size() == 3)
            {
                s.m[0]  = (float)n.scale[0];
                s.m[5]  = (float)n.scale[1];
                s.m[10] = (float)n.scale[2];
            }
        ...
        static float
        ReadComponent(const tinygltf::Model& mdl, const tinygltf::Accessor& acc, size_t i, int comp, int numComp)
        {
            ...
            const uint8_t* base =
                buf.data.data() + bv.byteOffset + acc.byteOffset + i * stride + (size_t)comp * compBytes;
            switch (acc.componentType)
            {
                ...
                case TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT: {
                    uint16_t v;
                    std::memcpy(&v, base, 2);
                    return acc.normalized ? v / 65535.0f : (float)v;
                }
                case TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE: {
                    uint8_t v = *base;
                    return acc.normalized ? v / 255.0f : (float)v;
                }
                default:
                    return 0.0f;
            }
        ...
        bool Load(ExampleApp& app, const char* path, bool keepCpuGeometry = false, bool buildTextureArray = false)
        {
            ...
            if (!warn.empty())
                std::printf("[gltf] warning: %s\n", warn.c_str());
            if (!ok)
            {
                std::printf("[gltf] failed to load '%s': %s\n", path, err.c_str());
                return false;
            }
            ...
            while (!stack.empty())
            {
                ...
                {
                    if (prim.indices < 0 || prim.attributes.count("POSITION") == 0)
                        continue;
                    ...
                    const tinygltf::Accessor* uvAcc =
                        hasUv ? &mdl.accessors[prim.attributes.at("TEXCOORD_0")] : nullptr;

                    for (size_t i = 0; i < posAcc.count; ++i)
                    {
                        GltfVertex v {};
                        float      lp[3] = {ReadComponent(mdl, posAcc, i, 0, 3),
                                            ReadComponent(mdl, posAcc, i, 1, 3),
                                            ReadComponent(mdl, posAcc, i, 2, 3)};
                        XformPoint(world, lp, v.pos);
                        if (nrmAcc)
                        {
                            float ln[3] = {ReadComponent(mdl, *nrmAcc, i, 0, 3),
                                           ReadComponent(mdl, *nrmAcc, i, 1, 3),
                                           ReadComponent(mdl, *nrmAcc, i, 2, 3)};
                            float wn[3];
                            XformDir(world, ln, wn);
                        ...
                        if (uvAcc)
                        {
                            v.uv[0] = ReadComponent(mdl, *uvAcc, i, 0, 2);
                            v.uv[1] = ReadComponent(mdl, *uvAcc, i, 1, 2);
                        }
                        for (int k = 0; k < 3; ++k)
                        {
                            if (!haveBounds)
                            {
                                boundsMin[k] = boundsMax[k] = v.pos[k];
                            }
                            else
                            {
                                boundsMin[k] = std::min(boundsMin[k], v.pos[k]);
                                boundsMax[k] = std::max(boundsMax[k], v.pos[k]);
                            }
                        }
                        haveBounds = true;
                        verts.push_back(v);
                    ...
                    for (size_t i = 0; i < idxAcc.count; ++i)
                        indices.push_back(vertexStart + ReadIndex(mdl, idxAcc, i));
                    if (prim.material >= 0)
                    {
            ...
            {
                std::printf("[gltf] '%s' has no triangles\n", path);
                return false;
            }
            ...
            app.BeginUpload();
            std::vector<std::vector<uint8_t>> pixelHold;
            pixelHold.reserve(mdl.textures.size());
            std::vector<std::pair<uint32_t, uint32_t>> texDims;
            texDims.reserve(mdl.textures.size());
            ...
            if (buildTextureArray && !pixelHold.empty())
            {
                ...
                VriTextureDesc ad {};
                ad.type           = VriTextureType_2D;
                ad.format         = VriFormat_RGBA8_UNORM;
                ad.width          = kDim;
                ad.height         = kDim;
                ad.depth          = 1;
                ad.mipNum         = 1;
                ad.layerNum       = textureArrayLayers;
                ad.sampleNum      = 1;
                ad.usage          = VriTextureUsage_ShaderResource | VriTextureUsage_TransferDst;
                ad.memoryLocation = VriMemoryLocation_Device;
                c.CreateTexture(app.dev, &ad, &textureArray);
                VriTextureViewDesc avd {};
                avd.texture  = textureArray;
                avd.viewType = VriTextureViewType_2DArray;
                avd.format   = VriFormat_Unknown;
                avd.aspect   = VriImageAspect_Color;
                avd.layerNum = textureArrayLayers;
        ...
        {
            VriCoreInterface& c = app.c;
            for (VriDescriptor* v : textureViews)
                c.DestroyDescriptor(v);
            for (VriTexture* t : textures)
                c.DestroyTexture(t);
            if (textureArrayView)
                c.DestroyDescriptor(textureArrayView);
            if (textureArray)
                c.DestroyTexture(textureArray);
            if (vertexBuffer)
                c.DestroyBuffer(vertexBuffer);
            if (indexBuffer)
                c.DestroyBuffer(indexBuffer);
            textureArrayView   = nullptr;
            textureArray       = nullptr;
            textureArrayLayers = 0;
            textureViews.clear();
            textures.clear();
            primitives.clear();
            cpuVerts.clear();
