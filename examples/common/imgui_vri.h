...
#include "shaders/lib/imgui_dxbc.h" // g_imguiDxbcVS / PS (D3D12)
...
namespace vriex
{
    struct ImguiVri
    {
        ...
        void Init(VriCoreInterface& core,
                  VriDevice*        device,
                  VriQueue*         queue,
                  VriFormat         swapFormat,
                  ...
                  bool              useDxbc)
        {
            ...
            td.width          = uint32_t(fw);
            td.height         = uint32_t(fh);
            td.depth          = 1;
            td.mipNum         = 1;
            td.layerNum       = 1;
            td.sampleNum      = 1;
            ...
            c->CreateBuffer(dev, &fsd, &fstg);
            std::memcpy(c->MapBuffer(fstg, 0, fontBytes), px, size_t(fontBytes));
            c->UnmapBuffer(fstg);
            {
                ...
                up.texture.width    = uint32_t(fw);
                up.texture.height   = uint32_t(fh);
            ...
            if (useDxbc)
            {
                sh[0].bytecode     = g_imguiDxbcVS;
                sh[0].bytecodeSize = sizeof(g_imguiDxbcVS);
                sh[1].bytecode     = g_imguiDxbcPS;
                sh[1].bytecodeSize = sizeof(g_imguiDxbcPS);
            }
            else
            {
                sh[0].bytecode = sh[1].bytecode =
                    useWgsl ? static_cast<const void*>(g_imguiWgsl) : static_cast<const void*>(g_imguiSpv);
            ...
            const VriDescriptor*         td0[1] = {fontView};
            const VriDescriptor*         sd0[1] = {sampler};
        ...
        void     WriteFrame(ImDrawData* dd)
        {
            ...
            for (int n = 0; n < dd->CmdListsCount; ++n)
            {
                ...
                std::memcpy(vmap, cl->VtxBuffer.Data, size_t(cl->VtxBuffer.Size) * sizeof(ImDrawVert));
                vmap += cl->VtxBuffer.Size;
                std::memcpy(imap, cl->IdxBuffer.Data, size_t(cl->IdxBuffer.Size) * sizeof(ImDrawIdx));
                imap += cl->IdxBuffer.Size;
            }
            c->UnmapBuffer(vstg);
            c->UnmapBuffer(istg);
        }
        ...
        {
            if (!dd || dd->TotalVtxCount == 0 || dd->DisplaySize.x <= 0 || dd->DisplaySize.y <= 0)
                return;
            Ortho o {};
            o.scale[0]     = 2.0f / dd->DisplaySize.x;
            o.scale[1]     = -2.0f / dd->DisplaySize.y; // -Y: ImGui origin is top-left
            o.translate[0] = -1.0f - dd->DisplayPos.x * o.scale[0];
            o.translate[1] = 1.0f - dd->DisplayPos.y * o.scale[1];
            ...
            for (int n = 0; n < dd->CmdListsCount; ++n)
            {
                ...
                for (int i = 0; i < cl->CmdBuffer.Size; ++i)
                {
                    ...
                    di.indexNum     = dc.ElemCount;
                    di.instanceNum  = 1;
                    di.baseIndex    = dc.IdxOffset + idxOff;
                    di.vertexOffset = int32_t(dc.VtxOffset + vtxOff);
        ...
        void Shutdown()
        {
            ...
            if (fontView)
                c->DestroyDescriptor(fontView);
            if (font)
                c->DestroyTexture(font);
        }
    };
} // namespace vriex
