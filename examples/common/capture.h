...
inline bool WriteBmpBGRA(const char* path, const uint8_t* bgra, uint32_t w, uint32_t h)
{
    ...
    // BITMAPFILEHEADER
    std::fputc('B', f); std::fputc('M', f); u32(fileSize); u16(0); u16(0); u32(54);
    // BITMAPINFOHEADER (negative height = top-down)
    u32(40); i32(static_cast<int32_t>(w)); i32(-static_cast<int32_t>(h)); u16(1); u16(24);
    u32(0); u32(dataSize); i32(2835); i32(2835); u32(0); u32(0);

    uint8_t pad[3] = {0, 0, 0};
    for (uint32_t y = 0; y < h; ++y)
    {
        const uint8_t* row = bgra + static_cast<size_t>(y) * w * 4;
        for (uint32_t x = 0; x < w; ++x) { std::fputc(row[x*4+0], f); std::fputc(row[x*4+1], f); std::fputc(row[x*4+2], f); }
