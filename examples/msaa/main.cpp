...
namespace
{
    ...
    std::vector<Vertex> BuildPinwheel(int blades)
    {
        ...
        for (int i = 0; i < blades; ++i)
        {
            ...
            const float cr = 0.5f + 0.5f * std::sin(h), cg = 0.5f + 0.5f * std::sin(h + 2.094f),
                        cb = 0.5f + 0.5f * std::sin(h + 4.188f);
...
int main(int, char**)
{
    ...
    auto makeTex =
        [&](uint32_t samples, VriTextureUsageFlags usage, VriDescriptor** outView, VriClearValue clear = {}) {
            VriTextureDesc td {};
            ...
            if (c.CreateTexture(app.dev, &td, &t) != VriResult_Success)
                app.Fail("CreateTexture (MSAA) failed");
            VriTextureViewDesc vd {};
            vd.texture  = t;
            vd.viewType = VriTextureViewType_2D;
            vd.format   = VriFormat_Unknown;
            vd.aspect   = VriImageAspect_Color;
            if (c.CreateTextureView(app.dev, &vd, outView) != VriResult_Success)
                app.Fail("view failed");
            return t;
        };
    VriClearValue msaaClear {};
    msaaClear.color.f32[0]      = 0.06f;
    msaaClear.color.f32[1]      = 0.07f;
    msaaClear.color.f32[2]      = 0.09f;
    msaaClear.color.f32[3]      = 1.0f;
    VriDescriptor* msaaView     = nullptr;
    VriTexture*    msaaColor    = makeTex(kSamples, VriTextureUsage_ColorAttachment, &msaaView, msaaClear);
    VriDescriptor* resolvedView = nullptr;
    VriTexture* resolved = makeTex(1, VriTextureUsage_ColorAttachment | VriTextureUsage_ShaderResource, &resolvedView);
    ...
    VriShaderDesc ssh[2] {};
    ssh[0].stage          = VriShaderStage_Vertex;
    ssh[0].entryPointName = "vertexMain";
    ssh[1].stage          = VriShaderStage_Fragment;
    ssh[1].entryPointName = "fragmentMain";
    if (useDxbc)
    {
        ssh[0].bytecode     = g_msaaDxbcVS;
        ssh[0].bytecodeSize = sizeof(g_msaaDxbcVS);
        ssh[1].bytecode     = g_msaaDxbcPS;
        ssh[1].bytecodeSize = sizeof(g_msaaDxbcPS);
    }
    else
    {
        ssh[0].bytecode = ssh[1].bytecode =
            useWgsl ? static_cast<const void*>(g_msaaWgsl) : static_cast<const void*>(g_msaaSpv);
    ...
    if (useDxbc)
    {
        csh[0].bytecode     = g_showTexDxbcVS;
        csh[0].bytecodeSize = sizeof(g_showTexDxbcVS);
        csh[1].bytecode     = g_showTexDxbcPS;
        csh[1].bytecodeSize = sizeof(g_showTexDxbcPS);
    }
    else
    {
        csh[0].bytecode = csh[1].bytecode =
            useWgsl ? static_cast<const void*>(g_showTexWgsl) : static_cast<const void*>(g_showTexSpv);
    ...
    {
        const VriDescriptor*         t[1] = {resolvedView};
    ...
    static bool  msaaOn = true;                                            // ImGui-controlled
    ...
    app.onPreRender = [=](VriCommandBuffer* cmd) {
        ...
        pc.rot[2] = float(kHeight) / float(kWidth);

        // The scene target: 4x MSAA color resolving into `resolved`, or `resolved` at 1x directly.
        std::vector<VriTextureBarrierDesc> pre;
        VriTextureBarrierDesc              rb {};
        rb.texture       = resolved;
        rb.aspect        = VriImageAspect_Color;
        rb.before.layout = g_resolvedInit ? VriLayout_ShaderResource : VriLayout_Undefined;
        rb.before.stages = g_resolvedInit ? VriPipelineStage_FragmentShader : VriPipelineStage_None;
        rb.after.access  = VriAccess_ColorAttachmentWrite;
        rb.after.layout  = VriLayout_ColorAttachment;
        rb.after.stages  = VriPipelineStage_ColorAttachmentOutput;
        pre.push_back(rb);
        if (msaaOn)
        {
            VriTextureBarrierDesc mb {};
            mb.texture       = msaaColor;
            mb.aspect        = VriImageAspect_Color;
            mb.before.layout = g_msaaInit ? VriLayout_ColorAttachment : VriLayout_Undefined;
            mb.before.stages = g_msaaInit ? VriPipelineStage_ColorAttachmentOutput : VriPipelineStage_None;
            mb.after.access  = VriAccess_ColorAttachmentWrite;
            mb.after.layout  = VriLayout_ColorAttachment;
            mb.after.stages  = VriPipelineStage_ColorAttachmentOutput;
            pre.push_back(mb);
        }
        VriBarrierGroupDesc gw {};
        gw.textures   = pre.data();
        gw.textureNum = static_cast<uint32_t>(pre.size());
        c.CmdBarrier(cmd, &gw);
        g_resolvedInit = true;
        g_msaaInit     = true;
        ...
        if (msaaOn)
            col.resolveView = resolvedView; // MSAA -> single-sample resolve at EndRendering
    ...
    c.DestroyPipeline(scene1x);
    c.DestroyPipeline(scene4x);
    c.DestroyPipelineLayout(sceneLayout);
    c.DestroyDescriptor(sampler);
    c.DestroyDescriptor(resolvedView);
    c.DestroyDescriptor(msaaView);
    c.DestroyTexture(resolved);
    c.DestroyTexture(msaaColor);
