...
inline bool LoadKtxRgba(const char* path, KtxImage& out)
{
    ...
    if (glFormat != 0x1908 || glInternalFormat != 0x8058) return false; // RGBA8 only
    if (pixelWidth == 0 || pixelHeight == 0) return false;

    if (std::fseek(f, static_cast<long>(bytesOfKeyValueData), SEEK_CUR) != 0) return false;

    uint32_t imageSize = 0;
    if (std::fread(&imageSize, 4, 1, f) != 1) return false;
    ...
    if (std::fread(out.rgba.data(), 1, expected, f) != expected) return false;
    return true;
}
