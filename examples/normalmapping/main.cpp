...
namespace
{
    ...
    struct Vertex { float px, py, pz; float nx, ny, nz; float tx, ty, tz; float u, v; };
    ...
    struct Ubo { Mat4 mvp; float lightPos[4]; float camPos[4]; };

    // device-local texture from a loaded RGBA8 image (staging upload + view).
    struct TexUpload { VriTexture* tex; VriDescriptor* view; VriBuffer* staging; uint32_t w, h; };
    TexUpload MakeTexture(vriex::ExampleApp& app, const KtxImage& img)
    {
        VriCoreInterface& c = app.c;
        VriTextureDesc td{}; td.type = VriTextureType_2D; td.format = VriFormat_RGBA8_UNORM; td.width = img.width; td.height = img.height; td.depth = 1;
        ...
        VriBufferDesc sd{}; sd.size = img.rgba.size(); sd.usage = VriBufferUsage_TransferSrc; sd.memoryLocation = VriMemoryLocation_HostUpload;
        VriBuffer* staging = nullptr; c.CreateBuffer(app.dev, &sd, &staging);
        std::memcpy(c.MapBuffer(staging, 0, img.rgba.size()), img.rgba.data(), img.rgba.size()); c.UnmapBuffer(staging);
...
int main(int, char**)
{
    ...
    auto loadTex = [&](const char* file) {
        ...
        if (!LoadKtxRgba(p.c_str(), img) && !LoadKtxRgba(file, img)) app.Fail("failed to load texture");
#else
    ...
    VriBufferDesc ubd{}; ubd.size = sizeof(Ubo); ubd.usage = VriBufferUsage_ConstantBuffer | VriBufferUsage_TransferDst; ubd.memoryLocation = VriMemoryLocation_Device;
    VriBuffer* ubo = nullptr; c.CreateBuffer(app.dev, &ubd, &ubo);
    VriBufferDesc usd{}; usd.size = sizeof(Ubo); usd.usage = VriBufferUsage_TransferSrc; usd.memoryLocation = VriMemoryLocation_HostUpload;
    VriBuffer* ustg = nullptr; c.CreateBuffer(app.dev, &usd, &ustg);
    VriBufferViewDesc ubv{}; ubv.buffer = ubo; ubv.viewType = VriDescriptorType_ConstantBuffer; ubv.offset = 0; ubv.size = sizeof(Ubo);
    ...
    ranges[0].baseRegister = 0; ranges[0].descriptorNum = 1; ranges[0].descriptorType = VriDescriptorType_ConstantBuffer; ranges[0].shaderStages = VriShaderStage_Vertex | VriShaderStage_Fragment;
    ...
    ranges[2].baseRegister = 2; ranges[2].descriptorNum = 1; ranges[2].descriptorType = VriDescriptorType_Texture;        ranges[2].shaderStages = VriShaderStage_Fragment;
    ranges[3].baseRegister = 3; ranges[3].descriptorNum = 1; ranges[3].descriptorType = VriDescriptorType_Sampler;        ranges[3].shaderStages = VriShaderStage_Fragment;
    VriDescriptorSetDesc setDesc{}; setDesc.registerSpace = 0; setDesc.ranges = ranges; setDesc.rangeNum = 4;
    ...
    if (app.useDxbc) { sh[0].bytecode = g_normalmapDxbcVS; sh[0].bytecodeSize = sizeof(g_normalmapDxbcVS); sh[1].bytecode = g_normalmapDxbcPS; sh[1].bytecodeSize = sizeof(g_normalmapDxbcPS); }
    else { sh[0].bytecode = sh[1].bytecode = app.useWgsl ? static_cast<const void*>(g_normalmapWgsl) : static_cast<const void*>(g_normalmapSpv);
    ...
    attrs[2].format = VriFormat_RGB32_SFLOAT; attrs[2].offset = 24; attrs[2].streamIndex = 0; // tangent
    attrs[3].format = VriFormat_RG32_SFLOAT;  attrs[3].offset = 36; attrs[3].streamIndex = 0; // uv
    ...
    pd.vertexInput.attributes = attrs; pd.vertexInput.attributeNum = 4; pd.vertexInput.streams = &stream; pd.vertexInput.streamNum = 1;
    ...
    VriDescriptorPoolDesc pdsc{}; pdsc.descriptorSetMaxNum = 1; pdsc.constantBufferMaxNum = 1; pdsc.textureMaxNum = 2; pdsc.samplerMaxNum = 1;
    ...
    const VriDescriptor* d0[1] = {uboView}; const VriDescriptor* d1[1] = {color.view}; const VriDescriptor* d2[1] = {normal.view}; const VriDescriptor* d3[1] = {sampler};
    VriDescriptorRangeUpdateDesc upd[4]{};
    upd[0].descriptors = d0; upd[0].descriptorNum = 1; upd[1].descriptors = d1; upd[1].descriptorNum = 1;
    upd[2].descriptors = d2; upd[2].descriptorNum = 1; upd[3].descriptors = d3; upd[3].descriptorNum = 1;
    ...
    // one-time upload: quad buffers + both textures -> read state (fence value 1)
    {
        ...
        VriBufferCopyDesc cp{}; cp.size = sizeof(kQuad); c.CmdCopyBuffer(cmd, vbuf, vstg, &cp);
        ...
        {
            VriTextureBarrierDesc tb{}; tb.texture = t->tex; tb.before.layout = VriLayout_Undefined; tb.before.stages = VriPipelineStage_None;
            ...
            VriBufferTextureCopyDesc up{}; up.texture.aspect = VriImageAspect_Color; up.texture.layerNum = 1; up.texture.width = t->w; up.texture.height = t->h;
            c.CmdUploadBufferToTexture(cmd, t->tex, t->staging, &up);
            VriTextureBarrierDesc tb2{}; tb2.texture = t->tex; tb2.before.access = VriAccess_CopyDestinationWrite; tb2.before.layout = VriLayout_CopyDestination; tb2.before.stages = VriPipelineStage_Transfer;
    ...
    app.onRecord = [pipeline, layout, set, vbuf, ibuf](VriCommandBuffer* cmd) {
        ...
        VriDrawIndexedDesc di{}; di.indexNum = 6; di.instanceNum = 1; app.c.CmdDrawIndexed(cmd, &di);
    };
    ...
    c.DestroyDescriptor(color.view); c.DestroyTexture(color.tex); c.DestroyBuffer(color.staging);
    c.DestroyDescriptor(normal.view); c.DestroyTexture(normal.tex); c.DestroyBuffer(normal.staging);
