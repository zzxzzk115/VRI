...
namespace
{
    ...
    struct SceneUbo { Mat4 viewProj; float lightDir[4]; float outlineColor[4]; };
} // namespace
...
int main(int, char**)
{
    ...
    VriDescriptorRangeDesc dr{}; dr.baseRegister = 0; dr.descriptorNum = 1; dr.descriptorType = VriDescriptorType_ConstantBuffer; dr.shaderStages = VriShaderStage_Vertex | VriShaderStage_Fragment;
    VriDescriptorSetDesc dss{}; dss.registerSpace = 0; dss.ranges = &dr; dss.rangeNum = 1;
    VriPipelineLayoutDesc pld{}; pld.descriptorSets = &dss; pld.descriptorSetNum = 1; pld.pushConstants = &pc; pld.pushConstantNum = 1;
    ...
    auto stencilFace = [](VriCompareOp op, VriStencilOp passOp, uint32_t writeMask) {
        VriStencilOpDesc f{}; f.compareOp = op; f.passOp = passOp; f.failOp = VriStencilOp_Keep; f.depthFailOp = VriStencilOp_Keep;
        f.compareMask = 0xFFu; f.writeMask = writeMask; f.reference = 1u; return f;
    };
    ...
    auto makePipeline = [&](const void* spv, size_t spvLen, const void* wgsl, size_t wgslLen,
        ...
        pd.depthStencil.depthTest = depthTest ? VRI_TRUE : VRI_FALSE; pd.depthStencil.depthWrite = depthWrite ? VRI_TRUE : VRI_FALSE; pd.depthStencil.depthCompareOp = depthCmp;
    ...
    { const VriDescriptor* a[1] = {uboView}; VriDescriptorRangeUpdateDesc u[1]{}; u[0].descriptors = a; u[0].descriptorNum = 1; c.UpdateDescriptorRanges(set, 0, 1, u); }
    ...
    app.onUpdate = [=](uint64_t) {
        ...
        s.lightDir[0] = ld[0]/len; s.lightDir[1] = ld[1]/len; s.lightDir[2] = ld[2]/len; s.lightDir[3] = 0;
        s.outlineColor[0] = outlineCol[0]; s.outlineColor[1] = outlineCol[1]; s.outlineColor[2] = outlineCol[2]; s.outlineColor[3] = 1.0f;
        std::memcpy(c.MapBuffer(ustg, 0, sizeof(SceneUbo)), &s, sizeof(SceneUbo)); c.UnmapBuffer(ustg);
    };
    ...
    app.onPreRender = [=](VriCommandBuffer* cmd) {
        ...
        ub.after.access = VriAccess_ConstantBufferRead; ub.after.stages = VriPipelineStage_VertexShader | VriPipelineStage_FragmentShader;
    ...
    app.onRecord = [=](VriCommandBuffer* cmd) {
        ...
        for (int i = 0; i < kCubeNum; ++i)
        {
            ...
            VriDrawIndexedDesc di{}; di.indexNum = 36; di.instanceNum = 1; di.baseIndex = 0; di.vertexOffset = 0; c.CmdDrawIndexed(cmd, &di);
        }
    ...
    c.DestroyPipeline(fillPipe); c.DestroyPipeline(outlinePipe); c.DestroyPipelineLayout(layout);
