...
namespace
{
    ...
    struct Vertex { float px, py, pz; float nx, ny, nz; };

    // A ground plane (4 verts) followed by a unit cube (24 verts, per-face normals), local space.
    const Vertex kVerts[] = {
        {-4, 0, -4, 0, 1, 0}, {4, 0, -4, 0, 1, 0}, {4, 0, 4, 0, 1, 0}, {-4, 0, 4, 0, 1, 0}, // plane (+Y)
    ...
    void BuildIndices()
    {
        kIndices[0]=0; kIndices[1]=1; kIndices[2]=2; kIndices[3]=0; kIndices[4]=2; kIndices[5]=3;
    ...
    struct Obj { Mat4 model; uint32_t indexNum, baseIndex; int32_t vertexOffset; };
    std::vector<Obj> g_objs;

    struct SceneUbo { Mat4 camViewProj; Mat4 lightViewProj; float lightDir[4]; float params[4]; };
...
int main(int, char**)
{
    ...
    VriBufferDesc vbd{}; vbd.size = sizeof(kVerts); vbd.usage = VriBufferUsage_VertexBuffer | VriBufferUsage_TransferDst; vbd.memoryLocation = VriMemoryLocation_Device;
    ...
    VriBuffer* ibuf = nullptr; c.CreateBuffer(app.dev, &ibd, &ibuf);

    // scene constant buffer (refreshed each frame from host staging)
    VriBufferDesc ubd{}; ubd.size = sizeof(SceneUbo); ubd.usage = VriBufferUsage_ConstantBuffer | VriBufferUsage_TransferDst; ubd.memoryLocation = VriMemoryLocation_Device;
    VriBuffer* ubo = nullptr; c.CreateBuffer(app.dev, &ubd, &ubo);
    VriBufferDesc usd{}; usd.size = sizeof(SceneUbo); usd.usage = VriBufferUsage_TransferSrc; usd.memoryLocation = VriMemoryLocation_HostUpload;
    VriBuffer* ustg = nullptr; c.CreateBuffer(app.dev, &usd, &ustg);
    VriBufferViewDesc ubv{}; ubv.buffer = ubo; ubv.viewType = VriDescriptorType_ConstantBuffer; ubv.offset = 0; ubv.size = sizeof(SceneUbo);
    ...
    VriSamplerDesc cmp{}; cmp.magFilter = VriFilter_Linear; cmp.minFilter = VriFilter_Linear; cmp.mipmapMode = VriMipmapMode_Nearest;
    cmp.addressModeU = VriAddressMode_ClampToEdge; cmp.addressModeV = VriAddressMode_ClampToEdge; cmp.addressModeW = VriAddressMode_ClampToEdge;
    cmp.compareEnable = VRI_TRUE; cmp.compareOp = VriCompareOp_LessOrEqual; cmp.maxLod = 1.0f;
    VriDescriptor* shadowSampler = nullptr; c.CreateSampler(app.dev, &cmp, &shadowSampler);

    VriVertexAttributeDesc attrs[2]{};
    attrs[0].format = VriFormat_RGB32_SFLOAT; attrs[0].offset = 0;  attrs[0].streamIndex = 0; // position
    attrs[1].format = VriFormat_RGB32_SFLOAT; attrs[1].offset = 12; attrs[1].streamIndex = 0; // normal
    ...
    VriPushConstantDesc pc{}; pc.baseRegister = 1; pc.size = sizeof(Mat4); pc.shaderStages = VriShaderStage_Vertex;

    // ---- shadow pass layout/pipeline: CB@0 (vertex) + push-constant model; depth-only ----
    VriDescriptorRangeDesc dr{}; dr.baseRegister = 0; dr.descriptorNum = 1; dr.descriptorType = VriDescriptorType_ConstantBuffer; dr.shaderStages = VriShaderStage_Vertex;
    VriDescriptorSetDesc dss{}; dss.registerSpace = 0; dss.ranges = &dr; dss.rangeNum = 1;
    VriPipelineLayoutDesc dld{}; dld.descriptorSets = &dss; dld.descriptorSetNum = 1; dld.pushConstants = &pc; dld.pushConstantNum = 1;
    VriPipelineLayout* shadowLayout = nullptr; c.CreatePipelineLayout(app.dev, &dld, &shadowLayout);
    ...
    if (useDxbc) { dsh[0].bytecode = g_shadowDepthDxbcVS; dsh[0].bytecodeSize = sizeof(g_shadowDepthDxbcVS); dsh[1].bytecode = g_shadowDepthDxbcPS; dsh[1].bytecodeSize = sizeof(g_shadowDepthDxbcPS); }
    else { dsh[0].bytecode = dsh[1].bytecode = useWgsl ? static_cast<const void*>(g_shadowDepthWgsl) : static_cast<const void*>(g_shadowDepthSpv);
    ...
    dpd.depthStencil.depthTest = VRI_TRUE; dpd.depthStencil.depthWrite = VRI_TRUE; dpd.depthStencil.depthCompareOp = VriCompareOp_Less;
    dpd.outputMerger.colorNum = 0; dpd.outputMerger.depthStencilFormat = kShadowFormat; // depth-only
    VriPipeline* shadowPipeline = nullptr;
    if (c.CreateGraphicsPipeline(app.dev, &dpd, &shadowPipeline) != VriResult_Success) app.Fail("shadow CreateGraphicsPipeline failed");

    // ---- main pass layout/pipeline: CB@0 + shadow Texture@1 + comparison Sampler@2 + push model ----
    VriDescriptorRangeDesc mr[3]{};
    mr[0].baseRegister = 0; mr[0].descriptorNum = 1; mr[0].descriptorType = VriDescriptorType_ConstantBuffer; mr[0].shaderStages = VriShaderStage_Vertex | VriShaderStage_Fragment;
    mr[1].baseRegister = 1; mr[1].descriptorNum = 1; mr[1].descriptorType = VriDescriptorType_Texture; mr[1].shaderStages = VriShaderStage_Fragment; mr[1].flags = VriDescriptorRange_Comparison; // depth sampleType
    mr[2].baseRegister = 2; mr[2].descriptorNum = 1; mr[2].descriptorType = VriDescriptorType_Sampler; mr[2].shaderStages = VriShaderStage_Fragment; mr[2].flags = VriDescriptorRange_Comparison; // comparison sampler
    VriDescriptorSetDesc msd{}; msd.registerSpace = 0; msd.ranges = mr; msd.rangeNum = 3;
    VriPipelineLayoutDesc mld{}; mld.descriptorSets = &msd; mld.descriptorSetNum = 1; mld.pushConstants = &pc; mld.pushConstantNum = 1;
    VriPipelineLayout* mainLayout = nullptr; c.CreatePipelineLayout(app.dev, &mld, &mainLayout);
    ...
    if (useDxbc) { msh[0].bytecode = g_shadowSceneDxbcVS; msh[0].bytecodeSize = sizeof(g_shadowSceneDxbcVS); msh[1].bytecode = g_shadowSceneDxbcPS; msh[1].bytecodeSize = sizeof(g_shadowSceneDxbcPS); }
    else { msh[0].bytecode = msh[1].bytecode = useWgsl ? static_cast<const void*>(g_shadowSceneWgsl) : static_cast<const void*>(g_shadowSceneSpv);
    ...
    mpd.pipelineLayout = mainLayout; mpd.shaders = msh; mpd.shaderNum = 2;
    mpd.vertexInput.attributes = attrs; mpd.vertexInput.attributeNum = 2; mpd.vertexInput.streams = &stream; mpd.vertexInput.streamNum = 1;
    ...
    mpd.outputMerger.colors = &ca; mpd.outputMerger.colorNum = 1; mpd.outputMerger.depthStencilFormat = app.depthFormat;
    VriPipeline* mainPipeline = nullptr;
    if (c.CreateGraphicsPipeline(app.dev, &mpd, &mainPipeline) != VriResult_Success) app.Fail("main CreateGraphicsPipeline failed");

    // descriptor sets (shadow set: UBO only; main set: UBO + shadow map + comparison sampler)
    VriDescriptorPoolDesc pdsc{}; pdsc.descriptorSetMaxNum = 2; pdsc.constantBufferMaxNum = 2; pdsc.textureMaxNum = 1; pdsc.samplerMaxNum = 1;
    VriDescriptorPool* pool = nullptr; c.CreateDescriptorPool(app.dev, &pdsc, &pool);
    VriDescriptorSet* shadowSet = nullptr; c.AllocateDescriptorSets(pool, shadowLayout, 0, &shadowSet, 1);
    VriDescriptorSet* mainSet = nullptr; c.AllocateDescriptorSets(pool, mainLayout, 0, &mainSet, 1);
    { const VriDescriptor* a[1] = {uboView}; VriDescriptorRangeUpdateDesc u[1]{}; u[0].descriptors = a; u[0].descriptorNum = 1; c.UpdateDescriptorRanges(shadowSet, 0, 1, u); }
    { const VriDescriptor* a[1] = {uboView}; const VriDescriptor* b[1] = {shadowView}; const VriDescriptor* s[1] = {shadowSampler};
      VriDescriptorRangeUpdateDesc u[3]{}; u[0].descriptors=a; u[0].descriptorNum=1; u[1].descriptors=b; u[1].descriptorNum=1; u[2].descriptors=s; u[2].descriptorNum=1; c.UpdateDescriptorRanges(mainSet, 0, 3, u); }
    ...
    app.onUpdate = [ustg, zeroToOne](uint64_t) {
        ...
        s.lightDir[0] = lpos[0]/len; s.lightDir[1] = lpos[1]/len; s.lightDir[2] = lpos[2]/len; s.lightDir[3] = 0;
        s.params[0] = depthBias; s.params[1] = 1.0f / float(kShadow); s.params[2] = zeroToOne; s.params[3] = ambient;
        std::memcpy(app.c.MapBuffer(ustg, 0, sizeof(SceneUbo)), &s, sizeof(SceneUbo)); app.c.UnmapBuffer(ustg);
    };
    app.onGui = [] {
    ...
    auto drawScene = [vbuf, ibuf](VriCommandBuffer* cmd, VriPipelineLayout* layout, VriDescriptorSet* set) {
        ...
        for (const Obj& o : g_objs)
        {
            ...
            VriDrawIndexedDesc di{}; di.indexNum = o.indexNum; di.instanceNum = 1; di.baseIndex = o.baseIndex; di.vertexOffset = o.vertexOffset;
    ...
    app.onPreRender = [=](VriCommandBuffer* cmd) {
        VriBufferCopyDesc ucp{}; ucp.size = sizeof(SceneUbo); c.CmdCopyBuffer(cmd, ubo, ustg, &ucp);
        ...
        VriTextureBarrierDesc tw{}; tw.texture = shadowTex; tw.aspect = VriImageAspect_Depth;
        tw.before.layout = g_shadowInit ? VriLayout_ShaderResource : VriLayout_Undefined; tw.before.stages = g_shadowInit ? VriPipelineStage_FragmentShader : VriPipelineStage_None;
        tw.after.access = VriAccess_DepthStencilAttachmentWrite; tw.after.layout = VriLayout_DepthStencilAttachment; tw.after.stages = VriPipelineStage_EarlyFragmentTests;
        VriBarrierGroupDesc gw{}; gw.textures = &tw; gw.textureNum = 1; c.CmdBarrier(cmd, &gw);
        g_shadowInit = true;

        VriAttachmentDesc depthRT{}; depthRT.view = shadowView; depthRT.loadOp = VriAttachmentLoadOp_Clear; depthRT.storeOp = VriAttachmentStoreOp_Store; depthRT.clearValue.depthStencil.depth = 1.0f;
        VriAttachmentsDesc att{}; att.colorNum = 0; att.depth = &depthRT; att.renderArea.width = kShadow; att.renderArea.height = kShadow; att.layerNum = 1;
        c.CmdBeginRendering(cmd, &att);
        VriViewport vp{0, 0, float(kShadow), float(kShadow), 0, 1}; c.CmdSetViewports(cmd, &vp, 1);
        VriRect scr{0, 0, kShadow, kShadow}; c.CmdSetScissors(cmd, &scr, 1);
        c.CmdSetPipeline(cmd, shadowPipeline);
        drawScene(cmd, shadowLayout, shadowSet);
        c.CmdEndRendering(cmd);

        // shadow map -> sampleable by the main pass
        VriTextureBarrierDesc ts{}; ts.texture = shadowTex; ts.aspect = VriImageAspect_Depth;
        ts.before.access = VriAccess_DepthStencilAttachmentWrite; ts.before.layout = VriLayout_DepthStencilAttachment; ts.before.stages = VriPipelineStage_LateFragmentTests;
    ...
    c.DestroyPipeline(mainPipeline); c.DestroyPipelineLayout(mainLayout);
    c.DestroyPipeline(shadowPipeline); c.DestroyPipelineLayout(shadowLayout);
    c.DestroyDescriptor(shadowSampler); c.DestroyDescriptor(shadowView); c.DestroyTexture(shadowTex);
