...
#include "tests/shaders/offscreen_spv.h"   // g_offscreenSpv  (Vulkan + OpenGL)
#include "tests/shaders/offscreen_wgsl.h"  // g_offscreenWgsl (WebGPU)
#include "tests/shaders/offscreen_dxbc.h"  // g_offscreenDxbcVS / PS (D3D12)
#include "tests/shaders/composite_spv.h"   // g_compositeSpv
#include "tests/shaders/composite_wgsl.h"  // g_compositeWgsl
#include "tests/shaders/composite_dxbc.h"  // g_compositeDxbcVS / PS
...
namespace
{
    ...
    const Vertex kCube[24] = {
        {-0.5f,-0.5f, 0.5f, 0,1}, { 0.5f,-0.5f, 0.5f, 1,1}, { 0.5f, 0.5f, 0.5f, 1,0}, {-0.5f, 0.5f, 0.5f, 0,0}, // +Z
        { 0.5f,-0.5f,-0.5f, 0,1}, {-0.5f,-0.5f,-0.5f, 1,1}, {-0.5f, 0.5f,-0.5f, 1,0}, { 0.5f, 0.5f,-0.5f, 0,0}, // -Z
        { 0.5f,-0.5f, 0.5f, 0,1}, { 0.5f,-0.5f,-0.5f, 1,1}, { 0.5f, 0.5f,-0.5f, 1,0}, { 0.5f, 0.5f, 0.5f, 0,0}, // +X
        {-0.5f,-0.5f,-0.5f, 0,1}, {-0.5f,-0.5f, 0.5f, 1,1}, {-0.5f, 0.5f, 0.5f, 1,0}, {-0.5f, 0.5f,-0.5f, 0,0}, // -X
        {-0.5f, 0.5f, 0.5f, 0,1}, { 0.5f, 0.5f, 0.5f, 1,1}, { 0.5f, 0.5f,-0.5f, 1,0}, {-0.5f, 0.5f,-0.5f, 0,0}, // +Y
        {-0.5f,-0.5f,-0.5f, 0,1}, { 0.5f,-0.5f,-0.5f, 1,1}, { 0.5f,-0.5f, 0.5f, 1,0}, {-0.5f,-0.5f, 0.5f, 0,0}, // -Y
    };
...
int main(int, char**)
{
    ...
    VriBuffer* vstg = nullptr; c.CreateBuffer(app.dev, &vsd, &vstg);
    ...
    VriBuffer* istg = nullptr; c.CreateBuffer(app.dev, &isd, &istg);
    ...
    VriSamplerDesc rsmp{}; rsmp.magFilter = VriFilter_Linear; rsmp.minFilter = VriFilter_Linear; rsmp.mipmapMode = VriMipmapMode_Linear;
    ...
    VriSamplerDesc csmp{}; csmp.magFilter = VriFilter_Linear; csmp.minFilter = VriFilter_Linear; csmp.mipmapMode = VriMipmapMode_Nearest;
    csmp.addressModeU = VriAddressMode_ClampToEdge; csmp.addressModeV = VriAddressMode_ClampToEdge; csmp.addressModeW = VriAddressMode_ClampToEdge; csmp.maxLod = 1.0f;
    VriDescriptor* clampSampler = nullptr; c.CreateSampler(app.dev, &csmp, &clampSampler);
    ...
    or_[0].baseRegister = 0; or_[0].descriptorNum = 1; or_[0].descriptorType = VriDescriptorType_ConstantBuffer; or_[0].shaderStages = VriShaderStage_Vertex;
    or_[1].baseRegister = 1; or_[1].descriptorNum = 1; or_[1].descriptorType = VriDescriptorType_Texture;        or_[1].shaderStages = VriShaderStage_Fragment;
    or_[2].baseRegister = 2; or_[2].descriptorNum = 1; or_[2].descriptorType = VriDescriptorType_Sampler;        or_[2].shaderStages = VriShaderStage_Fragment;
    VriDescriptorSetDesc osd{}; osd.registerSpace = 0; osd.ranges = or_; osd.rangeNum = 3;
    VriPipelineLayoutDesc old_{}; old_.descriptorSets = &osd; old_.descriptorSetNum = 1;
    VriPipelineLayout* offLayout = nullptr; c.CreatePipelineLayout(app.dev, &old_, &offLayout);

    VriShaderDesc osh[2]{};
    osh[0].stage = VriShaderStage_Vertex;   osh[0].entryPointName = "vertexMain";
    osh[1].stage = VriShaderStage_Fragment; osh[1].entryPointName = "fragmentMain";
    if (useDxbc) { osh[0].bytecode = g_offscreenDxbcVS; osh[0].bytecodeSize = sizeof(g_offscreenDxbcVS); osh[1].bytecode = g_offscreenDxbcPS; osh[1].bytecodeSize = sizeof(g_offscreenDxbcPS); }
    else { osh[0].bytecode = osh[1].bytecode = useWgsl ? static_cast<const void*>(g_offscreenWgsl) : static_cast<const void*>(g_offscreenSpv);
    ...
    oca[0].format = kOffFormat; oca[0].colorWriteMask = VriColorWrite_RGBA;
    oca[1].format = kOffFormat; oca[1].colorWriteMask = VriColorWrite_RGBA;
    VriGraphicsPipelineDesc opd{};
    opd.pipelineLayout = offLayout; opd.shaders = osh; opd.shaderNum = 2;
    opd.vertexInput.attributes = attrs; opd.vertexInput.attributeNum = 2; opd.vertexInput.streams = &stream; opd.vertexInput.streamNum = 1;
    opd.inputAssembly.topology = VriPrimitiveTopology_TriangleList;
    opd.rasterization.cullMode = VriCullMode_None; opd.rasterization.frontFace = VriFrontFace_CounterClockwise; opd.rasterization.lineWidth = 1.0f;
    opd.multisample.sampleNum = 1;
    opd.depthStencil.depthTest = VRI_TRUE; opd.depthStencil.depthWrite = VRI_TRUE; opd.depthStencil.depthCompareOp = VriCompareOp_Less;
    opd.outputMerger.colors = oca; opd.outputMerger.colorNum = 2; opd.outputMerger.depthStencilFormat = app.depthFormat;
    VriPipeline* offPipeline = nullptr;
    if (c.CreateGraphicsPipeline(app.dev, &opd, &offPipeline) != VriResult_Success) app.Fail("offscreen CreateGraphicsPipeline failed");
    ...
    cr[1].baseRegister = 1; cr[1].descriptorNum = 1; cr[1].descriptorType = VriDescriptorType_Texture; cr[1].shaderStages = VriShaderStage_Fragment;
    cr[2].baseRegister = 2; cr[2].descriptorNum = 1; cr[2].descriptorType = VriDescriptorType_Sampler; cr[2].shaderStages = VriShaderStage_Fragment;
    ...
    if (useDxbc) { csh[0].bytecode = g_compositeDxbcVS; csh[0].bytecodeSize = sizeof(g_compositeDxbcVS); csh[1].bytecode = g_compositeDxbcPS; csh[1].bytecodeSize = sizeof(g_compositeDxbcPS); }
    else { csh[0].bytecode = csh[1].bytecode = useWgsl ? static_cast<const void*>(g_compositeWgsl) : static_cast<const void*>(g_compositeSpv);
    ...
    if (c.CreateGraphicsPipeline(app.dev, &cpd, &compPipeline) != VriResult_Success) app.Fail("composite CreateGraphicsPipeline failed");

    // descriptor sets
    VriDescriptorPoolDesc pdsc{}; pdsc.descriptorSetMaxNum = 2; pdsc.constantBufferMaxNum = 1; pdsc.textureMaxNum = 3; pdsc.samplerMaxNum = 2;
    VriDescriptorPool* pool = nullptr; c.CreateDescriptorPool(app.dev, &pdsc, &pool);
    VriDescriptorSet* offSet = nullptr; c.AllocateDescriptorSets(pool, offLayout, 0, &offSet, 1);
    VriDescriptorSet* compSet = nullptr; c.AllocateDescriptorSets(pool, compLayout, 0, &compSet, 1);
    { const VriDescriptor* a[1] = {uboView}; const VriDescriptor* b[1] = {texView}; const VriDescriptor* s[1] = {repeatSampler};
      VriDescriptorRangeUpdateDesc u[3]{}; u[0].descriptors=a; u[0].descriptorNum=1; u[1].descriptors=b; u[1].descriptorNum=1; u[2].descriptors=s; u[2].descriptorNum=1; c.UpdateDescriptorRanges(offSet, 0, 3, u); }
    { const VriDescriptor* a[1] = {off0View}; const VriDescriptor* b[1] = {off1View}; const VriDescriptor* s[1] = {clampSampler};
      VriDescriptorRangeUpdateDesc u[3]{}; u[0].descriptors=a; u[0].descriptorNum=1; u[1].descriptors=b; u[1].descriptorNum=1; u[2].descriptors=s; u[2].descriptorNum=1; c.UpdateDescriptorRanges(compSet, 0, 3, u); }
    ...
    app.onPreRender = [=](VriCommandBuffer* cmd) {
        ...
        tb[0].texture = off0; tb[0].before.layout = g_offInit ? VriLayout_ShaderResource : VriLayout_Undefined; tb[0].before.stages = g_offInit ? VriPipelineStage_FragmentShader : VriPipelineStage_None;
        ...
        tb[1] = tb[0]; tb[1].texture = off1;
        tb[2].texture = offDepth; tb[2].before.layout = g_offDepthInit ? VriLayout_DepthStencilAttachment : VriLayout_Undefined; tb[2].before.stages = VriPipelineStage_None;
        tb[2].after.access = VriAccess_DepthStencilAttachmentWrite; tb[2].after.layout = VriLayout_DepthStencilAttachment; tb[2].after.stages = VriPipelineStage_EarlyFragmentTests; tb[2].aspect = VriImageAspect_Depth;
        VriBarrierGroupDesc gw{}; gw.textures = tb; gw.textureNum = 3; c.CmdBarrier(cmd, &gw);
        g_offInit = true; g_offDepthInit = true;

        VriAttachmentDesc colors[2]{};
        colors[0].view = off0View; colors[0].loadOp = VriAttachmentLoadOp_Clear; colors[0].storeOp = VriAttachmentStoreOp_Store;
        colors[0].clearValue.color.f32[0] = 0.08f; colors[0].clearValue.color.f32[1] = 0.10f; colors[0].clearValue.color.f32[2] = 0.14f; colors[0].clearValue.color.f32[3] = 1.0f;
        colors[1].view = off1View; colors[1].loadOp = VriAttachmentLoadOp_Clear; colors[1].storeOp = VriAttachmentStoreOp_Store; // normal target clears to black
        VriAttachmentDesc depthRT{}; depthRT.view = offDepthView; depthRT.loadOp = VriAttachmentLoadOp_Clear; depthRT.storeOp = VriAttachmentStoreOp_DontCare; depthRT.clearValue.depthStencil.depth = 1.0f;
        VriAttachmentsDesc att{}; att.colors = colors; att.colorNum = 2; att.depth = &depthRT; att.renderArea.width = kWidth; att.renderArea.height = kHeight; att.layerNum = 1;
        ...
        ts[0].texture = off0; ts[0].before.access = VriAccess_ColorAttachmentWrite; ts[0].before.layout = VriLayout_ColorAttachment; ts[0].before.stages = VriPipelineStage_ColorAttachmentOutput;
        ...
        ts[1] = ts[0]; ts[1].texture = off1;
        VriBarrierGroupDesc gs{}; gs.textures = ts; gs.textureNum = 2; c.CmdBarrier(cmd, &gs);
    };
    ...
    c.DestroyPipeline(offPipeline); c.DestroyPipelineLayout(offLayout);
    c.DestroyDescriptor(off0View); c.DestroyDescriptor(off1View); c.DestroyDescriptor(offDepthView);
    c.DestroyTexture(off0); c.DestroyTexture(off1); c.DestroyTexture(offDepth);
    c.DestroyDescriptor(clampSampler); c.DestroyDescriptor(repeatSampler);
