...
#include "shaders/examples/rt_gltf_rayquery_spv.h"  // g_rtGltfRayquerySpv  (Metal/VK: inline ray query CS)
#include "shaders/examples/rt_gltf_software_spv.h"  // g_rtGltfSoftwareSpv  (compute BVH fallback, GL/VK)
#include "shaders/examples/rt_gltf_software_wgsl.h" // g_rtGltfSoftwareWgsl (compute BVH fallback, WebGPU)
#include "shaders/examples/rt_gltf_spv.h"           // g_rtGltfSpv          (Vulkan: raygen/miss/closesthit)
...
#include "shaders/examples/show_tex_wgsl.h"         // g_showTexWgsl        (WebGPU display)
#if defined(_WIN32)
#include "shaders/examples/rt_gltf_dxil.h"          // g_rtGltfDxilRGEN/MISS/CHIT (D3D12 DXR library, per stage)
#include "shaders/examples/rt_gltf_rayquery_dxil.h" // g_rtGltfRayqueryDxilCS     (D3D12 inline ray query CS)
#include "shaders/examples/rt_gltf_software_dxil.h" // g_rtGltfSoftwareDxilCS      (D3D12 compute BVH fallback)
#endif
...
int main(int argc, char** argv)
{
    ...
    app.onGui = [hasRtPipeline, hasRayQuery, useSoftware] {
        ...
        {
            ImGui::TextColored(ImVec4(1, 0.5f, 0.5f, 1), "ray tracing unsupported");
            ImGui::Text("needs RT/ray query or compute");
            return;
        }
        ImGui::Text("%s",
                    hasRtPipeline ?
                        "RT pipeline (raygen/miss/hit + SBT)" :
                        (hasRayQuery ? "inline ray query (compute, Metal-style)" : "software BVH (compute)"));
    ...
    if (useSoftware)
    {
        ...
        rr[rn].descriptorNum  = 1;
        rr[rn].descriptorType = VriDescriptorType_StorageBuffer;
        rr[rn].shaderStages   = VriShaderStage_Compute;
        ++rn;
    ...
    VriPipelineLayout* traceLayout = nullptr;
    if (c.CreatePipelineLayout(app.dev, &tld, &traceLayout) != VriResult_Success)
        app.Fail("CreatePipelineLayout (trace) failed");
    ...
    pdsc.storageBufferMaxNum         = useSoftware ? 5 : 3;
    ...
    VriDescriptorSet* traceSet = nullptr;
    c.AllocateDescriptorSets(pool, traceLayout, 0, &traceSet, 1);
    {
        ...
        const VriDescriptor*         tr[1] = {triRefView};
        VriDescriptorRangeUpdateDesc u[10] {};
        uint32_t                     un = 0;
        if (!useSoftware)
        {
            u[un].descriptors   = as;
    ...
    VriPipeline* rtPipeline      = nullptr;
    ...
    {
        VriShaderDesc sh[3] = {
            // SPIR-V (VK) or per-stage DXIL (D3D12); the RT pipeline never runs on WebGPU
            app.Shader(VriShaderStage_RayGen,
                       "rayGenMain",
                       {VRI_SHADER_BLOB(g_rtGltfSpv), nullptr, 0, VRI_SHADER_D3D12(g_rtGltfDxilRGEN)}),
            app.Shader(VriShaderStage_Miss,
                       "missMain",
                       {VRI_SHADER_BLOB(g_rtGltfSpv), nullptr, 0, VRI_SHADER_D3D12(g_rtGltfDxilMISS)}),
            app.Shader(VriShaderStage_ClosestHit,
                       "closestHitMain",
                       {VRI_SHADER_BLOB(g_rtGltfSpv), nullptr, 0, VRI_SHADER_D3D12(g_rtGltfDxilCHIT)}),
        };
        ...
        rtpd.pipelineLayout    = traceLayout;
        rtpd.shaders           = sh;
        rtpd.shaderNum         = 3;
        rtpd.groups            = groups;
        rtpd.groupNum          = 3;
        rtpd.maxRecursionDepth = 1;
        if (rt.CreateRayTracingPipeline(app.dev, &rtpd, &rtPipeline) != VriResult_Success)
            app.Fail("CreateRayTracingPipeline failed");

        const uint32_t handleSize = dd->rtShaderGroupHandleSize;
        baseAlign                 = dd->rtShaderGroupBaseAlignment;
        std::vector<uint8_t> handles(size_t(3) * handleSize);
        if (rt.GetShaderGroupHandles(rtPipeline, 0, 3, handles.size(), handles.data()) != VriResult_Success)
            app.Fail("GetShaderGroupHandles failed");
        VriBufferDesc sbtd {};
        sbtd.size           = uint64_t(baseAlign) * 3;
        sbtd.usage          = VriBufferUsage_ShaderBindingTable;
        sbtd.memoryLocation = VriMemoryLocation_HostUpload;
        c.CreateBuffer(app.dev, &sbtd, &sbt);
        uint8_t* p = static_cast<uint8_t*>(c.MapBuffer(sbt, 0, sbtd.size));
        std::memset(p, 0, sbtd.size);
        for (uint32_t i = 0; i < 3; ++i)
            std::memcpy(p + uint64_t(i) * baseAlign, handles.data() + size_t(i) * handleSize, handleSize);
        c.UnmapBuffer(sbt);
    }
    else // compute: Metal-style inline ray query, or the software BVH fallback (no RT hardware)
    {
        ...
        const vriex::ExampleApp::ShaderVariants traceCs =
            useSoftware ?
                vriex::ExampleApp::ShaderVariants {VRI_SHADER_BLOB(g_rtGltfSoftwareSpv),
                                                   VRI_SHADER_BLOB(g_rtGltfSoftwareWgsl),
                                                   VRI_SHADER_D3D12(g_rtGltfSoftwareDxilCS)} :
                vriex::ExampleApp::ShaderVariants {
                    VRI_SHADER_BLOB(g_rtGltfRayquerySpv), nullptr, 0, VRI_SHADER_D3D12(g_rtGltfRayqueryDxilCS)};
        VriComputePipelineDesc cpd {};
        cpd.pipelineLayout = traceLayout;
        ...
        if (c.CreateComputePipeline(app.dev, &cpd, &computePipeline) != VriResult_Success)
            app.Fail("CreateComputePipeline (compute trace) failed");
    }
    ...
    app.onUpdate = [=](uint64_t) {
        ...
        std::memcpy(c.MapBuffer(camStg, 0, sizeof(Camera)), &cam, sizeof(Camera));
        c.UnmapBuffer(camStg);
    };

    static bool                 g_imgInit = false;
    const VriPipelineStageFlags traceStageBit =
        hasRtPipeline ? VriPipelineStage_RayTracingShader : VriPipelineStage_ComputeShader;
    app.onPreRender = [=](VriCommandBuffer* cmd) {
        VriBufferCopyDesc ucp {};
        ucp.size = sizeof(Camera);
        ...
        tr.before.stages = traceStageBit;
    ...
    std::printf("[raytracing] %s tracing '%s' (%zu primitives) into %ux%u\n",
                hasRtPipeline ? "RT pipeline" : "inline ray query (compute)",
                modelPath,
