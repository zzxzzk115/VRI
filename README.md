...
**Modern-feature extensions** are exposed as separate queryable interfaces (`vriGetInterface`), registered by a backend only when the backing feature was granted at device creation — so absence is explicit (`Unsupported`), never a silent no-op. **Variable Rate Shading** (`VRI_INTERFACE_VRS`, per-draw coarse shading rate) is implemented and tested on Vulkan (`VK_KHR_fragment_shading_rate`); the test self-skips on adapters that lack it (e.g. CI software rasterizers). Ray tracing and mesh-shader interfaces are scaffolded and land next.
