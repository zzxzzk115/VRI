...
- **Valid input behaves the same on every backend.** Where a backend has a quirk that isn't a *capability* difference, VRI absorbs it rather than leaking it to the app: `CmdUploadBufferToTexture` takes tightly-packed pixel data of any size and handles D3D12's 256-byte row-pitch / WebGPU's `bytesPerRow` alignment internally (bouncing through a padded buffer only when needed); integer vertex attributes (`R32_SINT`, …) work uniformly (GL `glVertexAttribIPointer`, WebGPU `Sint32`); 2D-array textures work on GL/WebGL2 too. The app writes one path; VRI makes it portable.
- **Shaders authored in [Slang](https://github.com/shader-slang/slang)**, compiled offline (`tools/vri-shaderc`) to per-target output — SPIR-V (Vulkan; OpenGL/WebGL via SPIRV-Cross→GLSL/ESSL; **Metal via SPIRV-Cross→MSL**, both at pipeline creation), WGSL (WebGPU), and DXBC/DXIL (Direct3D 12). The runtime consumes precompiled bytecode; it never links the Slang frontend.
...
| Metal (native) | macOS (Apple Silicon) | ✅ resources, explicit memory (`MTLHeap`), descriptor sets, graphics + compute pipelines (SPIR-V→MSL via SPIRV-Cross), command recording, `CAMetalLayer` swapchain, `MTLSharedEvent` timeline fences; **hardware ray query** + **mesh shaders**. Y-up native (no viewport flip). Verified on Apple-silicon hardware |
| OpenGL ES (native) | Linux / embedded (Raspberry Pi) | ✅ the same GL backend on the GLES3 subset, driven by a GLFW-free **EGL** context (`--vri_backend_gl_es=y`). Comes up headless via the surfaceless platform (renders into FBOs) so it runs over SSH with no display server; windowed present works on both **X11** (window XID) and **Wayland** (`wl_egl_window`; the app passes its `wl_display` via `VriDeviceCreationDesc::nativeDisplay` so the EGL context + present surface share one connection). Verified on a Pi CM4 (V3D, ES 3.1): offscreen render+readback parity (`vri-tests`, 46/46) headless on the GPU, **and** the windowed examples presenting on a wayfire (Wayland) session. No compute yet (ES 3.1 dispatch is a follow-up) — reported via `hasComputeShader` |
...
Windows / Linux (Vulkan, WebGPU, OpenGL) and Web (Emscripten → WebGL2 **and** WebGPU), all built + tested in CI. **macOS** runs the **native Metal backend** (core + swapchain, ray query, mesh shaders), verified on Apple-silicon hardware; MoltenVK (Vulkan) and desktop OpenGL (capped at 4.1) also run there. CI compile-verifies macOS but has no GPU (no MoltenVK ICD; software GL caps at 4.1), so Metal/GPU validation is on-device. Android / iOS are planned per backend.
...
# windowed examples (backend auto-picked; override with VRI_API=vulkan|webgpu|opengl|d3d12|metal)
...
`?backend=webgpu` / `?backend=webgl` URL query (or `VRI_API=vulkan|webgpu|opengl|d3d12|metal` on
